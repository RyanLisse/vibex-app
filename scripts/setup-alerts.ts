...
class AlertSetup {
  ...
  private async runMigrations(): Promise<void> {
    ...
    try {
      ...
      if (existsSync('drizzle.config.ts')) {
        ...
        const migrationPath = path.join(process.cwd(), 'db/migrations/001_create_alert_tables.sql')
  ...
  private async testChannels(): Promise<void> {
    ...
    try {
      ...
      console.log('Start your application and visit /alerts to test channels manually')
    } catch (error) {
