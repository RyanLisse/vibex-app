...
function extractMockIdeas(
	...
	if (
		...
		transcript.includes("artificial intelligence")
	) {
		ideas.push({
...
async function runVoiceBrainstormDemo() {
	...
	log(
		"This demo simulates a complete brainstorming session with mock data.\n",
		"cyan",
	);
	...
	try {
		...
		for (let i = 0; i < DEMO_CONFIG.mockTranscripts.length; i++) {
			...
			logStep(
				`Processing voice input ${i + 1}/${DEMO_CONFIG.mockTranscripts.length}...`,
			);
			...
			if (voiceResponse.extractedIdeas.length > 0) {
				log(
					`  💡 Ideas extracted: ${voiceResponse.extractedIdeas.length}`,
					"magenta",
				);
		...
		const uniqueIdeas = Array.from(
			new Set(allIdeas.map((idea) => idea.content)),
		);
		const uniqueInsights = Array.from(new Set(allInsights));

		log("Brainstorming Session Complete!", "bright");
		log(
			`  • Duration: ${DEMO_CONFIG.sessionDuration / 60} minutes (simulated)`,
			"green",
		);
		log(
			`  • Voice Inputs Processed: ${DEMO_CONFIG.mockTranscripts.length}`,
			"green",
		);
		...
		log(
			"The Voice Brainstorming System Demo has completed successfully!",
			"green",
		);
...
async function runPerformanceTest() {
	...
	log(
		`  • Average per Session: ${(duration / results.length).toFixed(2)}ms`,
		"green",
	);
	log(
		`  • Ideas Generated: ${results.reduce((sum, r) => sum + r.ideas, 0)}`,
		"green",
	);
}
