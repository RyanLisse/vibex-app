...
async function fixBunTestImports(filePath: string): Promise<void> {
  try {
    ...
    if (content.includes('import { test, expect, describe, it, beforeEach, afterEach, mock } from "bun:test"')) {
      return
    }
    ...
    if (importIndex !== -1) {
      // Add bun:test import
      lines.splice(importIndex, 0, 'import { test, expect, describe, it, beforeEach, afterEach, mock } from "bun:test"')
    } else {
      // Add at the beginning
      lines.unshift('import { test, expect, describe, it, beforeEach, afterEach, mock } from "bun:test"')
    }
  ...
  for (const dir of testDirs) {
    try {
      ...
      for (const file of files) {
        if (typeof file === 'string' && file.endsWith('.test.ts') && !file.includes('integration')) {
