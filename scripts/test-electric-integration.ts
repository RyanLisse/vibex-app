...
async function testSyncService(): Promise<boolean> {
	...
	try {
		...
		// Test force sync
		await enhancedElectricSyncService.forceSyncTable(
			"tasks",
			"test-user-electric",
		);
		...
		logInfo(
			`Cache statistics: ${cacheStats.totalKeys} keys, ${cacheStats.hitRate}% hit rate`,
		);
...
async function runElectricIntegrationTests(): Promise<void> {
	...
	try {
		...
		if (allTestsPassed) {
			logSuccess(
				"All ElectricSQL integration tests passed! Real-time sync is working correctly.",
			);
		} else {
