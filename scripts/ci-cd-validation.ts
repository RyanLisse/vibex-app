	...
	async validate(): Promise<ValidationReport> {
		...
		if (criticalFailures.length > 0) {
			...
			issues.push(
				...criticalFailures.map(
					(r) => `${r.name}: ${r.errors?.[0] || "Unknown error"}`,
				),
			);
		}
		...
		if (!hasIntegrationTests) {
			...
			recommendations.push(
				"Fix integration test configuration and hanging issues",
			);
		}

		const hasComponentTests = this.results.some(
			(r) => r.name.includes("Component") && r.success,
		);
		...
		if (totalDuration > 300_000) {
			// 5 minutes
			recommendations.push(
				"Total test suite time exceeds 5 minutes - consider parallel execution",
			);
		}
