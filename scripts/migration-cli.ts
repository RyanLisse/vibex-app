...
program
	...
	.action(async (options) => {
		try {
			...
			const { dataMigrationManager } = await import(
				"../lib/migration/data-migration"
			);
			...
			if (currentMigration) {
				console.log(
					`Current Migration: ${colorize(currentMigration.status, "cyan")}`,
				);
				console.log(
			...
			console.log(
				`Environments: ${migrationCheck.localStorageData.environments}`,
			);
			...
			if (options.verbose && backups.length > 0) {
				backups.slice(0, 5).forEach((backup) => {
					console.log(
						`  ${backup.id} (${backup.totalItems} items, ${formatBytes(backup.size)})`,
					);
				});
			}
		} catch (error) {
...
program
	...
	.action(async (options) => {
		try {
			...
			const result = await dataMigrationManager.startMigration(
...
program
	...
	.option(
		"-s, --source <source>",
		"Backup source (LOCALSTORAGE or DATABASE)",
		"LOCALSTORAGE",
	)
	...
	.action(async (options) => {
		try {
			...
			if (result.data.environments) {
				...
				if (options.verbose) {
					...
					if (result.data.environments.length > 3) {
						console.log(
							`  ... and ${result.data.environments.length - 3} more`,
						);
					}
				}
			}
			...
			if (
				...
				Object.keys(result.data.formData).length > 0
			) {
				console.log(
					`Form data keys: ${Object.keys(result.data.formData).length}`,
				);
				if (options.verbose) {
