...
async function getAllTestFiles(): Promise<TestFile[]> {
  ...
  const findTestFiles = async (dir: string, currentPath = ''): Promise<void> => {
        ...
        if (item.isDirectory() && !item.name.includes('node_modules')) {
          ...
          if (nameWithoutExt.includes('.test') || nameWithoutExt.includes('.spec')) {
            ...
            if (E2E_TEST_DIRS.some((e2eDir) => fullFilePath.startsWith(e2eDir))) {
              ...
              fullFilePath.includes('integration') ||
              nameWithoutExt.includes('integration')
            ) {
              type = 'integration'
              runner = 'vitest'
            }
  ...
  try {
    ...
    if (testType === 'unit') {
      ...
      updatedContent = updatedContent
        ...
        .replace(
          /import { test, expect, describe, it, beforeEach, afterEach, vi } from ['"]vitest['"];?\n/g,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const file of plan.filesToUpdate) {
      ...
      const testType = plan.unitTests.find((t) => t.path === file.path)
        ? 'unit'
        : plan.integrationTests.find((t) => t.path === file.path)
          ? 'integration'
          : 'e2e'
