...
<<<<<<< HEAD
	return new Promise((resolve) => {
		...
		const proc = spawn(
			"bun",
			["run", "vitest", "run", `--project=${project}`, "--reporter=json"],
			{
				cwd: process.cwd(),
		...
		proc.on("close", (code) => {
			...
			try {
				...
				const passed =
					result.numTotalTests - result.numFailedTests - result.numTodoTests;
...
async function main() {
	...
	if (totalFailed > 0 || totalSkipped > 0) {
		...
		if (totalFailed > 0) {
			console.log(
				"   - Fix failing tests by running: bun run test:all --reporter=verbose",
			);
		}
		if (totalSkipped > 0) {
