...
async function main() {
	...
	// Run each test suite
	results.push(
		await runTestSuite("Unit", "bunx vitest run --config=vitest.config.ts"),
	);
	results.push(
		await runTestSuite(
			"Components",
			"bunx vitest run --config=vitest.components.config.ts",
		),
	);
	results.push(
		await runTestSuite(
	...
	results.forEach((r) => {
		const percentage =
			r.total > 0 ? ((r.passed / r.total) * 100).toFixed(1) : "0.0";
		console.log(`\n${r.suite} Tests:`);
	...
	if (totals.skipped > 0) {
		...
		console.log(
			'Run "grep -r ".skip" --include="*.test.*" ." to find skipped tests',
		);
	}
