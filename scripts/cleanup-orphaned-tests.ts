...
async function analyzeTestContent(
	testPath: string,
): Promise<{ lines: number; hasTests: boolean }> {
...
async function generateCleanupReport() {
	...
	if (smallTests.length > 0) {
		console.log(
			`Small orphaned tests (${smallTests.length}) - Review before deletion:`,
		);
	...
	if (largeTests.length > 0) {
		console.log(
			`Large orphaned tests (${largeTests.length}) - May contain valuable tests:`,
		);
		largeTests.forEach((test) => {
			console.log(`  - ${test.testPath} (${test.lines} lines)`);
			console.log(
				`    Expected sources: ${test.expectedSourcePaths.slice(0, 2).join(", ")}`,
			);
		});
		console.log();
	}
	...
	const cleanupScriptPath = join(
		process.cwd(),
		"scripts/remove-orphaned-tests.sh",
	);
