...
async function collectTestHealth(): Promise<TestHealth> {
	...
	try {
		const analysisReport = JSON.parse(
			await fs.readFile(
				join(projectRoot, "test-analysis-report.json"),
				"utf-8",
			),
		);
	...
	try {
		const relevanceReport = JSON.parse(
			await fs.readFile(
				join(projectRoot, "test-relevance-report.json"),
				"utf-8",
			),
		);
		const criticalTests = relevanceReport.tests.filter(
			(t: any) => t.relevanceScore < 30,
		);
	...
	try {
		const optimizationReport = JSON.parse(
			await fs.readFile(
				join(projectRoot, "test-optimization-report.json"),
				"utf-8",
			),
		);
