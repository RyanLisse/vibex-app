...
async function testTaskInsertion(): Promise<boolean> {
  ...
  try {
    ...
    // Verify insertion
    const inserted = await db.select().from(tasks).where(eq(tasks.id, sampleTask.id)).limit(1)
    ...
    logError(`Task insertion failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    return false
  }
}
...
async function cleanupTestData(): Promise<void> {
  ...
  try {
    ...
    await db.delete(environments).where(eq(environments.id, sampleEnvironment.id))
    logSuccess('Test data cleaned up')
  } catch (error) {
    logError(`Cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
async function runBasicTests(): Promise<void> {
  ...
  try {
    ...
    if (allTestsPassed) {
      logSuccess('All basic tests passed! Core migration functionality is working.')
    } else {
