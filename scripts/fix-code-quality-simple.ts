...
async function fixLocalStorageFile() {
  const filePath = '/Volumes/Main SSD/CascadeProjects/experiments/vibekit/templates/codex-clone/tests/mocks/local-storage.ts'
  ...
  for (const filePath of filesToCheck) {
    try {
      ...
      if (nonNullMatches && nonNullMatches.length > 0) {
        console.log(`⚠️  Found ${nonNullMatches.length} non-null assertions in ${path.basename(filePath)}`)
      }
    } catch (error) {
