...
function generateNewPath(originalPath: string, category: string): string {
	...
	return path.join(
		testDirs[category as keyof typeof testDirs],
		subDir,
		fileName,
	);
}
...
function updateImportPaths(
	...
	if (updatedContent.includes("./vitest.setup.ts")) {
		const setupPath = path.relative(
			newDir,
			path.join(projectRoot, "tests/setup"),
		);
	...
	try {
		...
		for (const filePath of testFiles) {
			try {
				...
				console.log(
					`✓ Migrated ${filePath} → ${path.relative(projectRoot, newPath)} (${category})`,
				);
