 ...
 */
function categorizeTest(filePath: string, content: string): 'unit' | 'integration' | 'component' | 'e2e' {
  const normalizedPath = filePath.toLowerCase()
  
  // E2E tests
  if (normalizedPath.includes('e2e/') || normalizedPath.includes('.e2e.') || normalizedPath.includes('playwright')) {
    return 'e2e'
  }
...
async function findTestFiles(): Promise<string[]> {
  ...
  return await glob(patterns, { cwd: projectRoot })
}
  ...
  for (const [category, dir] of Object.entries(testDirs)) {
    try {
      ...
      const testFiles = files.filter(file => 
        typeof file === 'string' && file.match(/\.(test|spec)\.(ts|tsx|js|jsx)$/)
      )
