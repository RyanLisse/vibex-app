...
function setupTestData(): void {
	...
	if (typeof localStorage !== "undefined") {
		...
		logSuccess(
			`Set up ${sampleTasks.length} tasks and ${sampleEnvironments.length} environments`,
		);
	} else {
...
async function testDataExtraction(): Promise<boolean> {
	...
	try {
		...
		if (result.errors.length > 0) {
			logError(
				`Extraction errors: ${result.errors.map((e) => e.message).join(", ")}`,
			);
			return false;
		}
...
async function testMigrationCheck(): Promise<boolean> {
	...
	try {
		...
		logError(
			`Migration check failed: ${error instanceof Error ? error.message : "Unknown error"}`,
		);
		return false;
	}
}
