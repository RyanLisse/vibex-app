...
async function profileTest(testFile: string): Promise<TestPerformanceData | null> {
  const startTime = Date.now()
  const startMemory = process.memoryUsage().heapUsed

  return new Promise((resolve) => {
    const child = spawn('bunx', ['vitest', 'run', testFile, '--reporter=json'], {
      stdio: ['inherit', 'pipe', 'pipe'],
    })
...
async function profileAllTests(): Promise<PerformanceReport> {
  ...
  for (let i = 0; i < testFiles.length; i += batchSize) {
    const batch = testFiles.slice(i, i + batchSize)
    const batchResults = await Promise.all(batch.map((file) => profileTest(file)))
  ...
  const memoryIntensiveTests = [...results].sort((a, b) => b.memory - a.memory).slice(0, 20)
  ...
  return {
    ...
    failingTests,
    averageDuration: totalDuration / results.length,
    averageMemory: results.reduce((sum, r) => sum + r.memory, 0) / results.length,
  }
}
...
async function generatePerformanceReport() {
  ...
  console.log(`Total tests: ${report.totalTests}`)
  console.log(`Total duration: ${(report.totalDuration / 1000).toFixed(2)}s`)
  console.log(`Average duration per file: ${(report.averageDuration / 1000).toFixed(2)}s`)
  console.log(`Average memory per file: ${report.averageMemory.toFixed(2)}MB`)
  ...
  report.slowestTests.slice(0, 10).forEach((test, i) => {
    console.log(`${i + 1}. ${test.file}`)
    console.log(`   Duration: ${(test.duration / 1000).toFixed(2)}s`)
    console.log(`   Tests: ${test.tests} (${test.passed} passed, ${test.failed} failed)`)
  })
  ...
  if (slowTests.length > 0) {
    ...
    console.log('  Consider splitting these tests or optimizing their performance.\n')
  }
  ...
  if (memoryHogs.length > 0) {
    ...
    console.log('  Consider checking for memory leaks or reducing test data size.\n')
  }
>>>>>>> ryan-lisse/review-this-pr
}
