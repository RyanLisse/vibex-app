...
export default {
  ...
  const testGroups = {
    ...
    'test:group:2': priority1Tests.slice(Math.ceil(priority1Tests.length / 3), Math.ceil(priority1Tests.length * 2 / 3)),
...
${Array.from(testsByCategory.entries()).map(([cat, tests]) => `
