...
const testCategories: TestCategory[] = [
  ...
  { name: 'integration', pattern: /\.(integration\.test|int\.test)\.(ts|tsx)$/, priority: 2 },
  { name: 'e2e', pattern: /\.(e2e\.test|spec)\.(ts|tsx)$/, directory: 'e2e', priority: 3 },
  { name: 'component', pattern: /\.test\.(tsx)$/, directory: 'components', priority: 1 },
  ...
  { name: 'database', pattern: /\.(db|database|schema).*\.test\.(ts)$/, priority: 2 },
  { name: 'performance', pattern: /\.(perf|performance|benchmark).*\.test\.(ts)$/, priority: 3 },
  { name: 'security', pattern: /\.(security|auth).*\.test\.(ts)$/, priority: 3 },
]
...
async function categorizeTest(testPath: string): Promise<CategorizedTest> {
  ...
  if (content.includes('@testing-library/react')) {
    category = 'component'
    priority = 1
  } else if (content.includes('supertest') || content.includes('request(app)')) {
    category = 'api'
    priority = 2
...
async function generateTestConfigurations() {
  ...
  const categorizedTests = await Promise.all(testFiles.map((file) => categorizeTest(file)))
  ...
  for (const [category, tests] of testsByCategory) {
    ...
    console.log(`  Average lines per test: ${Math.round(totalLines / tests.length)}`)
  }
  ...
  const priority2Tests = categorizedTests.filter((t) => t.priority === 2).map((t) => t.path)

  // Priority 3 (E2E/Performance tests)
  const priority3Tests = categorizedTests.filter((t) => t.priority === 3).map((t) => t.path)
...
export default {
  ...
  const testGroups = {
    'test:group:1': priority1Tests.slice(0, Math.ceil(priority1Tests.length / 3)),
    'test:group:2': priority1Tests.slice(
      Math.ceil(priority1Tests.length / 3),
      Math.ceil((priority1Tests.length * 2) / 3)
    ),
    'test:group:3': priority1Tests.slice(Math.ceil((priority1Tests.length * 2) / 3)),
