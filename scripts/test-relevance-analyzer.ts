...
async function analyzeTestRelevance(testPath: string): Promise<TestRelevance> {
	...
	const hasAsyncTests = /\b(async\s+\(\)|\.resolves|\.rejects|await\s+)/m.test(
		content,
	);
	...
	if (stats.mtime < new Date(Date.now() - 90 * 24 * 60 * 60 * 1000))
		relevanceScore -= 10; // >90 days old
...
async function generateRelevanceReport() {
	...
	for (const analysis of analyses) {
		if (analysis.sourcePath) {
			const exists = await checkSourceFileExists(
				analysis.sourcePath,
				analysis.testPath,
			);
			if (!exists) {
	...
	const warningTests = analyses.filter(
		(a) => a.relevanceScore >= 30 && a.relevanceScore < 60,
	);
