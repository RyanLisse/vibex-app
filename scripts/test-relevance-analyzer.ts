...
async function generateRelevanceReport() {
  ...
  const analyses = await Promise.all(
    testFiles.map(file => analyzeTestRelevance(file))
  );
...
${criticalTests.filter(t => t.testCount === 0).map(t => `echo "  Empty test: ${t.testPath}"`).join('\n')}

echo ""
echo "Tests with missing source files:"
${criticalTests.filter(t => t.issues.includes('Source file not found')).map(t => `echo "  ${t.testPath}"`).join('\n')}

echo ""
echo "Tests with Jest patterns (need Vitest migration):"
${criticalTests.filter(t => t.issues.includes('Uses Jest mocks instead of Vitest')).map(t => `echo "  ${t.testPath}"`).join('\n')}
`;
