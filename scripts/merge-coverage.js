...
function mergeLcovFiles() {
  ...
  // Read Bun coverage
  const bunLcov = readLcovFile(path.join(COVERAGE_DIRS.bun, COVERAGE_FILES.lcov))
  if (bunLcov) {
  ...
  const vitestLcov = readLcovFile(path.join(COVERAGE_DIRS.vitest, COVERAGE_FILES.lcov))
  if (vitestLcov) {
  ...
  const integrationLcov = readLcovFile(path.join(COVERAGE_DIRS.integration, COVERAGE_FILES.lcov))
  if (integrationLcov) {
...
function mergeJsonCoverage() {
  ...
  // Read Bun coverage
  const bunCoverage = readJsonCoverage(path.join(COVERAGE_DIRS.bun, COVERAGE_FILES.json))
  Object.assign(mergedCoverage, bunCoverage)

  // Read Vitest component coverage
  const vitestCoverage = readJsonCoverage(path.join(COVERAGE_DIRS.vitest, COVERAGE_FILES.json))
...
function generateCoverageSummary() {
  ...
  summary.push(`- LCOV: ${path.join(COVERAGE_DIRS.merged, COVERAGE_FILES.lcov)}`)
  summary.push(`- JSON: ${path.join(COVERAGE_DIRS.merged, COVERAGE_FILES.json)}`)
  summary.push('')
...
function main() {
  ...
  if (!hasAnyCoverage) {
    console.error('✗ No coverage directories found. Run tests with coverage first.')
  ...
  if (fileExists(path.join(COVERAGE_DIRS.merged, 'index.html'))) {
    console.log(`  📊 View HTML report: open ${path.join(COVERAGE_DIRS.merged, 'index.html')}`)
  }
  console.log(`  📄 LCOV file: ${path.join(COVERAGE_DIRS.merged, COVERAGE_FILES.lcov)}`)
  console.log(`  📊 JSON file: ${path.join(COVERAGE_DIRS.merged, COVERAGE_FILES.json)}`)
}
