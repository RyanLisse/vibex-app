...
vi.mock("@/components/ui/code-block", () => ({
	...
	CodeBlockBody: ({ children }: any) => (
		<div data-testid="code-block-body">{children}</div>
	),
	...
	CodeBlockSelect: ({ children }: any) => (
		<div data-testid="select">{children}</div>
	),
	CodeBlockSelectContent: ({ children }: any) => (
		<div data-testid="select-content">
			{children && children({ language: "javascript" })}
		</div>
	),
	...
	CodeBlockSelectValue: () => (
		<span data-testid="select-value">JavaScript</span>
	),
}));
...
\`\`\``}
		...
		expect(screen.getByTestId("code-block-content")).toHaveAttribute(
			"data-language",
			"javascript",
		);
	});
...
\`\`\``}
	...
	it("should memoize component based on children prop", () => {
		const { rerender } = render(
			<AIResponse className="test">Content</AIResponse>,
		);
