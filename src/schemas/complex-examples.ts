...
export const QuestionSchema = z
	...
	.refine(
		(data) => {
			...
			if (
				requiresOptions.includes(data.type) &&
...
export const ApiEndpointSchema = z.object({
	...
	authentication: z
		.object({
			required: z.boolean().default(true),
			type: z
				.enum(["none", "basic", "bearer", "oauth2", "apikey"])
				.default("bearer"),
...
export const FormFieldSchema = z.object({
	...
	styling: z
		.object({
			...
			variant: z
				.enum(["default", "outlined", "filled", "underlined"])
				.default("default"),
			color: z
				.string()
				.regex(/^#[0-9A-F]{6}$/i, "Invalid hex color")
