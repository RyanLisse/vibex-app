...
export const ProductSchema = z.object({
  ...
  const expectedCombinations = data.options.reduce((acc, option) => acc * option.values.length, 1)
...
export const SocialLinkSchema = z.object({
  platform: z.enum(['twitter', 'linkedin', 'github', 'facebook', 'instagram', 'youtube', 'website']),
...
export const UserProfileSchema = z.object({
  ...
  profile: z.object({
    ...
    contact: z.object({
      ...
      socialLinks: z.array(SocialLinkSchema).max(10, 'Maximum 10 social links allowed').default([]),
    }).default({}),
...
export const QuestionSchema = z.object({
  ...
  logic: z.object({
    conditions: z.array(z.object({
      field: z.string().uuid(),
      operator: z.enum(['equals', 'not_equals', 'contains', 'not_contains', 'greater', 'less', 'is_empty', 'is_not_empty']),
      value: z.any(),
    })).default([]),
...
export const ApiEndpointSchema = z.object({
  ...
  requestBody: z.object({
    required: z.boolean().default(false),
    contentType: z.enum(['application/json', 'application/xml', 'application/x-www-form-urlencoded', 'multipart/form-data']).default('application/json'),
...
export const FormFieldSchema = z.object({
  ...
  type: z.enum([
    ...
    'date', 'time', 'datetime', 'range', 'color',
    'file', 'image', 'video', 'audio',
    'rich-text', 'code', 'json', 'markdown',
    'address', 'location', 'payment', 'signature',
    'rating', 'slider', 'toggle', 'divider', 'html'
  ]),
  name: z.string().min(1, 'Field name is required').regex(/^[a-zA-Z][a-zA-Z0-9_]*$/, 'Invalid field name'),
  ...
  conditional: z.object({
    show: z.object({
      field: z.string().uuid(),
      operator: z.enum(['equals', 'not_equals', 'contains', 'not_contains', 'greater', 'less', 'greater_equal', 'less_equal', 'is_empty', 'is_not_empty']),
      value: z.any(),
    }).optional(),
    hide: z.object({
      field: z.string().uuid(),
...
export const validateConditionalField = (
  ...
  if (field.conditional) {
    const { show: showCondition, hide: hideCondition, require: requireCondition } = field.conditional
