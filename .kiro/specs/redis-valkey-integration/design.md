...
export class RedisClientManager {
  ...
  private async createClient(name: string, config: RedisConnectionConfig): Promise<Redis | Cluster> {
...
export class CacheService {
  ...
  async exists(key: CacheKey, options?: CacheOptions): Promise<boolean> {
    ...
    try {
      ...
      this.observability.trackError('cache.exists.error', error as Error, { key: fullKey })
      return false
    }
  }
  ...
  async ttl(key: CacheKey, options?: CacheOptions): Promise<number> {
    ...
    try {
      ...
      this.observability.trackError('cache.ttl.error', error as Error, { key: fullKey })
      return -1
    }
  }

  async expire(key: CacheKey, ttl: number, options?: CacheOptions): Promise<boolean> {
    const client = this.redisManager.getClient(options?.clientName)
    const fullKey = this.buildKey(key)
    ...
    try {
      ...
      this.observability.trackError('cache.expire.error', error as Error, { key: fullKey })
      return false
    }
  }
...
export class SessionService {
  ...
  async getSession<T extends SessionData>(
    ...
    return this.observability.trackOperation('session.get', async () => {
      ...
      try {
        ...
        await client.setex(key, await client.ttl(key), JSON.stringify(sessionData))
  ...
  async updateSession<T extends SessionData>(
    ...
    return this.observability.trackOperation('session.update', async () => {
      ...
      try {
        ...
        if (!existingData) {
          this.observability.recordMetric('session.update.not_found', 1, { sessionId })
          return false
        }
  ...
  async getActiveSessions(pattern?: string, options?: SessionOptions): Promise<string[]> {
    const client = this.redisManager.getClient(options?.clientName)
    ...
    try {
      ...
      this.observability.trackError('session.get_active.error', error as Error, {
        pattern
      })
      return []
    }
