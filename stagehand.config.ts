...
export const TestUtils = {
  async waitForStable(stagehand: any, timeout = 5000) {
    ...
    while (Date.now() - start < timeout) {
      const isStable = await stagehand.observe({ description: 'page content is stable and fully loaded' });
      if (isStable) return;
  ...
  async extractWithSchema<T>(stagehand: any, description: string, schema: z.ZodType<T>): Promise<T> {
  ...
  async validateAccessibility(stagehand: any): Promise<z.infer<typeof AccessibilityDataSchema>> {
    const data = await stagehand.extract({
      description: 'accessibility information including alt text, headings, keyboard navigation, color contrast, and ARIA labels',
    });
    return AccessibilityDataSchema.parse(JSON.parse(data));
  },
  
  async captureMetrics(stagehand: any, testName: string, actions: any[]): Promise<z.infer<typeof TestMetricsSchema>> {
