...
describe('Database Schema and Migration Tests', () => {
  ...
  describe('Schema Validation', () => {
    ...
    it('should validate environment creation schema', () => {
      ...
      const result = DatabaseSchemas.CreateEnvironment.safeParse(validEnvironment)
      expect(result.success).toBe(true)
    })
    ...
    it('should validate agent execution schema', () => {
      ...
      const result = DatabaseSchemas.CreateAgentExecution.safeParse(validExecution)
      expect(result.success).toBe(true)
    })
  })
  ...
  describe('Database Operations', () => {
    ...
    it('should create and retrieve environments', async () => {
      ...
      const [created] = await db.insert(environments).values(newEnvironment).returning()
      expect(created).toBeDefined()
    ...
    it('should create agent executions with foreign key relationships', async () => {
      ...
      const [created] = await db.insert(agentExecutions).values(newExecution).returning()
      expect(created).toBeDefined()
    ...
    it('should create observability events', async () => {
      ...
      const [created] = await db.insert(observabilityEvents).values(newEvent).returning()
      expect(created).toBeDefined()
    ...
    it('should create execution snapshots', async () => {
      ...
      const [created] = await db.insert(executionSnapshots).values(newSnapshot).returning()
      expect(created).toBeDefined()
      expect(created.executionId).toBe(testExecutionId)
    ...
    it('should handle agent memory with vector embeddings', async () => {
      ...
      const [created] = await db.insert(agentMemory).values(newMemory).returning()
      expect(created).toBeDefined()
  ...
  describe('Database Indexes and Constraints', () => {
    it('should enforce unique constraints', async () => {
      ...
      await expect(db.insert(environments).values(environment2)).rejects.toThrow()
    })
