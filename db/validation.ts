...
export class DatabaseValidator {
	private readonly requiredExtensions = [
		"uuid-ossp",
		"vector",
		"pg_stat_statements",
	];
	...
	async validate(): Promise<DatabaseValidationResult> {
		...
		try {
			...
			if (extensionsStatus.missing.length > 0) {
				errors.push(
					`Missing required extensions: ${extensionsStatus.missing.join(", ")}`,
				);
			}
    ...
    `;
		...
		const constraintsCount = Number.parseInt(
			constraintsResult.rows[0]?.count || "0",
		);
	 ...
	 */
	private async validateVectorSearch(
		warnings: string[],
		recommendations: string[],
	): Promise<void> {
		try {
      ...
      `;
			...
			if (vectorColumns.rows.length === 0) {
				warnings.push(
					"No vector columns found - vector search capabilities may not be available",
				);
			} else {
	...
	private async validateMigrations(
		...
		try {
			...
			if (migrationStatus.pending.length > 0) {
				warnings.push(
					`${migrationStatus.pending.length} pending migrations found`,
				);
			}
		...
		try {
			...
			for (const extension of extensionsStatus.missing) {
				try {
					...
					errors.push(
						`Failed to install extension ${extension}: ${error.message}`,
					);
				}
			}
			...
			if (migrationStatus.pending.length > 0) {
				...
				if (migrationResult.success) {
					fixed.push(
						`Executed ${migrationResult.executed.length} pending migrations`,
					);
				} else {
