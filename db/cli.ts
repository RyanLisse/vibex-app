...
async function showMonitoringStats() {
	...
	console.log("\n🔗 Connection Pool:");
	console.log(
		`  Active Connections: ${poolStats.activeConnections}/${poolStats.maxConnections}`,
	);
	console.log(`  Utilization: ${poolStats.utilizationPercent}%`);
	console.log(
		`  Health Status: ${poolStats.isHealthy ? "✅ Healthy" : "❌ Unhealthy"}`,
	);
	console.log(
		`  Last Health Check: ${poolStats.lastHealthCheck?.toISOString() || "Never"}`,
	);
