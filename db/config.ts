...
const isDummyUrl =
	DATABASE_URL === "your_database_url_here" || DATABASE_URL === "";
const isBuildTime =
	process.env.NODE_ENV === "production" && (!DATABASE_URL || isDummyUrl);
...
export class DatabasePool {
	...
	async getConnection() {
		...
		if (this.connectionCount >= this.maxConnections) {
			throw new Error(
				`Maximum database connections reached (${this.maxConnections})`,
			);
		}
	...
	getPoolStats(): {
		...
		return {
			...
			utilizationPercent: Math.round(
				(this.connectionCount / this.maxConnections) * 100,
			),
