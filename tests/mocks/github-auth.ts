...
export const mockGitHubAPI: Record<
	string,
...
> & {
	...
	getBranch: vi
		.fn()
		.mockImplementation((owner: string, repo: string, branch: string) => {
			const repoKey = `${owner}/${repo}`;
	...
	simulateRateLimit: () => {
		...
		for (const method of Object.keys(mockGitHubAPI)) {
			if (
				typeof mockGitHubAPI[method as keyof typeof mockGitHubAPI] ===
					"function" &&
				method !== "simulateRateLimit"
			) {
				(
					mockGitHubAPI[method as keyof typeof mockGitHubAPI] as ReturnType<
						typeof vi.fn
					>
...
export const githubTestDataGenerators = {
	...
	createMockBranch: (
		overrides: Partial<MockGitHubBranch> = {},
	): MockGitHubBranch => ({
		name: "main",
		commit: {
			sha: "abc123def456",
...
export const githubStateUtils = {
	...
	simulateRateLimit: () => {
		...
		for (const method of Object.keys(mockGitHubAPI)) {
			...
			if (typeof mockFn === "function" && "mockRejectedValueOnce" in mockFn) {
				(mockFn as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
					rateLimitError,
				);
			}
		}
