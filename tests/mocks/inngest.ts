...
export const mockInngestClient = {
	...
	send: vi
		.fn()
		.mockImplementation((event: Partial<MockEvent> | Partial<MockEvent>[]) => {
	...
	createFunction: vi
		.fn()
		.mockImplementation(
			(config: MockFunctionConfig, handler: (...args: any[]) => any) => {
				const mockFunction = {
					id: config.id,
					name: config.name,
					config,
					...
					run: vi
						.fn()
						.mockImplementation(
							async (event: MockEvent, step: MockStepContext) => {
...
export const inngestTestDataGenerators = {
	...
	createFunctionConfig: (
		overrides: Partial<MockFunctionConfig> = {},
	): MockFunctionConfig => ({
		id: `fn-${Date.now()}`,
		name: "test-function",
		concurrency: 1,
...
export const setupInngestMocks = () => {
	vi.mock("inngest", () => ({
		...
		InngestFunction: vi
			.fn()
			.mockImplementation((config: MockFunctionConfig, handler: Function) => {
...
export const inngestTestHelpers = {
	...
	mockStepExecution: (stepId: string, result: any) => {
		const stepContext = createMockStepContext();
		stepContext.step.run.mockImplementation(
			async (id: string, handler: Function) => {
				if (id === stepId) {
					return result;
				}
				return handler();
