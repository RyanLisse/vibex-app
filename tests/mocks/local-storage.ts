			...
			(() => {
				...
				const db: MockIDBDatabase = {
					...
					createObjectStore: vi
						.fn()
						.mockImplementation(
							(storeName: string, opts?: IDBObjectStoreParameters) => {
								if (!dbMap) {
								...
								const store: MockIDBObjectStore = {
									...
									get: vi.fn((key: unknown) =>
										createRequest(storeMap?.get(key)),
									),
									getAll: vi.fn(() =>
										createRequest(storeMap ? [...storeMap.values()] : []),
									),
...
export const storageUtils = {
	...
	restoreNormalBehavior() {
		mockLocalStorage.setItem =
			MockStorage.prototype.setItem.bind(mockLocalStorage);
		mockSessionStorage.setItem =
			MockStorage.prototype.setItem.bind(mockSessionStorage);
	},
};
