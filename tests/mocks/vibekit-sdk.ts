...
export const mockVibeKitSDK = {
	...
	tasks: {
		...
		update: vi
			.fn()
			.mockImplementation((id: string, updates: Partial<MockTask>) => {
		...
		addMessage: vi
			.fn()
			.mockImplementation(
				(taskId: string, messageData: Partial<MockMessage>) => {
	...
	environments: {
		...
		get: vi.fn().mockImplementation((id: string) => {
			...
			return env
				? Promise.resolve(env)
				: Promise.reject(new Error("Environment not found"));
		}),
	...
	streaming: {
		subscribe: vi
			.fn()
			.mockImplementation((_taskId: string, callback: (data: any) => void) => {
...
export const testDataGenerators = {
	...
	createMockEnvironment: (
		overrides: Partial<MockEnvironment> = {},
	): MockEnvironment => ({
		id: `env-${Date.now()}`,
