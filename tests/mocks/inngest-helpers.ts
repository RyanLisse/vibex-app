...
export const createMockStep = () => ({
	run: vi
		.fn()
		.mockImplementation(async (name: string, callback: () => Promise<any>) => {
...
export const resetInngestMocks = async () => {
	...
	const { inngest, taskControl, createTask } =
		await vi.importMock("@/lib/inngest");
...
export const mockStreamingResponse = (callbacks?: {
