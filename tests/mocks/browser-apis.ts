...
export const mockMediaQueryAPI = {
	matchMedia: vi.fn().mockImplementation((query: string) => {
		...
		return {
			...
			addListener: vi
				.fn()
				.mockImplementation((listener: (mq: MediaQueryList) => void) => {
			...
			addEventListener: vi
				.fn()
				.mockImplementation(
					(type: string, listener: (mq: MediaQueryList) => void) => {
						if (type === "change") {
							mq.listeners.add(listener);
						}
					},
...
export const mockGeolocationAPI = {
	getCurrentPosition: vi
		.fn()
		.mockImplementation((success: Function, error?: Function) => {
			if (mockGeolocation) {
				success({
...
export const browserAPIUtils = {
	...
	notifications: {
		...
		findByTitle: (title: string) =>
			mockNotifications.find((n) => n.title === title),
...
export const browserAPITestHelpers = {
	...
	expectNotificationShown: (
		...
		expect(mockNotificationAPI.Notification).toHaveBeenCalledWith(
			title,
			options,
		);
	},
