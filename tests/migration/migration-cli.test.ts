...
describe("Migration CLI Tests", () => {
	...
	describe("Status Command", () => {
		...
		it("should display verbose status information", async () => {
			...
			const result = mockExecSync(
				"bun run scripts/migration-cli.ts status --verbose",
			);
			const output = result.toString();
		...
		it("should display user-specific status", async () => {
			mockCLIExecution(
				"bun run scripts/migration-cli.ts status --user-id user-123",
				{
					stdout:
						"Migration Status for user: user-123\nLocal Storage Data: filtered for user",
				},
			);
	...
	describe("Migrate Command", () => {
		...
		it("should perform dry run migration", async () => {
			...
			const result = mockExecSync(
				"bun run scripts/migration-cli.ts migrate --dry-run",
			);
			const output = result.toString();
		...
		it("should handle migration with custom batch size", async () => {
			mockCLIExecution(
				"bun run scripts/migration-cli.ts migrate --batch-size 25",
				{
		...
		it("should handle migration with continue on error", async () => {
			...
			mockCLIExecution(
				"bun run scripts/migration-cli.ts migrate --continue-on-error",
				{
					stdout:
						"Migration completed successfully!\nItems Failed: 3\nErrors:\n  1. Invalid task data\n  2. Invalid task data\n  3. Invalid task data",
				},
		...
		it("should handle migration with retry attempts", async () => {
			mockCLIExecution(
				"bun run scripts/migration-cli.ts migrate --retry-attempts 5",
				{
					stdout: "Migration completed successfully!",
				},
			);
		...
		it("should handle migration without backup", async () => {
			...
			const result = mockExecSync(
				"bun run scripts/migration-cli.ts migrate --no-backup",
			);
	...
	describe("Backup Commands", () => {
		it("should list backups", async () => {
			...
			const result = mockExecSync(
				"bun run scripts/migration-cli.ts backup list",
			);
			const output = result.toString();
		...
		it("should create new backup", async () => {
			...
			const result = mockExecSync(
				"bun run scripts/migration-cli.ts backup create",
			);
			const output = result.toString();
		...
		it("should restore backup", async () => {
			mockCLIExecution(
				"bun run scripts/migration-cli.ts backup restore backup-123",
				{
					stdout:
						"Restoring backup: backup-123\nBackup restored successfully!\nRestored Items: 15",
				},
			);
		...
		it("should handle backup restoration failure", async () => {
			...
			mockCLIExecution(
				"bun run scripts/migration-cli.ts backup restore invalid-backup",
				{
					exitCode: 1,
	...
	describe("Extract Command", () => {
		...
		it("should extract with verbose output", async () => {
			...
			const result = mockExecSync(
				"bun run scripts/migration-cli.ts extract --verbose",
			);
			const output = result.toString();
	...
	describe("Clear Command", () => {
		...
		it("should clear localStorage with confirmation", async () => {
			...
			const result = mockExecSync(
				"bun run scripts/migration-cli.ts clear --confirm",
			);
			const output = result.toString();
	...
	describe("Error Handling and Edge Cases", () => {
		...
		it("should handle invalid options", async () => {
			mockCLIExecution(
				"bun run scripts/migration-cli.ts migrate --invalid-option",
				{
					exitCode: 1,
