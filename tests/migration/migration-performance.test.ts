...
const generateTaskData = (count: number): LocalStorageTask[] =>
  Array.from({ length: count }, (_, i) => ({
    ...
    metadata: {
      ...
      dependencies: i % 4 === 0 ? [`task-${Math.max(0, i - 1)}`, `task-${Math.max(0, i - 2)}`] : [],
  ...
  Array.from({ length: count }, (_, i) => ({
    ...
    config: {
      ...
      envVars: Object.fromEntries(
        Array.from({ length: 10 }, (_, j) => [`ENV_VAR_${j}`, `value-${i}-${j}`])
      ),
      secrets: Object.fromEntries(
        Array.from({ length: 3 }, (_, j) => [`SECRET_${j}`, `secret-${i}-${j}`])
      ),
    },
  }))
...
const runPerformanceTest = async <T>(
  ...
  expect(avgMemoryDelta.heapUsedDelta).toBeLessThan(PERFORMANCE_THRESHOLDS.MEMORY_USAGE_LIMIT)
  ...
  describe('Backup System Performance', () => {
    ...
    it('should create compressed backup efficiently', async () => {
      ...
      if (uncompressedResult.manifest && compressedResult.manifest) {
        ...
        expect(compressionRatio).toBeLessThan(PERFORMANCE_THRESHOLDS.BACKUP_COMPRESSION_RATIO)
      }
    })
  ...
  describe('Memory Usage Optimization', () => {
    ...
    it('should process data in chunks to manage memory', async () => {
      ...
      for (const chunk of chunks) {
        ...
        expect(transformResult.transformed.length).toBeLessThanOrEqual(chunkSize)
      }

      const memoryAfterChunking = getMemoryUsage()
      const memoryDelta = memoryAfterChunking.heapUsed - memoryBeforeChunking.heapUsed
            ...
            level1: Array.from({ length: 20 }, (_, i) => ({
              ...
              level2: Array.from({ length: 10 }, (_, j) => ({
                ...
                level3: Object.fromEntries(
                  Array.from({ length: 5 }, (_, k) => [`key-${k}`, `value-${i}-${j}-${k}`])
                ),
              })),
            })),
          },
