...
export class ResponseAssertions {
	 ...
	 */
	static async assertPartialJsonResponse(
		response: Response,
		expectedPartial: any,
	) {
		expect(response.headers.get("content-type")).toContain("application/json");
	 ...
	 */
	static async assertResponseContains(
		response: Response,
...
export class ApiRouteTestRunner {
	...
	async testGet(
		...
		const request = MockRequestFactory.createGetRequest(url, {
			...this.defaultHeaders,
			...headers,
		});
