...
import { StagehandConfig, PageDataSchema, TaskDataSchema, EnvironmentDataSchema } from '../../../stagehand.config';
  ...
  utils: async ({ stagehand }, use) => {
    const utils = {
      ...
      async validateAccessibility(stagehand: Stagehand) {
        return await stagehand.page.extract({
          instruction: 'Analyze accessibility features including alt text, headings, keyboard navigation, contrast, and ARIA labels',
          schema: {
            hasAltText: 'boolean',
