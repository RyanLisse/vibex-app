...
test.describe("Task Page Structure E2E Tests", () => {
	...
	test("should show task not found message for non-existent task", async ({
		page,
	}) => {
		...
		await expect(
			page.locator("text=The requested task could not be found."),
		).toBeVisible();
	});
	...
	test("should handle task page components without errors", async ({
		page,
	}) => {
