...
test.describe("Basic Mock Application Test", () => {
	test("should be able to run a simple test with mock server", async ({
		mockPage,
	}) => {
		...
		await expect(
			mockPage.locator("div.flex.flex-col.px-4.py-2.h-screen"),
		).toBeVisible();
	});

	test("should have proper application structure with mock", async ({
		mockPage,
	}) => {
		await mockPage.goto("http://localhost:3000");
		await mockPage.waitForLoadState("networkidle");
		...
		await expect(
			mockPage.locator("h1").filter({ hasText: "VibeX" }),
		).toBeVisible(); // Navbar title

		// Check for navigation links
		await expect(
			mockPage.locator("a").filter({ hasText: "Home" }),
		).toBeVisible();
		await expect(
			mockPage.locator("a").filter({ hasText: "Environments" }),
		).toBeVisible();
		...
		const errorElements = mockPage.locator(
			'[data-error="true"], .error-message, .fatal-error',
		);
		await expect(errorElements).toHaveCount(0);
	});

	test("should handle API requests with mock responses", async ({
		mockPage,
	}) => {
