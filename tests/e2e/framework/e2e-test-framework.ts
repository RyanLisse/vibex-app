...
export class E2ETestRunner {
   ...
   */
  async runScenario(scenario: TestScenario, testFn: (context: E2ETestContext) => Promise<void>) {
...
export class UserInteractionHelper {
   ...
   */
  async typeNaturally(selector: string, text: string, options?: { delay?: number }) {
...
export class E2EAssertions {
  ...
  async assertInteractive(description: string) {
    const isVisible = await this.helper.verifyVisualState(`${description} is visible`)
  ...
  async assertComponentState(description: string, expectedState: string) {
    const hasState = await this.helper.verifyVisualState(`${description} ${expectedState}`)
  ...
  async assertNotLoading() {
    const isLoading = await this.helper.verifyVisualState('loading indicator is visible')
