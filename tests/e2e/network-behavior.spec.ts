...
test.describe('Network Behavior E2E Tests', () => {
  ...
  test('should handle network failures gracefully', async ({ page }) => {
    ...
    const fatalErrorElements = page.locator('[data-fatal="true"], .fatal-error')
  ...
  test('should handle WebSocket connection failures', async ({ page }) => {
    ...
    const criticalErrors = wsErrors.filter(
      (error) =>
        !(error.includes('connection') || error.includes('network') || error.includes('failed'))
    )
  ...
  test('should handle streaming connection errors', async ({ page }) => {
    ...
    const appCrashErrors = streamErrors.filter(
      (error) => error.includes('fatal') || error.includes('crashed') || error.includes('uncaught')
    )
  ...
  test('should handle authentication errors gracefully', async ({ page }) => {
    ...
    const authErrorElements = page.locator('[data-auth-error="true"], .auth-error')
