...
describe("Workflow Integration Template", () => {
	...
	describe("User Authentication Workflow", () => {
		it("should complete OAuth flow end-to-end", async () => {
			...
			const callbackResponse = await fetch(
				"/api/auth/github/callback?code=auth_code&state=abc123",
			);
		...
		it("should handle OAuth errors gracefully", async () => {
			...
			const callbackResponse = await fetch(
				"/api/auth/github/callback?code=invalid&state=abc123",
			);
			const callbackData = await callbackResponse.json();
	...
	describe("Complex Multi-Service Workflow", () => {
		...
		it("should handle partial service failures", async () => {
			...
			expect(taskResult.warnings).toContain(
				"Repository list unavailable, using cached data",
			);
		});
	});
