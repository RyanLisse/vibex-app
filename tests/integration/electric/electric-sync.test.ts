...
describe("ElectricSQL Synchronization Tests", () => {
	...
	beforeAll(async () => {
		...
		vi.mocked(mockElectricClient.on).mockImplementation(
		...
		vi.mocked(mockElectricClient.emit).mockImplementation(
	...
	beforeEach(async () => {
		...
		vi.mocked(mockElectricClient.query).mockImplementation(
			async (table: string, filters?: any) => {
	...
	describe("Connection Management", () => {
		...
		it("should handle connection failures gracefully", async () => {
			...
			const connectionError = new Error(
				"Failed to connect to Electric sync service",
			);
	...
	describe("Real-time Synchronization", () => {
		...
		it("should sync new tasks from server to client", async () => {
			...
			expect(localData.tasks.get(serverTask.id!).title).toBe(
				"Server Created Task",
			);
		});
		...
		it("should sync local changes to server", async () => {
			...
			expect(serverData.tasks.get(localTask.id!).title).toBe(
				"Locally Created Task",
			);
		});
		...
		it("should handle real-time updates via subscriptions", async () => {
			...
			// Mock subscription
			vi.mocked(mockElectricClient.subscribe).mockImplementation(
				(filter, callback) => {
					callback({
						type: "update",
						table: "tasks",
			...
			await mockElectricClient.subscribe(
				'tasks.user_id = "test-user-123"',
				updateListener,
			);
	...
	describe("Conflict Resolution", () => {
		...
		it("should resolve conflicts using last-write-wins strategy", async () => {
			...
			vi.mocked(mockElectricClient.conflict.resolve).mockImplementation(
				async (local, server) => {
			...
			const resolved = await mockElectricClient.conflict.resolve(
				localTask,
				serverTask,
			);
	...
	describe("Offline/Online Transitions", () => {
		...
		it("should queue operations while offline", async () => {
			...
			vi.mocked(mockElectricClient.offline.setEnabled).mockImplementation(
				(enabled) => {
					if (enabled) {
						// Mock queuing operations when offline
						vi.mocked(mockElectricClient.insert).mockImplementation(
							async (table, data) => {
			...
			vi.mocked(mockElectricClient.sync).mockImplementation(async () => {
				offlineQueue.forEach((op) => {
					if (op.operation === "insert") {
						serverData[op.table as keyof typeof serverData].set(
							op.data.id,
							op.data,
						);
					} else if (op.operation === "update") {
						const existing = serverData[
							op.table as keyof typeof serverData
						].get(op.data.id);
						if (existing) {
		...
		it("should handle offline storage limits", async () => {
			...
			for (let i = 0; i < maxOfflineSize; i++) {
				await mockElectricClient.insert(
					"tasks",
					createTestTask({ id: `offline-${i}` }),
				);
			}
	...
	describe("Subscription Management", () => {
		...
		it("should handle subscription errors gracefully", async () => {
			...
			vi.mocked(mockElectricClient.subscribe).mockRejectedValue(
				subscriptionError,
			);
	...
	describe("Data Validation and Consistency", () => {
		...
		it("should validate data before sync", async () => {
			...
			await expect(
				mockElectricClient.insert("tasks", invalidTask),
		...
		it("should maintain referential integrity during sync", async () => {
			...
			await expect(
				mockElectricClient.insert("agentExecutions", orphanExecution),
			).rejects.toThrow("Referenced task does not exist");
			...
			vi.mocked(mockElectricClient.sync).mockImplementation(async () => {
				...
				for (let i = 0; i < datasetSize; i += batchSize) {
					const batch = Array.from(
						...
						(_, j) => createTestTask({ id: `bulk-task-${i + j}` }),
					);
