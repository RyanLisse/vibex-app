...
const mockTaskFunction = {
  ...
  handler: async ({ event, step }) => {
    ...
    const result = await step.run('process', async () => {
      return { taskId, status: 'completed', result: 'Task processed successfully' }
    })
...
const mockAgentFunction = {
  ...
  handler: async ({ event, step }) => {
    ...
    const execution = await step.run('execute', async () => {
      return { agentId, task, status: 'executed', result: 'Agent task completed' }
    })
...
describe('Inngest Workflow Testing', () => {
  let testEnvironment: ReturnType<typeof InngestTestHelpers.createTestEnvironment>
  ...
  describe('Event Factory', () => {
    it('should create task events with correct structure', () => {
      const event = InngestTestEventFactory.createTaskEvent('task-123', { priority: 'high' })
    ...
    it('should create workflow events', () => {
      const event = InngestTestEventFactory.createWorkflowEvent('workflow-456', 'start', {
        config: 'test',
      })
    ...
    it('should create agent events', () => {
      const event = InngestTestEventFactory.createAgentEvent('agent-789', 'activate', {
        model: 'gpt-4',
      })
    ...
    it('should create AI completion events', () => {
      const event = InngestTestEventFactory.createAICompletionEvent('session-123', { tokens: 100 })
    ...
    it('should create error events', () => {
      const event = InngestTestEventFactory.createErrorEvent('Database connection failed', {
        database: 'postgres',
      })
  ...
  describe('Function Testing', () => {
    it('should execute function successfully', async () => {
      ...
      const event = InngestTestEventFactory.createTaskEvent('task-123', { action: 'process' })
    ...
    it('should handle function failures', async () => {
      const tester = new InngestFunctionTester(mockTaskFunction)
      const event = InngestTestEventFactory.createTaskEvent('task-456', { action: 'fail' })
    ...
    it('should test retry logic', async () => {
      const tester = new InngestFunctionTester(mockTaskFunction)
      const event = InngestTestEventFactory.createTaskEvent('task-retry', { action: 'fail' })
    ...
    it('should test step-by-step execution', async () => {
      ...
      const event = InngestTestEventFactory.createWorkflowEvent('workflow-123', 'execute', {
        steps: [
    ...
    it('should validate function timing', async () => {
      ...
      const event = InngestTestEventFactory.createAgentEvent('agent-123', 'coordinate', {
        task: 'complex-task',
      })

      const { execution, elapsed } = await tester.validateTiming(event, 50, 500)
  ...
  describe('Workflow Testing', () => {
    it('should execute sequential workflow', async () => {
      ...
      const events = [
        {
          functionName: 'task',
          event: InngestTestEventFactory.createTaskEvent('task-1', { action: 'process' }),
        },
        {
          functionName: 'agent',
          event: InngestTestEventFactory.createAgentEvent('agent-1', 'coordinate', {
            task: 'simple-task',
          }),
        },
      ]
    ...
    it('should handle workflow failures', async () => {
      ...
      const events = [
        {
          functionName: 'task',
          event: InngestTestEventFactory.createTaskEvent('task-fail', { action: 'fail' }),
        },
        {
          functionName: 'workflow',
          event: InngestTestEventFactory.createWorkflowEvent('workflow-after-fail', 'execute', {
            steps: [],
          }),
        },
      ]
    ...
    it('should execute parallel functions', async () => {
      ...
      const executions = [
        ...
        {
          functionName: 'task2',
          event: InngestTestEventFactory.createTaskEvent('task-2', { action: 'process' }),
        },
      ]
  ...
  describe('Mock Client Testing', () => {
    ...
    it('should assert events correctly', async () => {
      ...
      await mockClient.send(
        InngestTestEventFactory.createTaskEvent('task-123', { priority: 'high' })
      )
      await mockClient.send(InngestTestEventFactory.createAgentEvent('agent-456', 'activate'))

      // Should not throw
      mockClient.assertEventSent('task/created')
      mockClient.assertEventSent('agent/activate')
      mockClient.assertEventSent('task/created', { taskId: 'task-123', priority: 'high' })
    ...
    it('should assert event counts', async () => {
      ...
      await mockClient.send(InngestTestEventFactory.createAgentEvent('agent-1', 'activate'))
  ...
  describe('Test Helpers', () => {
    it('should create test scenarios', async () => {
      const tester = new InngestFunctionTester(mockTaskFunction)
      const scenario = InngestTestHelpers.createTestScenario('task-processing', [
        InngestTestEventFactory.createTaskEvent('task-1', { action: 'process' }),
        InngestTestEventFactory.createTaskEvent('task-2', { action: 'process' }),
      ])
    ...
    it('should create delayed events', async () => {
      ...
      const delayedEvent = await InngestTestHelpers.createDelayedEvent(baseEvent, 100)
      const elapsed = Date.now() - start
