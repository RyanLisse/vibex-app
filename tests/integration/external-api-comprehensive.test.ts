...
describe("External API Comprehensive Integration", () => {
	...
	describe("1. OpenAI API Integration", () => {
		...
		test("should handle audio transcription", async () => {
			const mockTranscribe = vi
				.fn()
				.mockResolvedValue(mockResponses.openai.transcription);
	...
	describe("2. Google AI (Gemini) Integration", () => {
		...
		test("should handle multimodal inputs", async () => {
			...
			const model = mockGemini.getGenerativeModel({
			...
			observability.api.recordExternalCall(
				"google",
				"gemini-vision",
				latency,
				true,
	...
	describe("3. Letta API Integration", () => {
		test("should manage agent lifecycle", async () => {
			...
			const updated = await mockLetta.updateAgent(agent.id, {
				name: "Updated Agent",
			});
			healthMonitor.recordRequest(
				"letta_update_agent",
		...
		test("should handle memory operations", async () => {
			...
			// Update memory
			const updateStart = Date.now();
			const updated = await mockLetta.updateMemory(
				"agent-123",
				"New memory entry",
			);
			healthMonitor.recordRequest(
			...
			const searchResults = await mockLetta.searchMemory(
				"agent-123",
				"relevant query",
			);
			healthMonitor.recordRequest(
			...
			const mockGitHub = {
				...
				rest: {
					users: {
						getAuthenticated: vi
							.fn()
							.mockResolvedValue({ data: mockResponses.github.user }),
					},
				},
			};
		...
		test("should track API latency and throughput", async () => {
			...
			const recordThroughput = (api: string) => {
				if (!performanceMetrics.throughput.has(api)) {
					performanceMetrics.throughput.set(api, {
						count: 0,
						startTime: Date.now(),
					});
				}
			...
			for (let i = 0; i < 100; i++) {
				...
				const latency =
					api.baseLatency + (Math.random() - 0.5) * api.variance * 2;
