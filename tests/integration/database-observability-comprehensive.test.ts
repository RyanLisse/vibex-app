...
describe('Database Observability Comprehensive Integration', () => {
  ...
  describe('1. Query Performance Tracking', () => {
    test('should track query execution times by type', async () => {
      ...
      const queries = [
        ...
        { sql: 'SELECT u.*, p.* FROM users u JOIN profiles p ON u.id = p.user_id', type: 'join' },
        { sql: 'INSERT INTO users (name, email) VALUES (?, ?)', type: 'insert' },
        { sql: 'UPDATE users SET last_login = NOW() WHERE id = ?', type: 'update' },
    ...
    test('should detect and track slow queries', async () => {
      ...
      const queries = [
        ...
        { sql: 'SELECT * FROM slow_table WITH COMPLEX CONDITIONS', expectSlow: true },
        { sql: 'SELECT * FROM users', expectSlow: false },
        { sql: 'SELECT * FROM slow_table JOIN other_slow_table', expectSlow: true },
      ]
  ...
  describe('4. Error Rate Monitoring', () => {
    test('should track database errors by type', async () => {
      const errorQueries = [
        { sql: 'SELECT * FROM non_existent_table', errorType: 'table_not_found' },
        { sql: 'INSERT INTO users (id) VALUES (1), (1)', errorType: 'duplicate_key' },
