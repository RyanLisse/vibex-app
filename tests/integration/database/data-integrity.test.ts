	...
	async function setupIntegrityTestData() {
		...
		const taskData = Array.from({ length: 5 }, (_, i) => ({
			...
			status: (i % 3 === 0
				...
				: i % 3 === 1
					? "in-progress"
					: "completed") as "pending" | "in-progress" | "completed",
		...
		const executionData = createdTasks.slice(0, 3).map((task, i) => ({
			...
			status: (i % 2 === 0 ? "completed" : "running") as
				| "completed"
				| "running",
		...
		it("should maintain referential integrity during bulk operations", async () => {
			...
			const taskUsers = await db
				.select({
					...
					userId: tasks.userId,
					userExists: exists(
						db.select().from(users).where(eq(users.id, tasks.userId)),
					),
				})
				.from(tasks)
				.where(
	...
	describe("Data Consistency Validation", () => {
		...
		it("should validate enum value consistency", async () => {
			...
			const invalidStatuses = await db
				...
				.from(tasks)
				.where(
					sql`${tasks.status} NOT IN ('pending', 'in-progress', 'completed', 'cancelled')`,
				);
			...
			const invalidPriorities = await db
				...
				.from(tasks)
				.where(
					sql`${tasks.priority} NOT IN ('low', 'medium', 'high', 'urgent')`,
				);
