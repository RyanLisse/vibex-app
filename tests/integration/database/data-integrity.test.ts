...
describe.skipIf(skipTests)('Data Integrity Validation Tests', () => {
  ...
  async function setupIntegrityTestData() {
    ...
    const taskData = Array.from({ length: 5 }, (_, i) => ({
      ...
      status: (i % 3 === 0 ? 'pending' : i % 3 === 1 ? 'in-progress' : 'completed') as
        | 'pending'
        | 'in-progress'
        | 'completed',
    ...
    const eventData = createdExecutions.map((execution, i) => ({
      ...
      severity: (i % 3 === 0 ? 'info' : i % 3 === 1 ? 'warning' : 'error') as
        | 'info'
        | 'warning'
        | 'error',
  ...
  describe('Constraint Enforcement Tests', () => {
    it('should enforce unique constraints on user email', async () => {
      ...
      await expect(db.insert(users).values(duplicateUser)).rejects.toThrow()
    })
    ...
    it('should handle concurrent constraint violations gracefully', async () => {
      ...
      const failures = results.filter(
        (r) => r.status === 'rejected' || (r.status === 'fulfilled' && 'error' in r.value)
      )
  ...
  describe('Data Consistency Validation', () => {
    ...
    it('should validate enum value consistency', async () => {
      ...
      const invalidPriorities = await db
        .select()
        .from(tasks)
    ...
    it('should validate timestamp consistency', async () => {
      ...
      const inconsistentTasks = await db
        .select()
        .from(tasks)
      ...
      const inconsistentEnvs = await db
        .select()
        .from(environments)
    ...
    it('should validate business rule consistency', async () => {
      ...
      const incompleteExecutions = await db
        .select()
        .from(agentExecutions)
        .where(
          and(
      ...
      const prematureExecutions = await db
        .select()
        .from(agentExecutions)
        .where(
          and(eq(agentExecutions.status, 'running'), isNotNull(agentExecutions.executionTimeMs))
        )
    ...
    it('should validate unique constraints across related data', async () => {
      ...
      const duplicateActiveEnvs = await db
        .select({
  ...
  async function validateReferentialIntegrity(): Promise<IntegrityViolation[]> {
    ...
    const orphanedTasks = await db
      .select()
      .from(tasks)
      .where(sql`NOT EXISTS (SELECT 1 FROM ${users} WHERE ${users.id} = ${tasks.userId})`)
  ...
  async function validateConsistency(): Promise<IntegrityViolation[]> {
    ...
    const timestampInconsistencies = await db
      .select()
      .from(tasks)
      .where(sql`${tasks.updatedAt} < ${tasks.createdAt}`)
