...
class IntegrationMonitor {
  ...
  startOperation(name: string): () => void {
    ...
    return () => {
      ...
      const metric = this.metrics.get(name) || { count: 0, totalTime: 0, errors: 0 }
      metric.count++
...
describe('Redis/Valkey Comprehensive Integration', () => {
  ...
  beforeAll(async () => {
    ...
    console.log(`Running tests with ${isRedisAvailable ? 'Real Redis' : 'Mock Redis'}`)
  })
  ...
  describe('2. PubSub Service Integration', () => {
    test('should handle pub/sub messaging', async () => {
      ...
      // Publish messages
      await pubsub.publish('test:channel', { type: 'user.created', userId: 123 })
      ...
      expect(receivedMessages[1]).toEqual({ type: 'user.updated', userId: 123 })
  ...
  describe('5. Job Queue Service Integration', () => {
    ...
    test('should handle job priorities', async () => {
      ...
      await jobQueue.addJob('test:priority', { data: { priority: 1 }, priority: 1 })
      await jobQueue.addJob('test:priority', { data: { priority: 5 }, priority: 5 })
      await jobQueue.addJob('test:priority', { data: { priority: 3 }, priority: 3 })
      await jobQueue.addJob('test:priority', { data: { priority: 10 }, priority: 10 })
  ...
  describe('8. Cross-Service Integration', () => {
    test('should handle cache-aside pattern with job processing', async () => {
      ...
      const jobId1 = await jobQueue.addJob('test:cache-aside', { data: { id: 'abc123' } })
      const jobId2 = await jobQueue.addJob('test:cache-aside', { data: { id: 'abc123' } })
    ...
    test('should handle rate-limited job processing', async () => {
      ...
      const jobIds = []
      for (let i = 0; i < 5; i++) {
        const id = await jobQueue.addJob('test:ratelimited', { data: { index: i } })
        jobIds.push(id)
      }
