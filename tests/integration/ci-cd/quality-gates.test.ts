...
class QualityGateValidator {
	async runQualityGates(): Promise<QualityGateReport> {
		...
		const warningsCount = stages.reduce(
			(sum, s) => sum + (s.warnings?.length || 0),
			0,
		);

		const overall =
			failedStages > 0 ? "FAILED" : warningsCount > 0 ? "WARNING" : "PASSED";
	...
	async validateTestCoverage(): Promise<PipelineResult> {
		...
		try {
			...
			if (coverage.uncoveredFiles && coverage.uncoveredFiles.length > 0) {
				warnings.push(
					`${coverage.uncoveredFiles.length} files have no test coverage`,
				);
			}
	...
	async validatePerformance(): Promise<PipelineResult> {
		...
		try {
			...
			try {
				...
				await execAsync(
					"bun run test:integration -- tests/integration/performance/",
				);
	...
	async validateSecurity(): Promise<PipelineResult> {
		...
		try {
			...
			try {
				...
				if (
					mediumVulns > QUALITY_THRESHOLDS.security.maxMediumVulnerabilities
				) {
					warnings.push(
			...
			if (secretsFound.length > 0) {
				errors.push(
					`Potential secrets found in code: ${secretsFound.join(", ")}`,
				);
			}
			...
			if (outdatedDeps.length > 0) {
				warnings.push(
					`Outdated dependencies detected: ${outdatedDeps.length} packages`,
				);
			}
		...
		try {
			...
			try {
				...
				healthCheckResults.forEach((result) => {
					if (
						result.responseTime > QUALITY_THRESHOLDS.performance.maxResponseTime
					) {
						warnings.push(
...
describe("CI/CD Quality Gates Integration Tests", () => {
	...
	describe("Quality Gate Validation", () => {
		...
		it("should enforce test coverage thresholds", async () => {
			...
			expect(coverage.lines).toBeGreaterThanOrEqual(
				QUALITY_THRESHOLDS.coverage.lines,
			);
			expect(coverage.functions).toBeGreaterThanOrEqual(
				QUALITY_THRESHOLDS.coverage.functions,
			);
			expect(coverage.branches).toBeGreaterThanOrEqual(
				QUALITY_THRESHOLDS.coverage.branches,
			);
			expect(coverage.statements).toBeGreaterThanOrEqual(
				QUALITY_THRESHOLDS.coverage.statements,
			);
		});

		it("should enforce performance benchmarks", async () => {
			const performanceResult =
				await qualityGateValidator.validatePerformance();

			expect(performanceResult.passed).toBe(true);

			const { buildTime, bundleSize } = performanceResult.details;
			expect(buildTime).toBeLessThanOrEqual(
				QUALITY_THRESHOLDS.performance.maxBuildTime,
			);

			if (bundleSize > QUALITY_THRESHOLDS.performance.maxBundleSize) {
				console.warn(
					`Bundle size (${bundleSize}KB) exceeds recommended threshold`,
				);
			}
		});
		...
		it("should validate build artifacts", async () => {
			const artifactsResult =
				await qualityGateValidator.validateBuildArtifacts();
		...
		it("should validate deployment readiness", async () => {
			const deploymentResult =
				await qualityGateValidator.validateDeploymentReadiness();
	...
	describe("Quality Gate Thresholds", () => {
		...
		it("should fail when security vulnerabilities are found", async () => {
			...
			mockValidator.scanForSecrets = async () => [
				"api-key-in-config.js",
				"password-in-utils.ts",
			];

			const securityResult = await mockValidator.validateSecurity();
			expect(securityResult.passed).toBe(false);
			expect(
				securityResult.errors?.some((error) => error.includes("secrets")),
			).toBe(true);
		});
	});
	...
	describe("CI/CD Pipeline Integration", () => {
		...
		it("should support parallel execution of quality gates", async () => {
			...
			const sequentialTime = parallelGates.reduce(
				(sum, gate) => sum + gate.duration,
				0,
			);
	...
	describe("Quality Metrics Tracking", () => {
		it("should track quality metrics over time", async () => {
			...
			expect(metrics.lintErrors).toBeLessThanOrEqual(
				QUALITY_THRESHOLDS.codeQuality.maxLintErrors,
			);
			expect(metrics.typeErrors).toBeLessThanOrEqual(
				QUALITY_THRESHOLDS.codeQuality.maxTypeErrors,
			);
		});
