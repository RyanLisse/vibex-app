...
export function useEnhancedInfiniteQuery<TData = unknown, TError = Error>(
	...
	const enhancedQueryFn = useCallback(
		async ({ pageParam }: { pageParam: unknown }): Promise<TData> => {
			try {
				if (shouldUseWASM) {
					console.log(
						"Using WASM-optimized infinite query for:",
						queryKey,
						"page:",
						pageParam,
					);
				...
				if (wasmFallback) {
					console.log(
						"Using fallback infinite query for:",
						queryKey,
						"page:",
						pageParam,
					);
				...
				if (shouldUseWASM && wasmFallback) {
					console.warn(
						"WASM infinite query failed, falling back to JS:",
						error,
					);
					return await wasmFallback({ pageParam });
				}
	...
	return {
		...
		isVirtualizationEnabled:
			enableVirtualization && config.infinite.enableVirtualization,
	};
}
	...
	const prefetchQuery = useCallback(
		async <TData>(
			...
			await queryClient.prefetchQuery({
				queryKey,
				queryFn:
					enableWASMOptimization && shouldUseWASMOptimization("sqlite")
						? queryFn
						: queryFn,
				staleTime,
			});
