...
export function useEnvironmentQuery(environmentId: string) {
  return useEnhancedQuery(
    ...
    async (): Promise<EnvironmentWithStats> => {
      const response = await fetch(`/api/environments/${environmentId}?include=stats`)
      if (!response.ok) {
...
export function useEnvironmentsQuery(filters: EnvironmentFilters = {}) {
  ...
  return useEnhancedQuery(
    ...
    async (): Promise<EnvironmentWithStats[]> => {
      ...
      if (schemaVersion) searchParams.append('schemaVersion', schemaVersion.toString())
      if (tags?.length) searchParams.append('tags', tags.join(','))

      const response = await fetch(`/api/environments?${searchParams.toString()}`)
      if (!response.ok) {
    ...
    },
    {
      ...
      wasmFallback: async () => {
        ...
        if (search) {
          ...
          filteredEnvironments = filteredEnvironments.filter((env: Environment) =>
...
export function useActiveEnvironmentQuery(userId?: string) {
  return useEnhancedQuery(
    ...
    async (): Promise<EnvironmentWithStats | null> => {
      ...
      const response = await fetch(`/api/environments/active?${searchParams.toString()}`)
      if (!response.ok) {
        if (response.status === 404) {
          return null // No active environment
        }
        throw new Error(`Failed to fetch active environment: ${response.statusText}`)
      }
...
export function useCreateEnvironmentMutation() {
  ...
  return useEnhancedMutation(
    async (newEnvironment: Omit<NewEnvironment, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  return useEnhancedMutation(
    ...
    },
    {
      optimisticUpdate: ({ environmentId, updates }) => {
        ...
        queryClient.setQueryData(
          queryKeys.environments.detail(environmentId),
          (old: EnvironmentWithStats) => (old ? { ...old, ...updates, updatedAt: new Date() } : old)
        )
        ...
        queryClient.setQueryData(
          ...
          (old: EnvironmentWithStats[] = []) =>
            old.map((env) =>
              env.id === environmentId ? { ...env, ...updates, updatedAt: new Date() } : env
            )
        )

        // Update active environment cache if this environment is being activated
        if (updates.isActive) {
          queryClient.setQueryData(queryKeys.environments.active(), (old: EnvironmentWithStats) =>
            old?.id === environmentId ? { ...old, ...updates, updatedAt: new Date() } : old
          )
        }
      ...
      rollbackUpdate: (context) => {
        if (context?.previousEnvironment && context?.environmentId) {
          ...
          queryClient.setQueryData(
            ...
            (old: EnvironmentWithStats[] = []) =>
              old.map((env) =>
                env.id === context.environmentId ? context.previousEnvironment : env
              )
          )
        }
      },
  ...
  return useEnhancedMutation(
    ...
    },
    {
      optimisticUpdate: (environmentId) => {
        ...
        const newActiveEnvironment = previousEnvironments?.find((env) => env.id === environmentId)
      ...
      rollbackUpdate: (context) => {
        if (context?.previousEnvironments) {
          ...
          queryClient.setQueryData(queryKeys.environments.lists(), context.previousEnvironments)

          // Restore active environment
          const previousActive = context.previousEnvironments.find((env) => env.isActive)
          queryClient.setQueryData(queryKeys.environments.active(), previousActive || null)
        }
      },
      invalidateQueries: [queryKeys.environments.all],
      enableRealTimeSync: true,
  ...
  return useEnhancedMutation(
    ...
    },
    {
      optimisticUpdate: (environmentId) => {
        ...
        queryClient.setQueryData(
          queryKeys.environments.lists(),
          (old: EnvironmentWithStats[] = []) => old.filter((env) => env.id !== environmentId)
        )
      ...
      rollbackUpdate: (context) => {
        if (context?.previousEnvironment && context?.environmentId) {
          ...
          queryClient.setQueryData(
            queryKeys.environments.lists(),
            (old: EnvironmentWithStats[] = []) => [context.previousEnvironment, ...old]
          )

          // Restore active environment if it was the deleted one
          if (context.previousEnvironment.isActive) {
            queryClient.setQueryData(queryKeys.environments.active(), context.previousEnvironment)
          }
        }
      },
      invalidateQueries: [queryKeys.environments.all],
...
export function useEnvironmentTemplatesQuery() {
  return useEnhancedQuery(
    ...
    async () => {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch environment templates: ${response.statusText}`)
      }
