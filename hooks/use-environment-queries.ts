...
export const environmentQueryKeys = {
  ...
  list: (filters: EnvironmentFilters) => [...environmentQueryKeys.lists(), filters] as const,
  ...
  return useEnhancedMutation(
    ...
    },
    {
      optimisticUpdate: (variables) => {
        ...
        if (queryClient) {
          queryClient.setQueryData(environmentQueryKeys.lists(), (old: Environment[] = []) => [
  ...
  return useEnhancedMutation(
    ...
    },
    {
      optimisticUpdate: (variables) => {
        ...
        if (queryClient) {
          queryClient.setQueryData(environmentQueryKeys.lists(), (old: Environment[] = []) =>
            old.map((env) => (env.id === id ? { ...env, ...updates, updatedAt: new Date() } : env))
          )
...
export function useActivateEnvironmentMutation() {
  return useEnhancedMutation(
    async ({ environmentId, userId }: { environmentId: string; userId: string }) => {
      ...
      optimisticUpdate: (environmentId) => {
        ...
        if (queryClient) {
          const previousEnvironments =
            (queryClient.getQueryData(environmentQueryKeys.lists()) as Environment[]) || []
          const environmentToDelete = previousEnvironments.find((env) => env.id === environmentId)
