	...
	const sendAudio = useCallback(
		async (audioBlob: Blob) => {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Failed to send audio");
				actions.setError("message", error.message);
				options.onError?.(error);
			}
		},
	...
	const playAudio = useCallback(
		async (audioUrl: string, messageId: string) => {
			try {
				...
				const currentAudio = actions.getAudioElement(
	...
	const resumeAudio = useCallback(async () => {
		...
		if (currentAudio) {
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Failed to resume audio");
				actions.setError("playback", error.message);
				options.onError?.(error);
			}
		}
