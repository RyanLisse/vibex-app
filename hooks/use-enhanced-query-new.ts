...
export function useEnhancedQuery<TData = unknown, TError = Error>(
  ...
  const { isReady: electricReady, client: electricClient } = useElectricContext()
  ...
  const enhancedQueryFn = useCallback(async (): Promise<TData> => {
    return observability.trackOperation(`query.${queryKey.join('.')}`, async () => {
      try {
        if (shouldUseWASM) {
...
export function useVectorSearchQuery<TData = unknown>(
  ...
  const { enabled = true, filters, limit = 10, threshold = 0.7, model = 'default' } = options || {}
  const observability = ObservabilityService.getInstance()

  return useEnhancedQuery(
    queryKeys.wasm.vectorSearch(searchQuery, { filters, limit, threshold, model }),
    async () => {
