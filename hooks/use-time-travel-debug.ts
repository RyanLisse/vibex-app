...
const debugQueryKeys = {
	...
	comparison: (leftId: string, rightId: string) => [
		"debug",
		"comparison",
		leftId,
		rightId,
...
export function useDebugSession(sessionId?: string) {
	...
	const createSession = useMutation({
		mutationFn: async (params: {
			...
			return debugSessionManager.createSession(
				params.executionId,
				params.userId,
				params.options,
			);
		...
		onSuccess: (newSession) => {
			queryClient.setQueryData(
				debugQueryKeys.session(newSession.id),
				newSession,
			);
...
export function useTimeTravelReplay(sessionId?: string) {
	...
	const { data: snapshots = [], isLoading: snapshotsLoading } = useQuery({
		...
		queryFn: async () => {
			if (!session) return [];
			return timeTravel.system.getExecutionSnapshots(
...
export function useSnapshotComparison(
	...
	useEffect(() => {
		const compare = async () => {
			...
			setIsComparing(true);
			try {
				const result = await comparisonEngine.compareSnapshots(
					leftSnapshot,
					rightSnapshot,
					{
...
export function useExecutionPatterns(executionId: string) {
	const {
		...
		queryFn: async () => {
			const snapshots =
				await timeTravel.system.getExecutionSnapshots(executionId);
