...
export function useElectricQuery<T = Record<string, unknown>>(
  ...
  const executeServerQuery = useCallback(async (query: string, params: unknown[]): Promise<T[]> => {
      ...
      if (syncMode === 'local-first' || syncMode === 'hybrid') {
        ...
        if (pglite && isConnected) {
          try {
            ...
            console.warn('Local query failed, falling back to server:', localError)
          }
        }
      }

      // Fallback to server or server-first mode
      if ((result.length === 0 && fallbackToServer) || syncMode === 'server-first') {
        try {
          ...
          throw new Error(`Both local and server queries failed: ${serverError}`)
        }
      }
