...
interface LoadingContextValue {
  ...
  setComponentLoading: (componentId: string, state: Partial<LoadingState>) => void
...
export function LoadingProvider({ children }: { children: React.ReactNode }) {
  ...
  const setComponentLoading = useCallback((componentId: string, state: Partial<LoadingState>) => {
  ...
  return (
    <LoadingContext.Provider value={contextValue}>
      <ErrorBoundary>
        ...
        <OfflineIndicator isOffline={isOffline} queuedOperations={offlineStats.queueSize} />
...
export function useComponentLoading(componentId: string) {
  ...
  const withComponentLoading = useCallback(
    <T extends unknown[]>(asyncFn: (...args: T) => Promise<void>, message?: string) => {
...
export function LoadingWrapper({
  ...
  const { isLoading, error, message, setLoading } = useComponentLoading(componentId)
  ...
  const content = (
    <div className="relative">
      ...
      {error && (
        <div className="mb-4">
          <ErrorDisplay error={error} onRetry={() => setLoading({ error: null })} />
        </div>
      )}
