	...
	return (
		<div className={cn("container mx-auto px-4 py-8", containerClassName)}>
			...
			<div className={cn("mb-8", headerClassName)}>
				<div className="flex items-start justify-between">
					<div className="flex-1">
						...
						{metadata && <div className="mt-2 text-sm text-gray-500">{metadata}</div>}
					</div>
			...
			{hasEmptyState ? (
				...
				<ContentCard className={contentClassName}>{children}</ContentCard>
			) : (
...
function ContentCard({ children, className }: ContentCardProps) {
	return (
		<div className={cn("bg-white rounded-lg shadow-sm border p-6", className)}>{children}</div>
	);
}
	...
	return (
		<ContentCard>
			<div className="text-center py-12">
				{emptyState.icon && (
					<div className="mb-4 flex justify-center text-gray-400">{emptyState.icon}</div>
				)}
...
export function DashboardPageTemplate({
	...
	return (
		<PageTemplate title={title} description={description} showContentCard={false} {...props}>
			{children}
		</PageTemplate>
	);
}
...
export function SettingsPageTemplate({
	...
	return (
		<PageTemplate title={title} description={description} {...props}>
			{children}
		</PageTemplate>
	);
}
...
export { ContentCard, EmptyStateCard };

// Type exports for consumers
export type { PageTemplateProps, ContentCardProps, EmptyStateCardProps };
