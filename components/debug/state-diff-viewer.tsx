		...
		>
			<div className="mb-1 flex items-center gap-2">
				<span
					className={cn(
						"font-medium text-xs",
						type === "old" ? "text-red-700" : "text-green-700",
					)}
				>
...
function StatisticsOverview({
	...
	statistics: ExecutionComparison["statistics"];
}) {
	return (
		<Card>
			<CardHeader>
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							...
							<span className="font-medium text-blue-600">
								{statistics.modifiedPaths}
							</span>
						</div>
						<div className="flex items-center justify-between">
							<span className="text-muted-foreground">Removed</span>
							<span className="font-medium text-red-600">
								{statistics.removedPaths}
							</span>
						</div>
					</div>
				</div>
...
export function StateDiffViewer({
	...
	const activeGroups = Object.entries(groupedDifferences).filter(
		([_, diffs]) => diffs.length > 0,
	);
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground text-sm">
						Step {comparison.leftSnapshot.stepNumber} → Step{" "}
						{comparison.rightSnapshot.stepNumber}
					</p>
				</div>
