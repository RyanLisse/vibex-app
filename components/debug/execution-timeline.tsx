			...
			<Tooltip>
				...
				<TooltipContent>
					<div className="space-y-1">
						...
						{hasError && (
							<div className="text-red-500 text-xs">Error occurred</div>
						)}
						{isCheckpoint && (
							<div className="text-indigo-500 text-xs">Checkpoint</div>
						)}
						{hasBreakpoint && (
							<div className="text-red-600 text-xs">Breakpoint</div>
						)}
					</div>
				</TooltipContent>
			</Tooltip>
...
function TimelineOverview({
	...
	const viewportWidth =
		((viewportEnd - viewportStart) / snapshots.length) * 100;
	...
	return (
		...
		>
			...
			{snapshots.map((snapshot, i) => {
				...
				const style =
					SNAPSHOT_STYLES[snapshot.type] || SNAPSHOT_STYLES.step_start;
...
export function ExecutionTimeline({
	...
	// Progress percentage
	const progress =
		snapshots.length > 0 ? (currentIndex / (snapshots.length - 1)) * 100 : 0;
				...
				<div className="flex items-center justify-between gap-4">
					<div className="flex items-center gap-2">
						...
						<Button
							...
							onClick={() =>
								onStepTo(Math.min(snapshots.length - 1, currentIndex + 1))
							}
							size="icon"
							variant="outline"
						>
					...
					<div className="relative h-2 w-full overflow-hidden rounded-full bg-muted">
						...
						{checkpoints.map((checkpoint) => (
							<div
								...
								key={checkpoint}
								style={{
									left: `${(checkpoint / (snapshots.length - 1)) * 100}%`,
								}}
							/>
						))}
						{/* Error markers */}
								...
								return (
									...
									>
										<TimelineItem
											...
											isCurrent={isCurrent}
											onBreakpointToggle={() =>
												onBreakpointToggle(snapshot.stepNumber)
											}
