...
const STAGE_PROMPTS = {
	...
	refinement:
		"Let's refine the best ideas. How can we combine strengths and address weaknesses?",
	action_planning:
		"What are the concrete next steps? Let's create an action plan with timelines.",
};
...
export function VoiceBrainstorm({
	...
	const extractIdeasFromTranscript = async (
		transcript: string,
			...
			<Card className={cn("mx-auto w-full max-w-2xl", className)}>
				<CardHeader className="text-center">
					...
					<p className="text-muted-foreground">
						Start a voice-powered brainstorming session
					</p>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="text-center">
						<Button
							className="w-full"
							onClick={() => startBrainstormSession("New Idea")}
							size="lg"
						>
				...
				<Card>
					<CardHeader>
						...
						<p className="text-muted-foreground text-sm">
							{STAGE_DESCRIPTIONS[session.stage]}
						</p>
					</CardHeader>
					<CardContent className="space-y-4">
						{/* Recording Controls */}
