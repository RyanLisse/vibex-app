...
export function EnhancedEnvironmentsList({
	...
	const handleActivateEnvironment = async (
		environmentId: string,
		environmentName: string,
	) => {
		if (!userId) return;
					...
					<CardContent>
						<div className="space-y-2">
							<div className="flex items-center gap-2 text-muted-foreground text-sm">
								...
								<span>
									{environment.config?.githubRepository || "No repository"}
								</span>
							</div>
							<div className="flex items-center justify-between">
								<p className="text-muted-foreground text-sm">
									Created{" "}
									{format(new Date(environment.createdAt), "MMM d, yyyy")}
								</p>
								{!environment.isActive && (
									<Button
										disabled={!isOnline}
		...
		<div className="mx-auto mt-14 flex w-full max-w-4xl flex-col gap-y-10">
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					>
						<RefreshCw
							className={`h-4 w-4 ${environmentsLoading ? "animate-spin" : ""}`}
						/>
					</Button>
					{isAuthenticated ? (
						<Button
			...
			{!isOnline && environments && environments.length > 0 && (
				<Alert>
					...
					<AlertDescription>
						You're currently offline. Changes will be synced when connection is
						restored.
					</AlertDescription>
				</Alert>
			)}
