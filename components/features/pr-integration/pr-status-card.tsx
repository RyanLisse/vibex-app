...
export function PRStatusCard({
	...
	useEffect(() => {
		if (
			currentPRStatus.status === "merged" &&
			taskPRLink.autoUpdateStatus &&
			onTaskStatusUpdate
		) {
	...
	const handleRefresh = async () => {
		...
		try {
			...
			const errorMessage =
				err instanceof Error ? err.message : "Failed to fetch PR status";
		...
		<Card className={className}>
						...
						<div>
							...
							<div className="mt-1 flex items-center gap-2">
								<Badge variant="outline">
									PR #{currentPRStatus.prId.replace("pr-", "")}
								</Badge>
								<span className="text-muted-foreground text-sm">
									{taskPRLink.repository}
								</span>
							</div>
						</div>
					</div>
			...
			<CardContent className="space-y-4">
				...
				<div className="flex items-center justify-between">
					...
					<div className="text-right">
						...
						<p className="text-muted-foreground text-xs">
							{
								currentPRStatus.reviewers.filter((r) => r.status === "approved")
									.length
							}{" "}
							of {currentPRStatus.reviewers.length} approved
						</p>
					</div>
				</div>
				...
				<div className="rounded-lg border p-3">
					{currentPRStatus.mergeable &&
					currentPRStatus.reviewStatus === "approved" &&
					currentPRStatus.checks.every(
						(check) => check.status === "success",
					) ? (
						<div
							className="flex items-center gap-2 text-green-700"
							data-testid="merge-ready"
						>
