...
export function ProgressDashboard({
	...
	const isProductivityConcern =
		metrics.averageCompletionTime > 480 ||
		metrics.overdueTasks > metrics.totalTasks * 0.3;
		...
		<div className={`space-y-6 ${className}`}>
					...
					{isProductivityConcern && (
						<div className="mt-4 rounded-lg border border-yellow-200 bg-yellow-50 p-3">
							...
							<p className="mt-1 text-sm text-yellow-700">
								High average completion time or overdue tasks detected. Consider
								reviewing task allocation and workload.
							</p>
			...
			<Tabs className="space-y-4" defaultValue="active">
				...
				<TabsContent className="space-y-4" value="completed">
					<div className="py-8 text-center">
						<div className="mb-2 font-bold text-4xl text-green-600">
							{metrics.completedTasks}
						</div>
						<p className="text-muted-foreground">
						...
						<p className="mt-1 text-muted-foreground text-sm">
							...
							{Math.round(metrics.averageCompletionTime / 60)} hours
						</p>
					</div>
				</TabsContent>
			</Tabs>
