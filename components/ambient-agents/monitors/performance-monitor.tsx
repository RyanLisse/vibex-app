...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
		...
		const interval = setInterval(() => {
			setMetrics((prev) => ({
				...
				memory: {
					usage: Math.max(
						0,
						Math.min(100, prev.memory.usage + (Math.random() - 0.5) * 5),
					),
					trend:
						Math.random() > 0.7
							? Math.random() > 0.5
				...
				network: {
					throughput: Math.max(
						0,
						prev.network.throughput + (Math.random() - 0.5) * 100,
					),
					latency: Math.max(
						0,
						prev.network.latency + (Math.random() - 0.5) * 50,
					),
					trend:
						Math.random() > 0.7
							? Math.random() > 0.5
				...
				rendering: {
					fps: Math.max(
						30,
						Math.min(60, prev.rendering.fps + (Math.random() - 0.5) * 5),
					),
					nodeCount: Math.max(
						0,
						prev.rendering.nodeCount + Math.floor((Math.random() - 0.5) * 3),
					),
					edgeCount: Math.max(
						0,
						prev.rendering.edgeCount + Math.floor((Math.random() - 0.5) * 5),
					),
					renderTime: Math.max(
						1,
						prev.rendering.renderTime + (Math.random() - 0.5) * 2,
					),
				},
	...
	return (
		<Card
			className={`w-80 bg-white/95 shadow-lg backdrop-blur-sm ${className}`}
		>
			<CardHeader
					...
					<div className="space-y-3">
						...
						<div className="grid grid-cols-2 gap-2 text-xs">
							...
							<div>
								...
								<div className="font-medium">
									{metrics.rendering.renderTime.toFixed(1)}ms
								</div>
							</div>
							<div>
					...
					<div className="grid grid-cols-3 gap-2 border-t pt-2">
						<div className="text-center">
							...
							<div className="font-medium text-sm">
								{metrics.rendering.nodeCount}
							</div>
						</div>
						<div className="text-center">
							<div className="text-gray-600 text-xs">Connections</div>
							<div className="font-medium text-sm">
								{metrics.rendering.edgeCount}
							</div>
						</div>
						<div className="text-center">
