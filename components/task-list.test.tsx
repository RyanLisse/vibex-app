...
vi.mock('lucide-react', () => ({
  ...
  Dot: ({ className, ...props }: any) => <svg data-testid="dot-icon" className={className} {...props} />,
...
vi.mock('@/components/ui/text-shimmer', () => ({
  TextShimmer: ({ children, className, ...props }: any) => (
    <div data-testid="text-shimmer" className={className} {...props}>
      {children}
    </div>
  ),
}))
...
describe('TaskList', () => {
  ...
  it('should render archived task details', async () => {
    ...
    await waitFor(() => {
      ...
      const archivedTabContent = screen.getAllByTestId('tabs-content').find(
        content => content.getAttribute('data-value') === 'archived'
      )
