...
export function TimeTravelDebugger({ executionId, onClose }: TimeTravelDebuggerProps) {
  ...
  useEffect(() => {
    const loadSnapshots = async () => {
      try {
        setIsLoading(true)
        const response = await fetch(`/api/time-travel/snapshots?executionId=${executionId}`)
      ...
      <Tabs className="space-y-4" defaultValue="timeline">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="flex items-center space-x-2">
                          <Badge variant="secondary">Step {snapshot.stepIndex}</Badge>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <ScrollArea className="h-48 rounded border p-3">
                    <pre className="text-sm">{JSON.stringify(currentSnapshot.state, null, 2)}</pre>
                  </ScrollArea>
                </div>
                <div>
          ...
          <Card className="p-6">
            ...
            {replaySession ? (
              <div className="space-y-4">
                ...
                <Progress value={(replaySession.currentStep / replaySession.totalSteps) * 100} />
          ...
          <Card className="p-6">
            ...
            {comparisonData ? (
                    ...
                    <div className="mb-4 rounded border p-4" key={index}>
                      <div className="mb-2 flex items-center justify-between">
                        <span className="font-medium">
                          Step {comparison.from.stepIndex} → {comparison.to.stepIndex}
                        </span>
                        <Badge variant="outline">{comparison.summary.stateChanges} changes</Badge>
                      </div>

                      {comparison.diff?.state?.changes
                        ?.slice(0, 3)
                        .map((change: any, changeIndex: number) => (
                          <div className="mb-1 rounded bg-muted p-2 text-sm" key={changeIndex}>
                            <span
                              className={`font-mono ${
                                change.type === 'added'
                                  ? 'text-green-600'
              ...
              <div className="py-8 text-center text-muted-foreground">
                ...
                <p>Select multiple snapshots in the Timeline tab to compare them</p>
              </div>
            )}
          </Card>
        </TabsContent>
        ...
        <TabsContent className="space-y-4" value="analysis">
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Card className="p-6">
              <h3 className="mb-4 font-semibold text-lg">Performance Timeline</h3>
              <div className="h-64">
                <ResponsiveContainer height="100%" width="100%">
                  ...
                  >
                    ...
                    <YAxis />
                    <Tooltip />
                    <Line dataKey="duration" stroke="#8884d8" strokeWidth={2} type="monotone" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1 text-muted-foreground text-sm">
                    ...
                    <li>• Consider adding checkpoints for long-running steps</li>
