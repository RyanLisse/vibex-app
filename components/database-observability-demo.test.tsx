...
vi.mock("@/components/providers/electric-provider", () => ({
	...
	ElectricConnectionStatus: () => (
		<div data-testid="electric-connection-status">Connected</div>
	),
	ElectricSyncButton: () => (
		<button data-testid="electric-sync-button">Sync</button>
	),
	ElectricOfflineIndicator: () => (
		<div data-testid="electric-offline-indicator">Online</div>
	),
}));
...
vi.mock("@/components/providers/query-provider", () => ({
	...
	QueryCacheStatus: () => (
		<div data-testid="query-cache-status">Cache Status</div>
	),
	WASMOptimizationStatus: () => (
		<div data-testid="wasm-optimization-status">WASM Status</div>
	),
}));
...
describe("DatabaseObservabilityDemo", () => {
	...
	it("renders the main demo interface", async () => {
		...
		expect(
			screen.getByText(/Comprehensive showcase of ElectricSQL/),
		).toBeInTheDocument();
	...
	it("displays tab navigation correctly", () => {
		...
		expect(
			screen.getByRole("tab", { name: "Smart Search" }),
		).toBeInTheDocument();
		expect(
	...
	it("handles task creation", async () => {
		...
		vi.mocked(
			require("@/hooks/use-task-queries").useCreateTaskMutation,
		).mockReturnValue({
	...
	it("displays performance metrics when available", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByTestId("wasm-optimization-status"),
			).toBeInTheDocument();
		});
	...
	it("handles metrics toggle", () => {
		...
		// Button text should change
		expect(
			screen.getByRole("button", { name: "Show Metrics" }),
		).toBeInTheDocument();
	});
});
