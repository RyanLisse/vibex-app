...
function findSourceFiles(dir, extensions = ['.ts', '.tsx'], exclude = ['node_modules', '.next', 'dist', 'coverage']) {
  const files = [];
  ...
  function walk(currentDir) {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (!item.includes('.test.') && !item.includes('.spec.') && !item.endsWith('.d.ts') && !item.includes('.stories.')) {
          files.push(fullPath);
        }
      }
    }
