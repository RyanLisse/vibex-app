...
export class PubSubService {
		...
		return this.observability.trackOperation(
			...
			async () => {
				...
				this.observability.recordEvent(
					"pubsub.pattern_subscription.created",
					1,
					{
						pattern,
						subscriptions: this.subscriptions.size.toString(),
		...
		return this.observability.trackOperation("pubsub.unsubscribe", async () => {
			try {
				...
				if (subscription.pattern) {
					...
					this.observability.recordEvent(
						"pubsub.pattern_subscription.removed",
						1,
						{
	...
	getStats() {
		...
		const channelSubscriptions = activeSubscriptions.filter(
			(sub) => !sub.pattern,
		).length;
		const patternSubscriptions = activeSubscriptions.filter(
			(sub) => sub.pattern,
		).length;
	...
	private setupMessageHandler(): void {
		this.subscriberClient!.on(
			"message",
			(channel: string, messageData: string) => {
				try {
					...
					this.observability.recordError(
						"pubsub.message.processing.error",
						error as Error,
						{
							channel,
						},
	...
	private setupPatternMessageHandler(): void {
		this.subscriberClient!.on(
			"pmessage",
			(pattern: string, channel: string, messageData: string) => {
				try {
					const message: PubSubMessage = JSON.parse(messageData);
					...
					this.observability.recordError(
						"pubsub.pattern_message.processing.error",
						error as Error,
						{
							pattern,
							channel,
