...
describe("JobQueueService", () => {
	...
	describe("Queue Management", () => {
		...
		test("should clean up old jobs", async () => {
			...
			for (let i = 0; i < 5; i++) {
				const jobId = await jobQueueService.addJob(queueName, "cleanup-test", {
					index: i,
				});
				const job = await jobQueueService.getNextJob(queueName);
				await jobQueueService.completeJob(job!);
	...
	describe("Integration Features", () => {
		...
		test("should support batch job processing", async () => {
			...
			const jobIds = [];
			for (let i = 0; i < 10; i++) {
				const jobId = await jobQueueService.addJob(queueName, "batch-item", {
					index: i,
				});
				jobIds.push(jobId);
			}
		...
		test("should support distributed job processing across multiple instances", async () => {
			...
			// Add jobs
			for (let i = 0; i < 10; i++) {
				await jobQueueService.addJob(queueName, "distributed-task", {
					index: i,
				});
			}
