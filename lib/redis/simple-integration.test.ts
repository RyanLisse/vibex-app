...
describe('Redis/Valkey Core Implementation', () => {
  ...
  test('should follow TDD approach with comprehensive scenarios', () => {
    ...
    const testScenarios = [
      // PubSub scenarios
      { file: 'pubsub-service.test.ts', scenarios: ['publish', 'subscribe', 'pattern'] },
      
      // Lock scenarios  
      { file: 'lock-service.test.ts', scenarios: ['acquire', 'release', 'expiration', 'retry'] },
      
      // Rate limiting scenarios
      { file: 'rate-limit-service.test.ts', scenarios: ['window', 'token', 'bucket', 'limit'] },
      
      // Job queue scenarios
      { file: 'job-queue-service.test.ts', scenarios: ['job', 'process', 'retry', 'priority'] },
      
      // Metrics scenarios
      { file: 'metrics-service.test.ts', scenarios: ['counter', 'gauge', 'histogram', 'metric'] },
      
      // Session scenarios
      { file: 'session-service.test.ts', scenarios: ['session', 'create', 'expiration', 'user'] }
    ]
