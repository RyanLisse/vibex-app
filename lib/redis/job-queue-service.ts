...
export class JobQueueService {
	...
	private workers = new Map<
		string,
		{ lastHeartbeat: Date; queueName: string }
	>();
	...
	async addJob(
		...
		return this.observability.trackOperation("job_queue.add", async () => {
			...
			const queueKey =
				delay > 0
					? this.buildDelayedQueueKey(queueName)
					: this.buildWaitingQueueKey(queueName);
	...
	async getJobHistory(
		jobId: string,
	): Promise<{ states: string[]; duration: number }> {
	...
	async cleanupCompletedJobs(
		...
		try {
			...
			this.observability.recordError(
				"job_queue.cleanup_completed.error",
				error as Error,
				{
					queue: queueName,
				},
	...
	async cleanupFailedJobs(
		...
		try {
			...
			this.observability.recordError(
				"job_queue.cleanup_failed.error",
				error as Error,
				{
					queue: queueName,
				},
	...
	private async processDelayedJobs(queueName: string): Promise<void> {
		...
		try {
			...
			this.observability.recordError(
				"job_queue.process_delayed.error",
				error as Error,
				{
					queue: queueName,
				},
