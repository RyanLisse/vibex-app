...
export class SessionService {
	...
	async rotateSession(sessionId: string): Promise<string> {
		...
		const { id, createdAt, lastAccessedAt, expiresAt, ...sessionData } =
			session;
	...
	async getUserSessions(userId: string): Promise<SessionData[]> {
		...
		try {
			...
			this.observability.recordError(
				"session.get_user_sessions.error",
				error as Error,
				{
					userId,
				},
	...
	async detectSuspiciousActivity(sessionId: string): Promise<{
		...
		const failedAttempts = activities.filter(
			(a) => a.action === "failed_login_attempt",
		).length;
		if (failedAttempts > 5) {
	...
	private async addUserSession(
		...
		try {
			...
			this.observability.recordError(
				"session.add_user_session.error",
				error as Error,
				{
					userId,
					sessionId,
	...
	private async removeUserSession(
		...
		try {
			...
			this.observability.recordError(
				"session.remove_user_session.error",
				error as Error,
				{
					userId,
					sessionId,
