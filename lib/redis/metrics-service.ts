...
export class MetricsService {
	...
	private metricSubscriptions = new Map<
		string,
		{ id: string; callback: (metric: any) => void }
	>();
	...
	async incrementGauge(
		...
		try {
			...
			this.observability.recordError(
				"metrics.gauge.increment.error",
				error as Error,
			);
		}
	}

	async decrementGauge(
		name: string,
		value: number,
		tags?: Record<string, string>,
	): Promise<void> {
	...
	async getGaugeHistory(
		...
		try {
			...
			this.observability.recordError(
				"metrics.gauge.history.get.error",
				error as Error,
			);
			return [];
		}
	...
	async recordHistogram(
		...
		try {
			...
			const currentMin = currentStats[0]
				? Number.parseFloat(currentStats[0])
				: value;
			const currentMax = currentStats[1]
				? Number.parseFloat(currentStats[1])
				: value;
	...
	async getHistogramBuckets(name: string): Promise<Map<number, number>> {
		...
		try {
			...
			this.observability.recordError(
				"metrics.histogram.buckets.error",
				error as Error,
			);
			return new Map();
		}
	...
	async recordTimeSeries(
		...
		try {
			...
			this.observability.recordEvent("metrics.timeseries.recorded", 1, {
				name,
			});
		} catch (error) {
			this.observability.recordError(
	...
	async aggregateTimeSeries(
		...
		while (currentWindow < endTime) {
			...
			if (windowData.length > 0) {
				...
				switch (aggregation) {
					case "sum":
						aggregatedValue = windowData.reduce(
					...
					case "min":
						aggregatedValue = Math.min(
							...windowData.map((point) => point.value),
						);
						break;
					case "max":
	...
	async publishMetric(streamName: string, metric: any): Promise<void> {
		...
		for (const subscription of this.metricSubscriptions.values()) {
			try {
				...
				this.observability.recordError(
					"metrics.publish.callback.error",
					error as Error,
				);
			}
		}
	...
	async checkAlerts(
		metricName: string,
	): Promise<Array<MetricAlert & { triggered: boolean }>> {
		const client = this.redisManager.getClient();
		const alertKey = this.buildAlertKey(metricName);
