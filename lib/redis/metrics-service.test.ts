...
describe("MetricsService", () => {
	...
	describe("Counter Metrics", () => {
		test("should increment and get counter values", async () => {
			...
			const taggedValue = await metricsService.getCounter(counterName, {
				page: "home",
			});
	...
	describe("Performance Analytics", () => {
		...
		test("should track user behavior analytics", async () => {
			...
			// Track user actions
			await metricsService.trackUserAction(userId, "button_click", {
				button: "signup",
			});
			await metricsService.trackUserAction(userId, "page_view", {
				page: "/pricing",
			});
			await metricsService.trackUserAction(userId, "form_submit", {
				form: "contact",
			});
