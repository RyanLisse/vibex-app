...
describe('MetricsService', () => {
  ...
  describe('Time Series Metrics', () => {
    ...
    test('should support different aggregation functions', async () => {
      ...
      const sumResult = await metricsService.aggregateTimeSeries(seriesName, now - 18000000, now, 3600000, 'sum')
      const avgResult = await metricsService.aggregateTimeSeries(seriesName, now - 18000000, now, 3600000, 'avg')
      const maxResult = await metricsService.aggregateTimeSeries(seriesName, now - 18000000, now, 3600000, 'max')
      const minResult = await metricsService.aggregateTimeSeries(seriesName, now - 18000000, now, 3600000, 'min')
  ...
  describe('Business Intelligence', () => {
    test('should track business KPIs', async () => {
      ...
      await metricsService.recordRevenue(today, 1250.50, 'USD')
    ...
    test('should calculate customer lifetime value', async () => {
      ...
      await metricsService.recordCustomerPurchase(customerId, 99.99, new Date(Date.now() - 86400000 * 30)) // 30 days ago
      await metricsService.recordCustomerPurchase(customerId, 149.99, new Date(Date.now() - 86400000 * 15)) // 15 days ago
