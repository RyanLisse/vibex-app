...
export class RateLimitService {
	...
	async checkSlidingWindowLimit(
        ...
        `;
					...
					this.observability.recordError(
						"rate_limit.sliding_window.error",
						error as Error,
						{
							key,
						},
	...
	async getAdaptiveLimits(
		...
		const adaptedMaxRequests = Math.floor(
			baseOptions.maxRequests * (0.5 + loadFactor * 0.5),
		);
