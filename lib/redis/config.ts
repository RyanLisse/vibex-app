	...
	if (isProduction && process.env.REDIS_REPLICA_HOST) {
		defaultConfig.replicas = {
			read: {
				...
				password:
					process.env.REDIS_REPLICA_PASSWORD || process.env.REDIS_PASSWORD,
				database: Number.parseInt(
					process.env.REDIS_REPLICA_DB || process.env.REDIS_DB || "0",
				),
				type: "standalone",
			},
		};
	...
	if (process.env.REDIS_PUBSUB_HOST || isProduction) {
		defaultConfig.pubsub = {
			...
			port: Number.parseInt(
				process.env.REDIS_PUBSUB_PORT || process.env.REDIS_PORT || "6379",
			),
...
export function getRedisServiceConfig(): RedisServiceConfig {
	...
	return {
		...
		cache: {
			...
			compressionThreshold: Number.parseInt(
				process.env.REDIS_COMPRESSION_THRESHOLD || "1024",
			), // 1KB
		},
		session: {
		...
		pubsub: {
			maxSubscriptions: Number.parseInt(
				process.env.REDIS_MAX_SUBSCRIPTIONS || "1000",
			),
			messageTimeout: Number.parseInt(
				process.env.REDIS_MESSAGE_TIMEOUT || "30000",
			), // 30 seconds
		...
		rateLimiting: {
			defaultWindowSize: Number.parseInt(
				process.env.REDIS_RATE_LIMIT_WINDOW || "60",
			), // 1 minute
			defaultMaxRequests: Number.parseInt(
		...
		jobs: {
			...
			defaultMaxAttempts: Number.parseInt(
				process.env.REDIS_JOB_MAX_ATTEMPTS || "3",
			),
			cleanupInterval: Number.parseInt(
				process.env.REDIS_JOB_CLEANUP_INTERVAL || "300000",
			), // 5 minutes
			retentionTime: Number.parseInt(
				process.env.REDIS_JOB_RETENTION || "86400",
			), // 24 hours
		},
		monitoring: {
