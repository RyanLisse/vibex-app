...
export interface TestMockAdapter {
  fn<T extends (...args: any[]) => any>(implementation?: T): MockFunction<T>;
  spyOn<T, K extends keyof T>(object: T, method: K): MockFunction<T[K] extends (...args: any[]) => any ? T[K] : never>;
...
class BunMockAdapter implements TestMockAdapter {
  ...
  mock(moduleName: string, factory?: () => any): void {
    ...
    if (this.bunTest.mock && this.bunTest.mock.module) {
      ...
      console.warn(`Module mocking for "${moduleName}" not fully supported in Bun compatibility mode`);
    }
  }
