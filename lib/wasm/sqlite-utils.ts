...
export class SQLiteWASMUtils {
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			console.warn(
				"Failed to initialize SQLite WASM, attempting fallback:",
				error,
			);
	...
	private async executeQueryPGLite(
		...
		try {
			...
			const queryPromise = this.executeWithTimeout(
				() => this.pglite!.query(sql, params),
				timeout,
			);
			...
			return {
				...
				rows: pgResult.rows.map((row) =>
					pgResult.fields.map((field) => row[field.name]),
				),
		...
		try {
			...
			if (this.wasmInstance.exports.optimize_query) {
				...
				const optimizationResult = (
					this.wasmInstance.exports.optimize_query as Function
				)(sqlHash);
	...
	private generateOptimizationSuggestions(
		...
		const hasSort = plan.some((step) =>
			step.detail.includes("USE TEMP B-TREE FOR ORDER BY"),
		);
		if (hasSort) {
