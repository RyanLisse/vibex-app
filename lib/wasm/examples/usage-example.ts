...
async function vectorSearchExample() {
  ...
  await observability.trackOperation('example.vector_search.index', async () => {
...
async function sqliteOptimizationExample() {
  ...
  const analysisResult = await observability.trackOperation('example.sqlite.analyze', async () => {
  ...
  console.log(`- Index Recommendations: ${analysisResult.indexRecommendations.join(', ')}`)
...
async function dataProcessingExample() {
  ...
  const sampleData = Array.from({ length: 10_000 }, (_, i) => ({
    ...
    agentType: ['code-assistant', 'ui-assistant', 'data-analyst'][Math.floor(Math.random() * 3)],
  }))
...
async function performanceMonitoringExample() {
  ...
  console.log(`- Average WASM Time: ${perfReport.summary.averageWASMTime.toFixed(2)}ms`)
  console.log(`- Average Fallback Time: ${perfReport.summary.averageFallbackTime.toFixed(2)}ms`)
...
if (typeof window !== 'undefined' || process.argv[1]?.endsWith('usage-example.ts')) {
