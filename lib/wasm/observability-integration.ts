...
export class WASMObservabilityIntegration {
	...
	static getInstance(
		config?: Partial<WASMObservabilityConfig>,
	): WASMObservabilityIntegration {
		if (!WASMObservabilityIntegration.instance) {
			WASMObservabilityIntegration.instance = new WASMObservabilityIntegration(
				config,
			);
		}
	...
	private async reportPerformanceMetrics(
		...
		const event: Partial<ObservabilityEvent> = {
			...
			severity:
				metrics.executionTime > this.config.performanceThreshold
					? "high"
					: "low",
			category: "wasm",
	...
	async getHealthStatus(): Promise<WASMHealthStatus> {
		const healthCheckResult = this.wasmServices
			...
			: {
					...
					services: {
						vectorSearch: {
							healthy: false,
							message: "WASM services not initialized",
						},
						sqlite: {
							healthy: false,
							message: "WASM services not initialized",
		...
		const services: WASMHealthStatus["services"] = {
			...
			sqlite: this.calculateServiceHealth(
				"sqlite",
				healthCheckResult.services.sqlite,
			),
			compute: this.calculateServiceHealth(
				"compute",
				healthCheckResult.services.compute,
			),
			...
			wasmUsageRate:
				performanceReport.summary.wasmCallCount > 0
					? (performanceReport.summary.wasmCallCount /
							(performanceReport.summary.wasmCallCount +
								performanceReport.summary.fallbackCount)) *
						100
					: 0,
			performanceGain:
	...
	private generateHealthRecommendations(
		...
		Object.entries(services).forEach(([name, health]) => {
			...
			if (health.errorRate > 5) {
				recommendations.push(
					`High error rate in ${name} service (${health.errorRate.toFixed(1)}%)`,
				);
			}
		...
		if (performance.wasmUsageRate < 50) {
			recommendations.push(
				"Low WASM usage rate - check WASM availability and thresholds",
			);
		}
		...
		if (memory.memoryPressure === "high") {
			recommendations.push(
				"High memory pressure detected - implement cleanup strategies",
			);
		}
		if (memory.wasmMemoryUsage > this.config.memoryThreshold) {
			recommendations.push(
				"WASM memory usage exceeds threshold - consider memory pooling",
			);
		}
