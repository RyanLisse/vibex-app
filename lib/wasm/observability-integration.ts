...
export class WASMObservabilityIntegration {
	...
	async initialize(): Promise<void> {
		...
		return observability.trackOperation("wasm.observability.initialize", async () => {
			try {
				...
				observability.recordError("wasm.observability.initialization-failed", error as Error);
				throw error;
			}
		});
	}
	...
	private setupPerformanceMonitoring(): void {
		...
		observability.trackOperation = async function <T>(
			...
			try {
				const result = await originalTrackOperation.call(this, operationName, operation);
				...
				if (isWASMOperation) {
					...
					wasmPerformanceTracker.recordOperation(operationName, executionTime, memoryUsage, true, {
						success: false,
	...
	trackModuleLoad(
		...
		return observability.trackOperation(`wasm.module.load.${moduleName}`, async () => {
			const startTime = performance.now();
	...
	async getHealthStatus(): Promise<{
		...
		const recentErrors = observability
			.getErrors(100)
			.filter(
				(error) => error.timestamp > fiveMinutesAgo && error.operation.includes("wasm")
			).length;
