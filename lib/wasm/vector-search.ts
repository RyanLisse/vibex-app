...
export class VectorSearchWASM {
	...
	removeDocuments(documentIds: string[]): void {
		...
		console.log(
			`Removed ${documentIds.length} documents from vector search index`,
		);
	}
	 ...
	 */
	private calculateSimilarityWASM(
		embedding1: number[],
		embedding2: number[],
	): number {
		...
		if (
			!(
				this.inlineWASMInstance &&
				this.inlineWASMInstance.exports.cosine_similarity
			)
		) {
		...
		try {
			...
			const memory = (
				this.inlineWASMInstance.exports.memory as WebAssembly.Memory
			).buffer;
			...
			const cosineSim = (
				this.inlineWASMInstance.exports.cosine_similarity as Function
			)(
			...
			console.warn(
				"WASM similarity calculation failed, falling back to JS:",
				error,
			);
			return this.calculateSimilarityJS(embedding1, embedding2);
		}
	...
	private calculateDistanceWASM(
		...
		if (
			!(
				this.inlineWASMInstance &&
				this.inlineWASMInstance.exports.euclidean_distance
			)
		) {
		...
		try {
			...
			const distance = (
				this.inlineWASMInstance.exports.euclidean_distance as Function
			)(embedding1Offset * 8, embedding2Offset * 8, embedding1.length);
			...
			console.warn(
				"WASM distance calculation failed, falling back to JS:",
				error,
			);
			return this.calculateDistanceJS(embedding1, embedding2);
		}
	...
	private async generateEmbeddingWASM(text: string): Promise<number[]> {
		try {
			...
			if (this.inlineWASMInstance?.exports.normalize_vector) {
				...
				(this.inlineWASMInstance.exports.normalize_vector as Function)(
					0,
					embedding.length,
				);
			...
			console.warn(
				"WASM embedding generation failed, falling back to JS:",
				error,
			);
			return this.generateEmbeddingJS(text);
		}
	...
	private async batchCalculateSimilarities(
		...
		for (let i = 0; i < documents.length; i += batchSize) {
			...
			const batchPromises = batch.map(async (doc) => {
				const similarity = this.calculateSimilarityWASM(
					queryEmbedding,
					doc.embedding,
				);
	...
	clear(): void {
		...
		if (this.inlineWASMInstance?.exports.memory) {
			try {
				const memory = this.inlineWASMInstance.exports
...
// Utility functions
export const createVectorSearchEngine = (
	config?: Partial<VectorSearchConfig>,
) => {
	return new VectorSearchWASM(config);
};

export const getVectorSearchEngine = (
	domain: string,
	config?: Partial<VectorSearchConfig>,
) => {
