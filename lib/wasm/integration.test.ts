...
describe('WASM Services Integration', () => {
  ...
  describe('Vector Search WASM Module', () => {
    const testDocuments = [
      ...
      {
        id: 'doc3',
        content: 'WebAssembly provides near-native performance for web applications',
        embedding: new Array(384).fill(0).map(() => Math.random()),
    ...
    it('should search by text query', async () => {
      const results = await vectorEngine.searchByText('artificial intelligence', {
        maxResults: 3,
      })
    ...
    it('should handle large datasets efficiently', async () => {
      ...
      const stats = vectorEngine.getStats()
      expect(stats.documentsCount).toBe(testDocuments.length + largeDocs.length)
    })
  ...
  describe('SQLite WASM Utilities', () => {
    ...
    it('should execute basic SQL queries', async () => {
      const result = await sqliteUtils.executeQuery('SELECT 1 as test_value, $1 as param_value', [
        'hello',
      ])
    ...
    it('should handle parameterized queries', async () => {
      const result = await sqliteUtils.executeQuery('SELECT $1 as name, $2 as age', ['John', 30])
  ...
  describe('Compute WASM Engine', () => {
    ...
    it('should perform time series analysis', async () => {
      const timeSeriesData = {
        values: Array.from({ length: 100 }, (_, i) => Math.sin(i * 0.1) + Math.random() * 0.1),
    ...
    it('should execute custom WASM functions', async () => {
      if (computeEngine.getStats().isWASMEnabled) {
        try {
          const result = await computeEngine.executeWASMFunction('calculate_mean', [1, 2, 3, 4, 5])
  ...
  describe('Cross-Module Integration', () => {
    it('should coordinate between vector search and compute engines', async () => {
      ...
      const text = 'Integration test for vector search and compute coordination'
    ...
    it('should store vector search results in SQLite', async () => {
      ...
      if (results.length > 0) {
        ...
        try {
          ...
          expect(storedResults.rows[0][0]).toBeGreaterThanOrEqual(results.length)
        } catch (error) {
  ...
  describe('Error Handling and Resilience', () => {
    ...
    it('should handle malformed queries in SQLite', async () => {
      await expect(sqliteUtils.executeQuery('INVALID SQL SYNTAX')).rejects.toThrow()

      // Should still work for valid queries after error
      const result = await sqliteUtils.executeQuery('SELECT 1 as recovery_test')
