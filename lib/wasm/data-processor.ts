...
export class WASMDataProcessor {
	...
	async initialize(): Promise<void> {
		...
		await wasmPerformanceTracker.trackInitialization(
			"data-processor",
			async () => {
				try {
					// Check if WASM should be used
					if (
						!(this.config.enableWASM && shouldUseWASMOptimization("compute"))
					) {
						console.log(
							"WASM data processing not available, using JavaScript fallback",
						);
	...
	private async initializeWorkers(): Promise<void> {
		...
		console.log(
			`Initializing ${this.config.maxWorkers} data processing workers`,
		);
	}
	...
	private async aggregateJS(
		...
		for (const [key, groupData] of groups) {
			...
			for (const op of operations) {
				...
				switch (op.operation) {
					case "sum":
						groupResult[alias] = values.reduce(
