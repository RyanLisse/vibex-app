...
export class WASMPerformanceTracker {
	...
	getMetrics(): WASMPerformanceMetrics {
		...
		return {
			...
			averageWASMTime:
				this.wasmCalls > 0 ? this.totalWASMTime / this.wasmCalls : 0,
			averageFallbackTime:
				this.fallbackCalls > 0
					? this.totalFallbackTime / this.fallbackCalls
					: 0,
	...
	getOperationMetrics(operationType: WASMOperationMetrics["operationType"]): {
		...
		const avgWASMTime =
			wasmOps.length > 0
				? wasmOps.reduce((sum, m) => sum + (m.endTime! - m.startTime), 0) /
					wasmOps.length
				: 0;

		const avgFallbackTime =
			fallbackOps.length > 0
				? fallbackOps.reduce((sum, m) => sum + (m.endTime! - m.startTime), 0) /
					fallbackOps.length
				: 0;

		const avgMemoryDelta =
			successfulOps.length > 0
				? successfulOps.reduce(
						(sum, m) => sum + ((m.memoryAfter || 0) - m.memoryBefore),
						0,
		...
		return {
			...
			successRate:
				metrics.length > 0 ? (successfulOps.length / metrics.length) * 100 : 0,
			performanceGain:
				avgFallbackTime > 0
					? ((avgFallbackTime - avgWASMTime) / avgFallbackTime) * 100
					: 0,
		};
	}
	...
	private getTotalInitTime(): number {
		return Array.from(this.initializationTimes.values()).reduce(
			(sum, time) => sum + time,
			0,
		);
	}
