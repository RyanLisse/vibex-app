...
export class WASMModuleLoader {
	...
	async loadModule(
		...
		// Start loading
		const loadingPromise = this.loadModuleInternal(
			moduleId,
			config,
			onProgress,
		);
			...
			async () => {
				...
				try {
					...
					if (config.bytes) {
						...
						moduleBytes = await this.downloadModule(
							config.url,
							config,
							(loaded, total) => {
								onProgress?.({
					...
					if (config.fallbackUrl && config.url) {
						console.warn(
							`Failed to load WASM from ${config.url}, trying fallback`,
							error,
						);
		...
		for (let attempt = 0; attempt < retries; attempt++) {
			try {
				...
				if (attempt < retries - 1) {
					console.warn(
						`Download attempt ${attempt + 1} failed, retrying...`,
						error,
					);
	...
	getModuleInfo(moduleId: string): {
		...
		return {
			...
			memoryPages: module.memory
				? module.memory.buffer.byteLength / 65_536
				: undefined,
		};
	}
