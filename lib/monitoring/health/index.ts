...
class HealthCheckManager {
	...
	private results: Map<
		string,
		{ result: HealthCheckResult; lastChecked: Date }
	> = new Map();
	...
	async initialize(config: any): Promise<void> {
		...
		console.log(
			`🏥 Health check system initialized with ${this.checks.size} checks`,
		);
	}
	...
	private async runCheck(name: string): Promise<void> {
		...
		try {
			...
			if (previousResult && previousResult.result.status !== result.status) {
				console.log(
					`Health check ${name}: ${previousResult.result.status} → ${result.status}`,
				);
