...
export class CapacityPlanningManager {
	...
	async initialize(config: any): Promise<void> {
		...
		console.log(
			`📈 Capacity planning initialized with ${this.thresholds.length} thresholds`,
		);
	}
		...
		for (const threshold of this.thresholds) {
			try {
				...
				const filtered = history.filter(
					(dp) => dp.timestamp.getTime() > thirtyDaysAgo,
				);
		...
		for (const forecast of report.forecasts) {
			...
			if (criticalPredictions.length > 0) {
				const daysUntilCritical = Math.ceil(
					(criticalPredictions[0].date.getTime() - Date.now()) /
						(24 * 3_600_000),
				);
	...
	async getCurrentMetrics(): Promise<CapacityMetric[]> {
		...
		for (const threshold of this.thresholds) {
			...
			if (growthRate > 0) {
				...
				const warningUtilization =
					threshold.warningThreshold / threshold.criticalThreshold;
				...
				if (currentUtilization < warningUtilization) {
					daysUntilWarning =
						Math.log(warningUtilization / currentUtilization) /
						Math.log(1 + dailyGrowth);
				}

				if (currentUtilization < criticalUtilization) {
					daysUntilCritical =
						Math.log(criticalUtilization / currentUtilization) /
						Math.log(1 + dailyGrowth);
				}
			}
			...
			metrics.push({
				...
				daysUntilWarning: daysUntilWarning
					? Math.ceil(daysUntilWarning)
					: undefined,
				daysUntilCritical: daysUntilCritical
					? Math.ceil(daysUntilCritical)
					: undefined,
			});
		}
	...
	async forecastResource(
		...
		if (lastPrediction.value >= threshold.criticalThreshold) {
			recommendations.push(
				`Urgent: ${resource} will exceed critical threshold within ${days} days`,
			);
			...
			recommendations.push(
				`Warning: ${resource} will exceed warning threshold within ${days} days`,
			);
		...
		if (growthRate > 5) {
			recommendations.push(
				`Rapid growth detected (${growthRate.toFixed(1)}% per day)`,
			);
	...
	async generateReport(): Promise<CapacityReport> {
		...
		const nearTermRisks = metrics.filter(
			(m) => m.daysUntilCritical && m.daysUntilCritical <= 7,
		);

		if (criticalMetrics.length > 0) {
			overallRisk = "critical";
			riskDetails.push(
				`${criticalMetrics.length} resources at critical utilization`,
			);
