...
export class PerformanceMonitor {
  ...
  async time<T>(
    ...
    try {
      ...
      this.track(`${name}_error`, duration, {
        ...options,
        context: { ...options?.context, error: error instanceof Error ? error.message : String(error) },
      });
      throw error;
    }
  }
  ...
  private reportToSentry(metric: PerformanceMetric): void {
    ...
    if (metric.threshold && PERFORMANCE_THRESHOLDS[metric.threshold]) {
      const thresholds = PERFORMANCE_THRESHOLDS[metric.threshold];
      if (metric.value > thresholds.critical) {
        Sentry.captureMessage(`Critical performance: ${metric.name} = ${metric.value}${metric.unit}`, {
          level: "error",
          tags: {
