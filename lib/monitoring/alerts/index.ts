...
export const defaultAlertRules: AlertRule[] = [
	...
	// Capacity alerts
	{
		...
		expression:
			"predict_linear(sum(db_table_size_bytes)[1h], 7 * 24 * 3600) > 900e9",
		for: "1h",
		severity: "medium",
		labels: {
			category: "capacity",
	...
	},
	{
		...
		expression:
			'predict_linear(process_memory_usage_bytes{type="rss"}[1h], 24 * 3600) > 8e9',
		for: "30m",
		severity: "medium",
		labels: {
			category: "capacity",
...
class AlertManager {
	...
	private getMockMetricValue(metric: string): number {
		...
		const mockValues: Record<string, number> = {
			...
			'db_connection_pool_size{state="waiting"}': Math.floor(
				Math.random() * 15,
			),
		...
		const baseMetric = Object.keys(mockValues).find((key) =>
			metric.includes(key),
		);
