...
export function createDefaultLoggingConfig(): LoggingConfig {
	...
	return {
		...
		sampling: {
			...
			highVolumeThreshold: Number.parseInt(
				process.env.LOGGING_HIGH_VOLUME_THRESHOLD || "1000",
			),
		},
		...
		performance: {
			...
			slowOperationThreshold: Number.parseInt(
				process.env.LOGGING_SLOW_THRESHOLD || "1000",
			),
		},
	};
}
