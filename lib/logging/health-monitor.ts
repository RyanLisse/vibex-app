...
export class LoggingHealthMonitor {
	...
	private async checkLoggingPerformance(): Promise<HealthCheck> {
		...
		const status =
			metrics.averageLoggingTime > this.healthThresholds.maxAverageLoggingTime
				? "warn"
				: "pass";
	...
	private async checkMemoryUsage(): Promise<HealthCheck> {
		...
		const status =
			memoryUsage.heapUsed > this.healthThresholds.maxMemoryUsage
				? "warn"
				: "pass";
	...
	private async checkErrorRates(): Promise<HealthCheck> {
		...
		const errorRate =
			metrics.totalLogs > 0 ? metrics.errors / metrics.totalLogs : 0;
		const status =
			errorRate > this.healthThresholds.maxErrorRate ? "warn" : "pass";
