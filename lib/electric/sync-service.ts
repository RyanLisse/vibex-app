...
export class ElectricSyncService {
					...
					(tasks) => {
						...
						if (typeof window !== "undefined") {
							window.dispatchEvent(
								new CustomEvent("tasks:updated", { detail: tasks }),
							);
						}
					},
					{
	...
	private async setupEnvironmentsSync(): Promise<void> {
		return this.observability.trackOperation(
			"sync-service.setup-environments",
			async () => {
	...
	private async setupAgentExecutionsSync(): Promise<void> {
		return this.observability.trackOperation(
			"sync-service.setup-agent-executions",
			async () => {
				const unsubscribe =
					this.electricClient.subscribe<schema.AgentExecution>(
						"agent_executions",
						(executions) => {
	...
	private async setupObservabilityEventsSync(): Promise<void> {
		return this.observability.trackOperation(
			...
			async () => {
				const unsubscribe =
					this.electricClient.subscribe<schema.ObservabilityEvent>(
						"observability_events",
						(events) => {
							this.observability.recordEvent(
								"sync.observability-events.updated",
								{
									count: events.length,
									timestamp: new Date(),
								},
	...
	private async setupAgentMemorySync(): Promise<void> {
		return this.observability.trackOperation(
			"sync-service.setup-agent-memory",
			async () => {
				const unsubscribe = this.electricClient.subscribe<schema.AgentMemory>(
					...
					(memories) => {
						...
						if (typeof window !== "undefined") {
							window.dispatchEvent(
								new CustomEvent("agent-memory:updated", { detail: memories }),
							);
						}
					},
					{
			...
			async () => {
				const unsubscribe = this.electricClient.subscribe<schema.Workflow>(
					...
					(workflows) => {
						...
						if (typeof window !== "undefined") {
							window.dispatchEvent(
								new CustomEvent("workflows:updated", { detail: workflows }),
							);
						}
					},
					{
	...
	private async setupWorkflowExecutionsSync(): Promise<void> {
		return this.observability.trackOperation(
			"sync-service.setup-workflow-executions",
			async () => {
				const unsubscribe =
					this.electricClient.subscribe<schema.WorkflowExecution>(
						"workflow_executions",
						(executions) => {
							this.observability.recordEvent(
								"sync.workflow-executions.updated",
								{
									count: executions.length,
									timestamp: new Date(),
								},
	...
	private async setupExecutionSnapshotsSync(): Promise<void> {
		return this.observability.trackOperation(
			"sync-service.setup-execution-snapshots",
			async () => {
				const unsubscribe =
					this.electricClient.subscribe<schema.ExecutionSnapshot>(
						"execution_snapshots",
						(snapshots) => {
							this.observability.recordEvent(
								"sync.execution-snapshots.updated",
								{
	...
	subscribeToTable<T>(
		...
		return this.observability.trackOperationSync(
			"sync-service.subscribe",
			() => {
		...
		const healthCheckInterval = setInterval(async () => {
			try {
				...
				if (status.offlineQueueSize > 100) {
					console.warn(
						`Large offline queue detected: ${status.offlineQueueSize} operations`,
					);
				}
				...
				if (status.conflictCount > 50) {
					console.warn(
						`High conflict count detected: ${status.conflictCount} conflicts`,
					);
				}
				...
				if (!status.isConnected && status.lastSyncTime) {
					...
					if (timeSinceLastSync > 300_000) {
						// 5 minutes
						console.warn(
							`ElectricSQL disconnected for ${Math.round(timeSinceLastSync / 1000)}s`,
						);
					}
				}
			} catch (error) {
