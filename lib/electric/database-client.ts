...
export class ElectricDatabaseClient {
  ...
  private conflictResolvers = new Map<string, (conflict: any) => Promise<any>>()
   ...
   */
  async executeOperation<T>(operation: DatabaseOperation): Promise<DatabaseResult<T>> {
    return this.observability.trackOperation(`electric.db.${operation.operation}`, async () => {
      ...
      try {
        ...
        if (realtime && (op === 'insert' || op === 'update' || op === 'delete')) {
   ...
   */
  private async executeSelect(table: string, where?: any, userId?: string): Promise<any[]> {
    switch (table) {
      case 'tasks':
   ...
   */
  private async executeInsert(table: string, data: any, userId?: string): Promise<any> {
   ...
   */
  private async executeUpdate(table: string, data: any, where: any, userId?: string): Promise<any> {
  ...
  private async selectTasks(where?: any, userId?: string): Promise<Task[]> {
    ...
    if (where) {
      if (where.status) {
        query = query.where(
          and(userId ? eq(tasks.userId, userId) : sql`true`, eq(tasks.status, where.status))
        )
      }
      if (where.archived !== undefined) {
        query = query.where(
          and(
            ...
            eq(tasks.metadata, sql`jsonb_build_object('isArchived', ${where.archived})`)
          )
        )
      }
    }
   ...
   */
  private async selectEnvironments(where?: any, userId?: string): Promise<Environment[]> {
   ...
   */
  private async selectAgentExecutions(where?: any, userId?: string): Promise<AgentExecution[]> {
  ...
  private async selectObservabilityEvents(
    ...
    if (where?.executionId) {
      query = query.where(eq(observabilityEvents.executionId, where.executionId))
    }
   ...
   */
  private buildCacheKey(table: string, operation: string, where?: any, userId?: string): string {
   ...
   */
  private async setCachedResult(cacheKey: string, data: any, ttl: number): Promise<void> {
    try {
      const redis = await this.getRedisCache()
      if (redis) {
   ...
   */
  subscribeToTable(table: string, callback: (data: any) => void, userId?: string): () => void {
    const subscriptionKey = userId ? `${table}:${userId}` : table
