...
export class ElectricAuthService {
		...
		return this.observability.trackOperation(
			...
			async () => {
				try {
					...
					console.error(
						"Failed to initialize ElectricSQL authentication:",
						error,
					);
					throw error;
				}
	...
	private async generateUserToken(
		...
		return this.observability.trackOperation(
			"electric-auth.generate-user-token",
			async () => {
				const authEndpoint =
					process.env.ELECTRIC_AUTH_ENDPOINT || "/api/auth/electric";
	...
	private async generateAnonymousToken(): Promise<void> {
		return this.observability.trackOperation(
			"electric-auth.generate-anonymous-token",
			async () => {
				...
				console.warn(
					"Using anonymous token for ElectricSQL - not suitable for production",
				);
			},
		);
	}
	...
	private getUserPermissions(userId: string): string[] {
		...
		const adminUsers = (process.env.ELECTRIC_ADMIN_USERS || "")
			.split(",")
			.filter(Boolean);
