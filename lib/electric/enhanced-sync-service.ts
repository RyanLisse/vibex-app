...
export class EnhancedSyncService {
	...
	async initialize(config?: Partial<SyncConfiguration>): Promise<void> {
		...
		return this.observability.trackOperation(
			...
			async () => {
				...
				console.log(
					"Enhanced sync service initialized with config:",
					this.config,
				);
			},
		);
	 ...
	 */
	private async handleElectricChange(
		table: string,
		data: any[],
	): Promise<void> {
		return this.observability.trackOperation(
			"enhanced-sync.handle-electric-change",
			async () => {
				try {
	...
	private async handleDatabaseChange(table: string, event: any): Promise<void> {
		return this.observability.trackOperation(
			"enhanced-sync.handle-database-change",
			async () => {
				try {
					...
					switch (operation) {
						...
						case "update":
							...
							break;
						case "delete":
							await pglite.execute(`DELETE FROM ${table} WHERE id = $1`, [
								data.id,
							]);
							break;
					}
		...
		return this.observability.trackOperation(
			...
			async () => {
				try {
					...
					this.metrics.averageSyncTime = this.calculateAverageSyncTime(
						Date.now() - startTime,
					);
	...
	private async syncTable(table: string): Promise<void> {
		return this.observability.trackOperation(
			`enhanced-sync.sync-table.${table}`,
			async () => {
				...
				// Find differences
				const { toInsert, toUpdate, toDelete } = this.findDifferences(
					localData,
					remoteData,
				);
	...
	private emitSyncEvent(event: SyncEvent): void {
		this.subscriptions.forEach((subscription) => {
			...
			if (
				subscription.filter &&
				!this.matchesFilter(event.data, subscription.filter)
			)
				return;
