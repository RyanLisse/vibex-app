...
export class RealtimeSyncService {
  ...
  private async handleSyncMessage(message: WebSocketMessage): Promise<void> {
    ...
    try {
      switch (message.operation) {
        ...
        case 'update':
        case 'delete':
          await this.applyRemoteChange(message.table, message.operation, message.data)
          break
      }
    } catch (error) {
  ...
  private handleErrorMessage(message: WebSocketMessage): void {
    ...
    this.observability.recordError('realtime-sync.server-error', new Error(JSON.stringify(message)))
  }

  /**
   * Apply remote change to local database
   */
  private async applyRemoteChange(table: string, operation: string, data: any): Promise<void> {
    const dbOperation = {
      table,
   ...
   */
  async sendUpdate(table: string, operation: string, data: any, userId?: string): Promise<void> {
