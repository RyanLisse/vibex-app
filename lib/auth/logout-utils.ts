 ...
 */
export function clearAuthCookies(response: NextResponse, cookiesToClear: string[]): void {
	const cookieOptions = {
		httpOnly: true,
...
export async function handleGetLogout(
	...
	try {
		...
		if (token && config.clientId && config.clientSecret && config.revokeTokenFn) {
			try {
...
export async function handlePostLogout(
	...
	try {
		...
		const { redirect_uri, revoke_token: shouldRevokeToken = true, token: tokenFromBody } = body;
