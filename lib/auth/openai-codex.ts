...
export class CodexAuthenticator {
	...
	private readonly oauthConfig = {
		...
		clientId:
			process.env.OPENAI_CLIENT_ID || "pdlLIX2Y72MIl2rhLhTE9VV9bN905kBh",
		scope: "openid profile email offline_access",
		audience: "https://api.openai.com/v1",
	};

	constructor(
		codexHome: string = process.env.CODEX_HOME ||
	...
	private startOAuthServer(state: string): Promise<string> {
        ...
        `);
			...
			const authUrl = this.buildAuthorizationUrl(
				state,
				this.generatePKCE().codeChallenge,
			);
			open(authUrl);
	...
	private async createApiKey(
		...
		const response = await fetch(
			"https://api.openai.com/v1/dashboard/api_keys",
			{
				method: "POST",
				headers: {
					Authorization: `Bearer ${tokens.access_token}`,
...
export async function validateCodexToken(
	token: string,
): Promise<{ active: boolean }> {
