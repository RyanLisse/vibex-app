...
Always prioritize user experience and provide value in every interaction.
	...
	async processVoiceMessage(audioData: ArrayBuffer): Promise<{
		...
		return this.client.sendVoiceMessage(
			this.agentId,
			session.sessionId,
			audioData,
		);
	...
	private async analyzeMessage(message: string): Promise<{
		...
		return {
			needsBrainstorming: brainstormKeywords.some((keyword) =>
				lowerMessage.includes(keyword),
			),
			needsResearch: researchKeywords.some((keyword) =>
				lowerMessage.includes(keyword),
			),
			needsCoding: codingKeywords.some((keyword) =>
				lowerMessage.includes(keyword),
			),
			complexity:
				message.length > 200 ? "high" : message.length > 50 ? "medium" : "low",
		};
	}

	private async coordinateWithBrainstormAgent(
		message: string,
	): Promise<Message> {
		if (!this.agentId) {
			throw new Error("Orchestrator agent not initialized");
