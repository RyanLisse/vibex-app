...
export class LettaClient {
  ...
  async sendVoiceMessage(
    ...
    const response = await fetch(`${this.config.baseUrl}/agents/${agentId}/voice/messages`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${this.config.apiKey}`,
      },
  ...
  // Memory Management
  async updateMemory(agentId: string, blockId: string, content: string): Promise<void> {
