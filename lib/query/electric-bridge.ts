...
export class ElectricQueryBridge {
	...
	private tableQueryMappings = new Map<string, (data?: any) => string[][]>([
		...
		[
			...
			(data) => [
				...
				...(data?.workflowId
					? [queryKeys.workflows.executions(data.workflowId)]
					: []),
		...
		return observability.trackOperation(
			...
			async () => {
				...
				if (this.config.debugMode) {
					...
					console.log(
						"📊 Table subscriptions:",
						this.config.tableSubscriptions,
					);
				}
			},
		);
	...
	private async setupTableSubscriptions(): Promise<void> {
		return observability.trackOperation(
			"electric-bridge.setup-subscriptions",
			async () => {
	...
	private processBatchedInvalidations(): void {
		...
		observability.trackOperation("electric-bridge.batch-invalidate", () => {
			...
			for (const [tableName, queryKeys] of this.invalidationQueue) {
				...
				if (this.config.debugMode) {
					console.log(
						`🔄 Invalidating ${queryKeys.size} queries for ${tableName}`,
					);
				}
			}
			...
			if (this.config.debugMode && allQueryKeys.size > 0) {
				console.log(
					`✅ Batch invalidated ${allQueryKeys.size} unique query keys`,
				);
			}
		});
	}
	...
	addTableMapping(
		tableName: string,
		queryKeyGenerator: (data?: any) => string[][],
	): void {
