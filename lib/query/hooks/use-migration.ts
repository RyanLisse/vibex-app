 ...
 *
 * NOTE: This file still imports from deprecated Zustand stores for backward compatibility
 * during the migration process. Once all data is migrated and the stores are no longer
	...
	useEffect(() => {
		const checkLocalData = () => {
			try {
				...
				const hasTaskData =
					taskData && JSON.parse(taskData)?.state?.tasks?.length > 0;
				const hasEnvData =
					envData && JSON.parse(envData)?.state?.environments?.length > 0;
...
export function useUnifiedEnvironments() {
	...
	// Zustand store hooks
	const zustandEnvironments = useEnvironmentStore(
	...
	if (status.isUsingDatabase || status.migrationCompleted) {
		return {
			...
			actions: {
				createEnvironment: () =>
					console.warn("Use useCreateEnvironment hook instead"),
				updateEnvironment: () =>
					console.warn("Use useUpdateEnvironment hook instead"),
				deleteEnvironment: () =>
					console.warn("Use useDeleteEnvironment hook instead"),
			},
			source: "database" as const,
		};
	}
