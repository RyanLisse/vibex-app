...
async function fetchAgentExecutions(params: {
  ...
  return observability.trackOperation('api.fetch-agent-executions', async () => {
...
> {
  const searchParams = new URLSearchParams()
  if (options?.includeSnapshots) searchParams.append('includeSnapshots', 'true')
  ...
  return observability.trackOperation('api.fetch-agent-execution', async () => {
    const response = await fetch(`/api/agents/executions/${id}?${searchParams}`)
    if (!response.ok) {
...
async function createAgentExecution(data: CreateAgentExecutionInput): Promise<AgentExecution> {
  return observability.trackOperation('api.create-agent-execution', async () => {
...
async function updateAgentExecution(
  ...
  return observability.trackOperation('api.update-agent-execution', async () => {
...
async function cancelAgentExecution(id: string): Promise<void> {
  return observability.trackOperation('api.cancel-agent-execution', async () => {
  ...
  return useMutation({
    ...
    onMutate: async (newExecution) => {
      ...
      queryClient.setQueryData(queryKeys.executions.all, (old: any) => {
        if (!old) return { executions: [optimisticExecution], total: 1, hasMore: false }
        return {
          ...old,
    ...
    onSuccess: (newExecution, variables, context) => {
      ...
      queryClient.setQueryData(queryKeys.executions.detail(newExecution.id), newExecution)
    ...
    onError: (error, variables, context) => {
      ...
      if (context?.tempId) {
        queryClient.setQueryData(queryKeys.executions.all, (old: any) => {
          ...
          return {
            ...old,
            executions: old.executions.filter((exec: AgentExecution) => exec.id !== context.tempId),
            total: old.total - 1,
          }
        })
      }
...
export function useUpdateAgentExecution() {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateAgentExecutionInput }) =>
      updateAgentExecution(id, data),
    onMutate: async ({ id, data }) => {
      // Cancel outgoing refetches
      await queryClient.cancelQueries({ queryKey: queryKeys.executions.detail(id) })

      // Snapshot previous value
      const previousExecution = queryClient.getQueryData(queryKeys.executions.detail(id))

      // Optimistically update
      queryClient.setQueryData(queryKeys.executions.detail(id), (old: AgentExecution) => ({
        ...old,
        ...data,
    ...
    onSuccess: (updatedExecution) => {
      ...
      queryClient.setQueryData(queryKeys.executions.detail(updatedExecution.id), updatedExecution)

      // Update executions in lists
      queryClient.setQueriesData({ queryKey: queryKeys.executions.all }, (old: any) => {
        if (!old) return old
        return {
          ...old,
          executions: old.executions.map((exec: AgentExecution) =>
      ...
      // Invalidate related queries
      queryClient.invalidateQueries({ queryKey: queryKeys.executions.infinite({}) })
...
export function useRecentAgentExecutions(limit: number = 50) {
  const timeRange = {
...
export function useAgentExecutionStats(timeRange?: { start: Date; end: Date }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return observability.trackOperation('api.agent-execution-stats', async () => {
        const response = await fetch(`/api/agents/executions/stats?${searchParams}`)
        if (!response.ok) {
...
export function useSearchAgentExecutions(query: string, filters: any = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return observability.trackOperation('api.search-agent-executions', async () => {
        const response = await fetch(`/api/agents/executions/search?${searchParams}`)
        if (!response.ok) {
