...
async function fetchWorkflowExecutions(params: {
...
}): Promise<{ executions: WorkflowExecution[]; total: number; hasMore: boolean }> {
  const searchParams = new URLSearchParams()
  ...
  return observability.trackOperation('api.fetch-workflow-executions', async () => {
...
async function fetchWorkflowExecution(id: string): Promise<WorkflowExecution> {
  return observability.trackOperation('api.fetch-workflow-execution', async () => {
...
async function pauseWorkflowExecution(id: string): Promise<WorkflowExecution> {
  return observability.trackOperation('api.pause-workflow-execution', async () => {
...
async function resumeWorkflowExecution(id: string): Promise<WorkflowExecution> {
  return observability.trackOperation('api.resume-workflow-execution', async () => {
...
async function cancelWorkflowExecution(id: string): Promise<WorkflowExecution> {
  return observability.trackOperation('api.cancel-workflow-execution', async () => {
...
async function retryWorkflowExecution(id: string): Promise<WorkflowExecution> {
  return observability.trackOperation('api.retry-workflow-execution', async () => {
...
async function fetchWorkflowStats(timeRange?: { start: Date; end: Date }): Promise<WorkflowStats> {
  const searchParams = new URLSearchParams()
...
export function useCreateWorkflow() {
  ...
  return useMutation({
    ...
    onSuccess: (newWorkflow) => {
      ...
      queryClient.setQueryData(queryKeys.workflows.detail(newWorkflow.id), newWorkflow)
...
export function useUpdateWorkflow() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      queryClient.setQueryData(queryKeys.workflows.detail(id), (old: Workflow) => ({
        ...old,
        ...data,
      }))
    ...
    onSuccess: (updatedWorkflow) => {
      ...
      queryClient.setQueryData(queryKeys.workflows.detail(updatedWorkflow.id), updatedWorkflow)

      // Update workflows in lists
      queryClient.setQueriesData({ queryKey: queryKeys.workflows.lists() }, (old: any) => {
        if (!old) return old
        return {
          ...old,
...
export function useDeleteWorkflow() {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      queryClient.removeQueries({ queryKey: queryKeys.workflows.detail(deletedId) })
      ...
      queryClient.setQueriesData({ queryKey: queryKeys.workflows.lists() }, (old: any) => {
        ...
        return {
          ...old,
          workflows: old.workflows.filter((workflow: Workflow) => workflow.id !== deletedId),
          total: old.total - 1,
        }
      })

      // Remove related executions
      queryClient.removeQueries({ queryKey: queryKeys.workflows.executions(deletedId) })
    },
  })
}
  ...
  return useMutation({
    ...
    onSuccess: (newExecution) => {
      ...
      queryClient.setQueryData(
        ...
        (old: any) => {
          if (!old) return { executions: [newExecution], total: 1, hasMore: false }
          return {
            ...old,
...
export const workflowQueries = {
  ...
  executions: (workflowId: string) => queryKeys.workflows.executions(workflowId),
}
