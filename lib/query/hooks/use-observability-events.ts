...
async function fetchObservabilityEvents(params: {
	...
	return observability.trackOperation(
		"api.fetch-observability-events",
		async () => {
...
async function fetchObservabilityEvent(
	id: string,
): Promise<ObservabilityEvent> {
	return observability.trackOperation(
		"api.fetch-observability-event",
		async () => {
...
async function fetchEventMetrics(
	...
	return observability.trackOperation("api.fetch-event-metrics", async () => {
		const response = await fetch(
			`/api/observability/events/metrics?${searchParams}`,
		);
		if (!response.ok) {
...
async function searchObservabilityEvents(
	...
	return observability.trackOperation(
		"api.search-observability-events",
		async () => {
			const response = await fetch(
				`/api/observability/events/search?${searchParams}`,
			);
			if (!response.ok) {
...
export function useEventsByExecution(executionId: string) {
	return useQuery({
		...
		queryFn: () =>
			fetchObservabilityEvents({ filters: { executionId }, limit: 1000 }),
		enabled: !!executionId,
		staleTime: 1000 * 30, // 30 seconds
...
export function useEventsByTrace(traceId: string) {
	return useQuery({
		...
		queryFn: () =>
			fetchObservabilityEvents({ filters: { traceId }, limit: 1000 }),
		enabled: !!traceId,
		staleTime: 1000 * 30, // 30 seconds
	});
}
...
export function useEventsBySeverity(severity: string[]) {
	return useQuery({
		...
		queryFn: () =>
			fetchObservabilityEvents({ filters: { severity }, limit: 100 }),
...
export function useEventsByType(eventType: string) {
	return useQuery({
		...
		queryFn: () =>
			fetchObservabilityEvents({ filters: { eventType }, limit: 100 }),
		enabled: !!eventType,
		staleTime: 1000 * 30, // 30 seconds
	});
}
...
export function useSearchObservabilityEvents(
	query: string,
	filters?: EventFilters,
) {
	return useQuery({
		...
		onSuccess: (data) => {
			...
			queryClient.setQueryData(queryKeys.events.list(filters), (old: any) => {
				...
				const existingIds = new Set(
					old.events.map((event: ObservabilityEvent) => event.id),
				);
