...
async function createAgentMemory(data: CreateAgentMemoryInput): Promise<AgentMemory> {
...
async function updateAgentMemory(id: string, data: UpdateAgentMemoryInput): Promise<AgentMemory> {
...
async function semanticSearchMemories(
  ...
  if (options?.useWASM && wasmServices.isReady()) {
    return observability.trackOperation('wasm.semantic-search', async () => {
      ...
      const results = await vectorSearch.searchSimilar(queryEmbedding, options.limit || 10)
  ...
  return observability.trackOperation('api.semantic-search-memories', async () => {
...
export function useCreateAgentMemory() {
  ...
  return useMutation({
    ...
    onSuccess: (newMemory) => {
      ...
      queryClient.setQueryData(queryKeys.memory.all.concat('detail', newMemory.id), newMemory)
...
export function useUpdateAgentMemory() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      const previousMemory = queryClient.getQueryData(queryKeys.memory.all.concat('detail', id))

      // Optimistically update
      queryClient.setQueryData(queryKeys.memory.all.concat('detail', id), (old: AgentMemory) => ({
        ...old,
        ...data,
    ...
    onSuccess: (updatedMemory) => {
      ...
      queryClient.setQueriesData({ queryKey: queryKeys.memory.lists() }, (old: any) => {
        if (!old) return old
        return {
          ...old,
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      queryClient.setQueriesData({ queryKey: queryKeys.memory.lists() }, (old: any) => {
        ...
        return {
          ...old,
          memories: old.memories.filter((memory: AgentMemory) => memory.id !== deletedId),
          total: old.total - 1,
        }
      })
...
export function useMemoryStats(agentType?: string, timeRange?: { start: Date; end: Date }) {
  return useQuery({
...
export function useCleanupExpiredMemories() {
  ...
  return useMutation({
    mutationFn: async () => {
      return observability.trackOperation('api.cleanup-expired-memories', async () => {
...
export function useArchiveOldMemories() {
  ...
  return useMutation({
    mutationFn: async (params: { olderThanDays: number; minImportance?: number }) => {
      return observability.trackOperation('api.archive-old-memories', async () => {
...
export function useSharedContext(agentTypes: string[], contextKey: string) {
  return useQuery({
    queryKey: [...queryKeys.memory.all, 'shared', agentTypes.join(','), contextKey],
    queryFn: async () => {
      const searchParams = new URLSearchParams()
      agentTypes.forEach((type) => searchParams.append('agentType', type))
      searchParams.append('contextKey', contextKey)

      return observability.trackOperation('api.fetch-shared-context', async () => {
        const response = await fetch(`/api/agents/memory/shared?${searchParams}`)
        if (!response.ok) {
...
export const agentMemoryQueries = {
  ...
  search: (query: string, agentType?: string) => queryKeys.memory.search(query, agentType),
