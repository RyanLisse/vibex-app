...
async function fetchWorkflowVersions(name: string): Promise<Workflow[]> {
	return observability.trackOperation("query.workflow.versions", async () => {
		const response = await fetch(
			`/api/workflows/versions/${encodeURIComponent(name)}`,
		);
		if (!response.ok) {
...
async function fetchWorkflowExecutions(
	...
	return observability.trackOperation(
		"query.workflow-executions.fetch",
		async () => {
			const searchParams = new URLSearchParams();
			Object.entries(filters).forEach(([key, value]) => {
				if (value !== undefined && value !== null) {
...
async function fetchWorkflowExecution(id: string): Promise<WorkflowExecution> {
	return observability.trackOperation(
		"query.workflow-execution.fetch",
		async () => {
...
async function fetchWorkflowExecutionStats(
	workflowId?: string,
...
async function pauseWorkflowExecution(id: string): Promise<WorkflowExecution> {
	return observability.trackOperation(
		"mutation.workflow-execution.pause",
		async () => {
...
async function resumeWorkflowExecution(id: string): Promise<WorkflowExecution> {
	return observability.trackOperation(
		"mutation.workflow-execution.resume",
		async () => {
...
async function cancelWorkflowExecution(id: string): Promise<WorkflowExecution> {
	return observability.trackOperation(
		"mutation.workflow-execution.cancel",
		async () => {
...
async function fetchExecutionSnapshots(
	...
	return observability.trackOperation(
		"query.execution-snapshots.fetch",
		async () => {
...
async function createExecutionSnapshot(
	data: NewExecutionSnapshot,
): Promise<ExecutionSnapshot> {
	return observability.trackOperation(
		"mutation.execution-snapshot.create",
		async () => {
...
export function useCreateWorkflow(
	options?: UseMutationOptions<Workflow, Error, NewWorkflow>,
) {
	...
	return useMutation({
		...
		onError: (err, newWorkflow, context) => {
			...
			observability.recordError("mutation.workflow.create", err, {
...
export function useUpdateWorkflow(
	options?: UseMutationOptions<
		Workflow,
		Error,
		{ id: string; data: UpdateWorkflowInput }
	>,
) {
	const queryClient = useQueryClient();
...
export function useDeleteWorkflow(
	...
	return useMutation({
		...
		onError: (err, workflowId, context) => {
			...
			observability.recordError("mutation.workflow.delete", err, {
				workflowId,
			});
		},
...
export function useExecuteWorkflow(
	...
	return useMutation({
		...
		onSettled: () => {
			queryClient.invalidateQueries({
				queryKey: queryKeys.workflowExecutions.lists(),
			});
			queryClient.invalidateQueries({
				queryKey: queryKeys.workflowExecutions.stats(),
			});
		},
		...options,
	});
...
export function usePauseWorkflowExecution(
	...
	return useMutation({
		...
		onError: (err, executionId, context) => {
			...
			observability.recordError("mutation.workflow-execution.pause", err, {
				executionId,
			});
		},
...
export function useResumeWorkflowExecution(
	...
	return useMutation({
		...
		onError: (err, executionId, context) => {
			...
			observability.recordError("mutation.workflow-execution.resume", err, {
				executionId,
			});
		},
		onSettled: (data, error, executionId) => {
...
export function useCancelWorkflowExecution(
	...
	return useMutation({
		...
		onError: (err, executionId, context) => {
			...
			observability.recordError("mutation.workflow-execution.cancel", err, {
				executionId,
			});
		},
		onSuccess: (execution) => {
...
export function useWorkflowsSubscription(
	...
	useEffect(() => {
		...
		const unsubscribe = electricClient.subscribe(
			...
			(workflows: Workflow[]) => {
				queryClient.setQueryData<Workflow[]>(
					queryKeys.workflows.list(filters),
					workflows,
				);
...
export function useWorkflowExecutionsSubscription(
	...
	useEffect(() => {
		...
		const unsubscribe = electricClient.subscribe(
			"workflowExecutions",
			(executions: WorkflowExecution[]) => {
				observability.trackOperation(
					"realtime.workflow-executions.update",
					() => {
						queryClient.setQueryData<ExecutionsResponse>(
...
export function useWorkflowsByTag(
	tags: string[],
	options?: UseQueryOptions<Workflow[], Error>,
) {
	return useWorkflows(
