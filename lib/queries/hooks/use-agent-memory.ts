...
// API functions
async function fetchMemories(filters: MemoryFilters = {}): Promise<AgentMemory[]> {
...
async function searchMemoryByVector(
  ...
  return observability.trackOperation('query.memory.search-vector', async () => {
...
async function updateMemory(id: string, data: UpdateMemoryInput): Promise<AgentMemory> {
...
export function useAgentMemory(id: string, options?: UseQueryOptions<AgentMemory, Error>) {
  return useQuery({
...
export function useMemoryStats(agentType?: string, options?: UseQueryOptions<MemoryStats, Error>) {
  return useQuery({
...
export function useCreateAgentMemory(
  ...
  return useMutation({
    ...
    onError: (err, newMemory, context) => {
      observability.recordError('mutation.memory.create', err, { memory: newMemory })
    },
    onSuccess: (data) => {
...
export function useUpdateAgentMemory(
  options?: UseMutationOptions<AgentMemory, Error, { id: string; data: UpdateMemoryInput }>
) {
  const queryClient = useQueryClient()
  ...
  return useMutation({
    ...
    onMutate: async ({ id, data }) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.memory.detail(id) })

      const previousMemory = queryClient.getQueryData<AgentMemory>(queryKeys.memory.detail(id))
    ...
    onError: (err, variables, context) => {
      if (context?.previousMemory) {
        queryClient.setQueryData(queryKeys.memory.detail(variables.id), context.previousMemory)
      }
    ...
    onSettled: (data, error, variables) => {
      ...
      queryClient.invalidateQueries({
        predicate: (query) =>
          ...
          (query.queryKey.includes('search') || query.queryKey.includes('vector')),
      })
    },
    ...options,
  })
...
export function useDeleteAgentMemory(options?: UseMutationOptions<void, Error, string>) {
  const queryClient = useQueryClient()
  ...
  return useMutation({
    ...
    onMutate: async (memoryId) => {
      ...
      queryClient.setQueriesData<AgentMemory[]>({ queryKey: queryKeys.memory.lists() }, (old) => {
        if (!old) return old
    ...
    onSettled: (data, error, memoryId) => {
      queryClient.removeQueries({ queryKey: queryKeys.memory.detail(memoryId) })
      queryClient.invalidateQueries({ queryKey: queryKeys.memory.lists() })
      queryClient.invalidateQueries({ queryKey: queryKeys.memory.stats() })
    },
    ...options,
...
export function useCleanupMemories(options?: UseMutationOptions<{ deleted: number }, Error, void>) {
  const queryClient = useQueryClient()
      ...
      (memories: AgentMemory[]) => {
        observability.trackOperation('realtime.memory.update', () => {
          ...
          if (filters?.agentType) {
            ...
            queryClient.setQueryData<AgentMemory[]>(queryKeys.memory.list(), memories)
          }

          // Update individual memory caches
          memories.forEach((memory) => {
            queryClient.setQueryData(queryKeys.memory.detail(memory.id), memory)
          })
...
export function useWASMVectorSearch(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return observability.trackOperation('wasm.memory.vector-search', async () => {
...
export async function prefetchMemoryStats(queryClient: QueryClient, agentType?: string) {
  await queryClient.prefetchQuery({
    queryKey: queryKeys.memory.stats(agentType),
    queryFn: () => fetchMemoryStats(agentType),
    staleTime: 1000 * 60 * 2, // 2 minutes
