...
async function fetchEventTimeline(executionId: string): Promise<EventTimeline> {
	return observability.trackOperation("query.events.timeline", async () => {
		const response = await fetch(
			`/api/observability-events/timeline/${executionId}`,
		);
		if (!response.ok) {
...
async function fetchEventAggregation(
	...
	return observability.trackOperation("query.events.aggregate", async () => {
		...
		const response = await fetch(
			`/api/observability-events/aggregate?${searchParams}`,
		);
		if (!response.ok) {
...
async function bulkCreateEvents(
	events: NewObservabilityEvent[],
): Promise<ObservabilityEvent[]> {
	return observability.trackOperation(
		"mutation.events.bulk-create",
		async () => {
...
export function useInfiniteObservabilityEvents(
	filters: EventFilters = {},
	options?: any,
) {
	...
	return useMutation({
		...
		onSettled: () => {
			...
			queryClient.invalidateQueries({
				queryKey: queryKeys.events.aggregate("severity"),
			});
		},
		...options,
	});
...
export function useBulkCreateObservabilityEvents(
	...
	return useMutation({
		...
		onSuccess: (events) => {
			...
			Object.entries(eventsByExecution).forEach(
		...
		onSettled: () => {
			queryClient.invalidateQueries({ queryKey: queryKeys.events.lists() });
			queryClient.invalidateQueries({
				queryKey: queryKeys.events.aggregate("eventType"),
			});
		},
		...options,
	});
				...
				observability.trackOperation("realtime.events.update", () => {
					...
					queryClient.invalidateQueries({
						predicate: (query) =>
							...
							query.queryKey.includes("aggregate"),
					});
...
export async function prefetchEventTimeline(
