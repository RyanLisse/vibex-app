...
// API functions
async function fetchExecutions(filters: AgentExecutionFilters = {}): Promise<ExecutionsResponse> {
...
async function fetchExecutionStats(filters?: AgentExecutionFilters): Promise<ExecutionStats> {
...
async function fetchExecutionPerformance(executionId: string): Promise<ExecutionPerformance> {
  return observability.trackOperation('query.execution.performance', async () => {
    const response = await fetch(`/api/agent-executions/${executionId}/performance`)
    if (!response.ok) {
...
async function createExecution(data: NewAgentExecution): Promise<AgentExecution> {
...
export function useAgentExecution(id: string, options?: UseQueryOptions<AgentExecution, Error>) {
  return useQuery({
...
export function useInfiniteAgentExecutions(filters: AgentExecutionFilters = {}, options?: any) {
  ...
  return useMutation({
    ...
    onMutate: async (newExecutionData) => {
      ...
      if (newExecutionData.taskId) {
        const previousTaskExecutions = queryClient.getQueryData<ExecutionsResponse>(
        ...
        if (previousTaskExecutions) {
          queryClient.setQueryData<ExecutionsResponse>(
            queryKeys.executions.byTask(newExecutionData.taskId),
            {
              ...
              executions: [optimisticExecution, ...previousTaskExecutions.executions],
    ...
    onError: (err, newExecution, context) => {
      observability.recordError('mutation.execution.create', err, { execution: newExecution })
    },
    onSuccess: (data) => {
...
export function useCancelAgentExecution(
  ...
  return useMutation({
    ...
    onMutate: async (executionId) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.executions.detail(executionId) })
      ...
      if (previousExecution) {
        ...
        queryClient.setQueryData(queryKeys.executions.detail(executionId), cancelledExecution)
      }
    ...
    onError: (err, executionId, context) => {
      ...
      observability.recordError('mutation.execution.cancel', err, { executionId })
    },
    onSuccess: (data) => {
          ...
          queryClient.setQueryData<ExecutionsResponse>(
            ...
            (old) => {
              ...
              executions.forEach((execution) => {
                ...
                queryClient.setQueryData(queryKeys.executions.detail(execution.id), execution)
