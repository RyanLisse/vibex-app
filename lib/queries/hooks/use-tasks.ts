	...
	return useQuery({
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/tasks/stats${userId ? `?userId=${userId}` : ""}`,
			);
...
export function useArchiveTask() {
	...
	return useMutation({
		mutationFn: (id: string) =>
			updateTask.mutateAsync({ id, data: { isArchived: true } }),
	});
}
...
export function useUnarchiveTask() {
	...
	return useMutation({
		mutationFn: (id: string) =>
			updateTask.mutateAsync({ id, data: { isArchived: false } }),
	});
}
...
export function useBulkUpdateTasks() {
	...
	return useMutation({
		...
		mutationFn: async (
			updates: Array<{ id: string; data: UpdateTaskInput }>,
		) => {
