...
export class PerformanceBenchmarker {
  ...
  async compareAgainstBaseline(): Promise<{
    ...
    const currentAvg =
      currentBenchmarks.flatMap((s) => s.results).reduce((sum, r) => sum + r.executionTime, 0) /
  ...
  private async benchmarkDatabaseOperations(): Promise<BenchmarkSuite> {
    ...
    results.push(
      await this.runBenchmark('single_insert', 'Insert single task record', async () => {
    ...
    results.push(
      await this.runBenchmark('batch_insert', 'Insert 100 task records in batch', async () => {
    ...
    results.push(
      await this.runBenchmark('simple_select', 'Select tasks by user ID', async () => {
        await db.select().from(tasks).where(eq(tasks.userId, 'benchmark-user')).limit(20)
      })
    )

    // Complex join benchmark
    results.push(
      await this.runBenchmark('complex_join', 'Join tasks with agent executions', async () => {
        await db
          .select()
          .from(tasks)
    ...
    // Update benchmark
    results.push(
      await this.runBenchmark('update_operation', 'Update task status', async () => {
        await db
          .update(tasks)
          .set({ status: 'completed', updatedAt: new Date() })
          .where(and(eq(tasks.userId, 'benchmark-user'), eq(tasks.status, 'pending')))
      })
    )
  ...
  private async benchmarkQueryPerformance(): Promise<BenchmarkSuite> {
    ...
    results.push(
      await this.runBenchmark('filtered_query', 'Query with multiple filters', async () => {
        await db
          .select()
          .from(tasks)
          .where(
    ...
    results.push(
      await this.runBenchmark('search_query', 'Text search in task titles', async () => {
        await db
          .select()
          .from(tasks)
          .where(and(eq(tasks.userId, 'benchmark-user'), like(tasks.title, '%benchmark%')))
          .limit(10)
      })
    )
    ...
    results.push(
      await this.runBenchmark('aggregation_query', 'Count tasks by status', async () => {
        await db
          .select({
            status: tasks.status,
    ...
    results.push(
      await this.runBenchmark('pagination_query', 'Paginated task list', async () => {
        await db
          .select()
          .from(tasks)
          .where(eq(tasks.userId, 'benchmark-user'))
  ...
  private async benchmarkElectricSqlSync(): Promise<BenchmarkSuite> {
    ...
    results.push(
      await this.runBenchmark('sync_latency', 'Simulated sync operation latency', async () => {
        // Simulate sync operation
        await new Promise((resolve) => setTimeout(resolve, 10))
        await db.select().from(tasks).where(eq(tasks.userId, 'benchmark-user')).limit(5)
      })
    )

    // Batch sync benchmark
    results.push(
      await this.runBenchmark('batch_sync', 'Batch sync simulation', async () => {
  ...
  private async benchmarkVectorSearch(): Promise<BenchmarkSuite> {
    ...
    results.push(
      await this.runBenchmark('vector_search', 'Vector similarity search', async () => {
  ...
  private async benchmarkConcurrentOperations(): Promise<BenchmarkSuite> {
    ...
    results.push(
      await this.runBenchmark('concurrent_reads', '10 concurrent read operations', async () => {
        const promises = Array.from({ length: 10 }, () =>
          db.select().from(tasks).where(eq(tasks.userId, 'benchmark-user')).limit(5)
        )
    ...
    results.push(
      await this.runBenchmark('mixed_operations', 'Mixed read/write operations', async () => {
        const promises = [
  ...
  private async benchmarkResourceUsage(): Promise<BenchmarkSuite> {
    ...
    results.push(
      await this.runBenchmark('memory_usage', 'Memory usage during large query', async () => {
  ...
  private async getDatabaseInfo(): Promise<{
    ...
    try {
      ...
      const connectionResult = await db.execute(sql`SELECT count(*) FROM pg_stat_activity`)
