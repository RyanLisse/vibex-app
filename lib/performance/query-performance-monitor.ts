...
export class QueryPerformanceMonitor {
	...
	getCurrentMetrics(): {
		...
		const errorRate =
			totalQueries > 0
				? (recentMetrics.filter((m) => !m.success).length / totalQueries) * 100
				: 0;

		const recentAlerts = this.alerts
			.filter(
				(a) =>
					!a.resolved && Date.now() - a.timestamp.getTime() < 60 * 60 * 1000,
			) // Last hour
	...
	analyzePerformanceTrends(): {
		...
		const previousAvg =
			previousMetrics.reduce((sum, m) => sum + m.executionTime, 0) /
		...
		for (const [queryId, recentGroup] of queryGroups) {
			...
			const recentAvgTime =
				recentGroup.reduce((sum, m) => sum + m.executionTime, 0) /
				recentGroup.length;
			const previousAvgTime =
				previousGroup.reduce((sum, m) => sum + m.executionTime, 0) /
				previousGroup.length;

			const queryChangePercent =
				((recentAvgTime - previousAvgTime) / previousAvgTime) * 100;

			if (queryChangePercent > 50) {
				regressions.push(
					`Query ${queryId}: ${Math.round(queryChangePercent)}% slower`,
				);
			} else if (queryChangePercent < -30) {
				improvements.push(
					`Query ${queryId}: ${Math.round(Math.abs(queryChangePercent))}% faster`,
				);
			}
		}
	...
	private createAlert(
		...
		// Log alert
		console.warn(
			`🚨 Performance Alert [${newAlert.severity.toUpperCase()}]: ${newAlert.message}`,
		);
