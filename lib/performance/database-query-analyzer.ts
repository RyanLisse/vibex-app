...
export class DatabaseQueryAnalyzer {
		...
		for (const query of criticalQueries) {
			try {
				const analysis = await this.analyzeQuery(
					query,
					this.generateSampleParams(query),
				);
				analyses.push(analysis);
			} catch (error) {
				console.warn(
					`Failed to analyze query: ${query.substring(0, 50)}...`,
					error,
				);
			}
		}
	...
	private identifyBottlenecks(planData: any): string[] {
		...
		if (this.hasExpensiveNestedLoop(planData.Plan)) {
			bottlenecks.push(
				"Expensive nested loop - consider index or join optimization",
			);
		}
	...
	private hasExpensiveNestedLoop(plan: any): boolean {
		...
		if (plan.Plans) {
			return plan.Plans.some((child: any) =>
				this.hasExpensiveNestedLoop(child),
			);
		}
	...
	private generateRecommendations(analysis: QueryAnalysis): string[] {
		...
		if (analysis.executionTime > 100) {
			recommendations.push(
				"Consider adding appropriate indexes to reduce execution time",
			);
		}

		if (
			analysis.bottlenecks.includes("Sequential scan detected - missing index")
		) {
			recommendations.push(
				"Add indexes for WHERE clause columns to avoid sequential scans",
			);
		}

		if (analysis.queryType === "SELECT" && analysis.rows > 1000) {
			recommendations.push(
				"Consider adding LIMIT clause or pagination for large result sets",
			);
		}
	...
	private generateGlobalRecommendations(
		...
		if (slowQueries.length > 0) {
			immediate.push(
				`Optimize ${slowQueries.length} slow queries (>100ms execution time)`,
			);
		}
		...
		if (missingIndexes > 0) {
			shortTerm.push(
				`Add ${missingIndexes} recommended indexes for better performance`,
			);
		}
