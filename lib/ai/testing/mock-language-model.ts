...
const DEFAULT_RESPONSES: Record<string, PromptResponse> = {
	explain: {
		chunks: [
			{
				type: "text-delta",
				textDelta: "Here is an explanation of the concept:\n\n",
			},
			{
				type: "text-delta",
			...
			{
				type: "text-delta",
				textDelta: " The explanation includes relevant context and examples.",
			},
	...
	error: {
		chunks: [
			{
				type: "text-delta",
				textDelta: "I encountered an error while processing your request.",
			},
	...
	default: {
		chunks: [
			{
				type: "text-delta",
				textDelta: "This is a default response for testing purposes.",
			},
			{ type: "finish", finishReason: "stop" },
		],
		delay: 25,
...
export function createTestChatModel(
	customResponses?: Record<string, PromptResponse>,
) {
	...
	return new MockLanguageModelV1({
		...
		doGenerate: async ({ prompt }) => {
			...
			const finishChunk = response.chunks.find(
				(chunk) => chunk.type === "finish",
			);
			const finishReason =
				finishChunk?.type === "finish" ? finishChunk.finishReason : "stop";
...
export const AITestUtils = {
	 ...
	 */
	createTestPrompt: (content: string, role: "user" | "assistant" = "user") => [
		{ role, content },
	],
	...
	assertResponseContains: (response: string, expectedContent: string) => {
		if (!response.includes(expectedContent)) {
			throw new Error(
				`Expected response to contain "${expectedContent}", but got: "${response}"`,
			);
		}
	},
