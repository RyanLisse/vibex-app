...
function getResponseChunksByPrompt(prompt: CoreMessage[]) {
  ...
  if (typeof recentMessage === 'string') {
    if (recentMessage.includes('explain')) {
      return [
        { type: 'text-delta' as const, textDelta: 'Here is an explanation of ' },
        { type: 'text-delta' as const, textDelta: 'the concept you requested. ' },
    ...
    if (recentMessage.includes('code')) {
      return [
        ...
        { type: 'text-delta' as const, textDelta: '  return "Hello, World!";\n' },
    ...
    if (recentMessage.includes('error') || recentMessage.includes('fail')) {
      return [
        { type: 'text-delta' as const, textDelta: 'I encountered an error while ' },
  ...
  return [
    { type: 'text-delta' as const, textDelta: 'This is a default test response ' },
    { type: 'text-delta' as const, textDelta: 'from the mock AI model. ' },
    { type: 'text-delta' as const, textDelta: 'It provides consistent output ' },
    { type: 'text-delta' as const, textDelta: 'for testing purposes.' },
    { type: 'finish' as const, finishReason: 'stop' as const },
  ]
}
...
function getClaudeResponseChunks(prompt: CoreMessage[]) {
  ...
  if (typeof recentMessage === 'string' && recentMessage.includes('analysis')) {
    return [
      ...
      { type: 'text-delta' as const, textDelta: 'I can provide the following insights:\n\n' },
  ...
  return [
    { type: 'text-delta' as const, textDelta: "I'm Claude, and I'm here to help. " },
    { type: 'text-delta' as const, textDelta: 'This is a test response for verification.' },
    { type: 'finish' as const, finishReason: 'stop' as const },
  ]
}
...
function getGPTResponseChunks(prompt: CoreMessage[]) {
  ...
  if (typeof recentMessage === 'string' && recentMessage.includes('creative')) {
    return [
      { type: 'text-delta' as const, textDelta: 'Let me think creatively about this... ' },
      { type: 'text-delta' as const, textDelta: 'Here are some innovative ideas:\n\n' },
      { type: 'text-delta' as const, textDelta: '💡 Idea 1: Creative solution\n' },
      { type: 'text-delta' as const, textDelta: '🚀 Idea 2: Innovative approach\n' },
      { type: 'text-delta' as const, textDelta: '✨ Idea 3: Unique perspective' },
      { type: 'finish' as const, finishReason: 'stop' as const },
    ]
  }

  return [
    { type: 'text-delta' as const, textDelta: "Hello! I'm a GPT model ready to assist. " },
    { type: 'text-delta' as const, textDelta: 'This is a standardized test response.' },
    { type: 'finish' as const, finishReason: 'stop' as const },
  ]
}
