...
describe("AgentMemorySystem", () => {
	...
	describe("storeMemory", () => {
		it("should store a memory and share it if enabled", async () => {
			...
			const createSpy = vi
				.spyOn(memoryRepository, "create")
				.mockResolvedValue(mockMemory);
			const shareSpy = vi
				.spyOn(memorySharingService, "shareMemory")
				.mockResolvedValue(["agent2"]);
	...
	describe("getMemoryContext", () => {
		it("should get memory context from context manager", async () => {
			...
			const result = await memorySystem.getMemoryContext(
				"test-agent",
				"test-context",
				{
	...
	describe("memory sharing", () => {
		it("should share memory with specific agents", async () => {
			...
			const result = await memorySystem.shareMemory(mockMemory, [
				"agent2",
				"agent3",
			]);

			expect(shareMemoriesSpy).toHaveBeenCalledWith(
				[mockMemory],
				["agent2", "agent3"],
			);
	...
	describe("statistics and health", () => {
		it("should get memory statistics", async () => {
			...
			vi.spyOn(memorySharingService, "getSharingStats").mockReturnValue(
				mockSharingStats,
			);
		...
		it("should get health report", async () => {
			...
			vi.spyOn(memoryLifecycleManager, "getHealthReport").mockResolvedValue(
				mockReport,
			);
	...
	describe("cache management", () => {
		it("should clear all caches", () => {
			...
			const clearSuggestionCacheSpy = vi.spyOn(
