...
export class ProgressTracker extends EventEmitter {
	...
	updateProgress(update?: Partial<MigrationProgress>): void {
		...
		if (update) {
			...
			if (update.errors) {
				...
				if (this.recentErrors.length > this.config.errorHistoryLimit) {
					this.recentErrors = this.recentErrors.slice(
						-this.config.errorHistoryLimit,
					);
				}
			}
		}
		...
		if (this.currentProgress.processedItems > 0) {
			...
			const remainingItems =
				this.currentProgress.totalItems - this.currentProgress.processedItems;
			this.currentProgress.estimatedTimeRemaining = Math.round(
		...
		if (
			this.currentProgress.processedItems >= this.currentProgress.totalItems
		) {
	...
	getProgressPercentage(): number {
		if (!this.currentProgress || this.currentProgress.totalItems === 0)
			return 0;
	...
	private calculateCurrentMetrics(): ProgressMetrics {
		...
		const itemsPerSecond =
			elapsedTime > 0 ? (processedItems * 1000) / elapsedTime : 0;
		...
		const successRate =
			processedItems > 0
				? ((processedItems - this.currentProgress!.errors.length) /
						processedItems) *
					100
				: 100;
	...
	private calculateMetrics(): void {
		...
		if (
			metrics.itemsPerSecond < 1 &&
			this.currentProgress.processedItems > 10
		) {
