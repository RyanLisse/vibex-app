...
## Data Comparison
Records Compared: ${comparison.recordsCompared || 0}
Differences Found: ${comparison.differences?.length || 0}

## Recommendations
${validation.recommendations?.map((rec) => `- ${rec}`).join("\n") || "No recommendations"}

---
Report generated by Migration Validation Service
`;
		...
		return {
			...
			validation,
			comparison,
			success: validation.valid && comparison.identical,
		};
	}
}
