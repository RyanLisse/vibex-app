...
export class RollbackService {
   ...
   */
  private async findBackupForRollback(targetBackupId?: string): Promise<BackupManifest | null> {
    if (targetBackupId) {
      // Use specific backup
   ...
   */
  private async preserveNewData(): Promise<{ count: number; warnings: string[] }> {
    ...
    try {
      ...
      const newTasks = await db.select().from(tasks).where(/* gt(tasks.createdAt, migrationTime) */)
      ...
      if (newTasks.length > 0) {
        ...
        warnings.push(`Preserved ${newTasks.length} tasks created after migration`)
      }
      ...
      if (newEnvironments.length > 0) {
        localStorage.setItem('preserved-environments', JSON.stringify(newEnvironments))
        count += newEnvironments.length
        warnings.push(`Preserved ${newEnvironments.length} environments created after migration`)
      }
    } catch (error) {
  ...
  private async validateRollback(): Promise<{
    ...
    try {
      ...
      const [envCount] = await db.select({ count: environments.id }).from(environments)

      if (Number(taskCount.count) > 0) {
        warnings.push(`Database still contains ${taskCount.count} tasks after rollback`)
      }

      if (Number(envCount.count) > 0) {
        warnings.push(`Database still contains ${envCount.count} environments after rollback`)
      }
...
${
  this.listRollbackPoints()
    .map((rp) => `- ${rp.id}: ${rp.description} (${rp.timestamp.toLocaleString()})`)
    .join('\n') || 'None'
}
`
