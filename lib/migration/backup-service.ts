...
export class BackupService {
  ...
  async createBackup(options: BackupOptions): Promise<BackupResult> {
    ...
    try {
      ...
      if (options.source === 'LOCALSTORAGE') {
        ...
        itemCount = (databaseData.tasks?.length || 0) + (databaseData.environments?.length || 0)
        dataTypes = ['database']
      }
  ...
  verifyBackup(backupId: string): {
    ...
    // Check timestamp
    if (!(backup.timestamp instanceof Date) || isNaN(backup.timestamp.getTime())) {
  ...
  private extractLocalStorageData(): LocalStorageData {
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.includes('form') || key.includes('draft') || key.includes('temp'))) {
          try {
