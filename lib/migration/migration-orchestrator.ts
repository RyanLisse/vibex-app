...
export class MigrationOrchestrator extends EventEmitter {
  ...
  static getInstance(config?: Partial<OrchestrationConfig>): MigrationOrchestrator {
  ...
  async initialize(): Promise<void> {
    ...
    if (
      ...
      this.emit('migrationOpportunity', {
        ...
        recommendations: autoDetector.getMigrationRecommendations(detectionResult),
      })
    }
   ...
   * Start migration
   */
  async startMigration(userId: string, config?: Partial<MigrationConfig>): Promise<string> {
  ...
  private async validatePreMigration(): Promise<void> {
    progressTracker.updateStage('ANALYZING', 'Validating data before migration')
    ...
    if (!validation.valid && validation.errors.some((e) => e.severity === 'ERROR')) {
      throw new Error('Pre-migration validation failed: ' + validation.errors[0].message)
    }
  ...
  private async startZeroDowntimeMigration(userId: string): Promise<void> {
    progressTracker.updateStage('MIGRATING', 'Starting zero-downtime migration')
  ...
  private async completeMigration(): Promise<void> {
    progressTracker.updateStage('VALIDATING', 'Validating migration completion')
    ...
    progressTracker.updateStage('COMPLETED', 'Migration completed successfully')
    ...
    this.currentMigration!.result = {
      ...
      duration: Date.now() - this.currentMigration!.progress.startTime.getTime(),
    }
  ...
  private saveMigrationState(): void {
    if (this.currentMigration) {
      localStorage.setItem('migration-state', JSON.stringify(this.currentMigration))
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          status: this.currentMigration.status,
          completedAt:
            this.currentMigration.status === 'COMPLETED' ? new Date().toISOString() : undefined,
        })
      )
    }
  }
