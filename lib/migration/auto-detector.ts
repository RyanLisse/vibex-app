...
export class AutoDetector extends EventEmitter {
	...
	private analyzeLocalStorage(): DetectionResult {
		...
		try {
			...
			result.itemCounts.total =
				result.itemCounts.tasks +
				result.itemCounts.environments +
				result.itemCounts.formData;
	...
	private shouldNotify(result: DetectionResult): boolean {
		...
		if (this.lastDetectionResult) {
			const hasNewData =
				result.itemCounts.total > this.lastDetectionResult.itemCounts.total;
	...
	getMigrationRecommendations(detectionResult?: DetectionResult): string[] {
		...
		if (result.itemCounts.total > 100) {
			recommendations.push(
				"Large dataset detected. Migration may take several minutes.",
			);
		}
		...
		if (result.storageSize > 5 * 1024 * 1024) {
			// 5MB
			recommendations.push(
				"Storage usage is high. Consider cleaning up old data before migration.",
			);
		}

		if (result.dataTypes.includes("formData")) {
			recommendations.push(
				"Form data detected. Review and save any unsaved work before migration.",
			);
		}

		if (result.lastModified) {
			const hoursSinceModified =
				(Date.now() - result.lastModified.getTime()) / (1000 * 60 * 60);
