...
export class DataMapper {
					...
					try {
						...
						if (rule.validate && !rule.validate(sourceValue)) {
							if (rule.defaultValue !== undefined) {
								...
								warnings.push(
									`Task ${i}: Using default value for ${rule.field}`,
								);
								continue;
							}
							errors.push({
					...
					try {
						...
						if (rule.validate && !rule.validate(sourceValue)) {
							if (rule.defaultValue !== undefined) {
								...
								warnings.push(
									`Environment ${i}: Using default value for ${rule.field}`,
								);
								continue;
							}
							errors.push({
	...
	generateMigrationStats(data: {
		...
		const totalTransformed =
			data.transformedTasks + data.transformedEnvironments;

		return {
			totalOriginalItems: totalOriginal,
			totalTransformedItems: totalTransformed,
			successRate:
				totalOriginal > 0 ? (totalTransformed / totalOriginal) * 100 : 0,
			errorRate: totalOriginal > 0 ? (data.errors / totalOriginal) * 100 : 0,
			warningRate:
				totalOriginal > 0 ? (data.warnings / totalOriginal) * 100 : 0,
			conflictRate:
				totalTransformed > 0 ? (data.conflicts / totalTransformed) * 100 : 0,
		};
	}
}
