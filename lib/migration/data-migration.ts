...
export class DataMigrationManager {
	...
	async checkMigrationNeeded(): Promise<{
		...
		try {
			...
			const localTasks = taskStoreData
				? JSON.parse(taskStoreData)?.state?.tasks || []
				: [];
			...
			const [dbEnvironmentsCount] = await db
			...
			const needed =
				localStorageData.tasks > 0 || localStorageData.environments > 0;
		...
		try {
			...
			await observability.events.collector.collectEvent(
				...
				`Data migration completed for user ${userId}`,
				{
					...
					userId,
					duration:
						this.currentMigration.endTime.getTime() - startTime.getTime(),
