...
export class SlackTransport implements AlertTransport {
  ...
  async send(
    ...
    if (config.webhookUrl) {
      ...
      throw new Error('Either webhookUrl or botToken must be provided for Slack transport')
    }
  }
  ...
  private buildAttachment(error: CriticalError, notification: AlertNotification): SlackAttachment {
