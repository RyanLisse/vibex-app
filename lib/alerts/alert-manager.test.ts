...
describe("AlertManager", () => {
	...
	describe("processAlert", () => {
		...
		it("should skip processing when alerts are disabled", async () => {
			...
			expect(mockLogger.debug).toHaveBeenCalledWith("Alert processing disabled", {
				errorId: criticalError.id,
			});
	...
	describe("findDuplicateAlert", () => {
		...
		it("should find duplicate in Redis when not in memory", async () => {
			...
			expect(duplicate).toEqual(existingAlert);
			expect(mockRedis.get).toHaveBeenCalledWith(expect.stringContaining("alert_dedup:"));
		});
	...
	describe("getAlertHistory", () => {
		it("should retrieve alert history from Redis", async () => {
			...
			mockRedis.get.mockResolvedValueOnce(alertData1).mockResolvedValueOnce(alertData2);
		...
		it("should handle missing alert data gracefully", async () => {
			...
			mockRedis.zrevrange.mockResolvedValue(alertIds);
			mockRedis.get.mockResolvedValueOnce(alertData1).mockResolvedValueOnce(null); // Missing data
	...
	describe("Integration Scenarios", () => {
		it("should handle complete alert lifecycle", async () => {
			...
			// Get active alerts
			const activeAlerts = await alertManager.getActiveAlerts();
			expect(activeAlerts).toContain(expect.objectContaining({ id: criticalError.id }));
			...
			expect(remainingActive).not.toContain(expect.objectContaining({ id: criticalError.id }));
		});
	});
});
