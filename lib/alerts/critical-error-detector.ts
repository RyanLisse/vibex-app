...
export class CriticalErrorDetector {
  ...
  private initializeTemplates(): void {
    ...
    this.templates.set(CriticalErrorType.REDIS_CONNECTION_FAILURE, {
      ...
      message: 'Redis connection has failed. Caching and session management may be impacted.',
      severity: 'critical' as EventSeverity,
      priority: 'high',
    })
    ...
    this.templates.set(CriticalErrorType.AUTH_SERVICE_FAILURE, {
      ...
      message: 'Authentication service has failed. Users may not be able to log in.',
      severity: 'critical' as EventSeverity,
      priority: 'critical',
    })
    ...
    this.templates.set(CriticalErrorType.WORKFLOW_EXECUTION_FAILURE, {
      ...
      message: 'Workflow execution has failed. Automated processes may be impacted.',
  ...
  detectCriticalError(logEntry: LogEntry): CriticalError | null {
    ...
    if (this.isHighFrequencyError(logEntry)) {
      return this.createCriticalError(CriticalErrorType.ERROR_RATE_THRESHOLD_EXCEEDED, logEntry)
    }
  ...
  private isHighFrequencyError(logEntry: LogEntry): boolean {
    ...
    const message = logEntry.message || ''
    return /error.*rate.*exceeded/i.test(message) || /too.*many.*errors/i.test(message)
  }

  private createCriticalError(type: CriticalErrorType, logEntry: LogEntry): CriticalError {
    ...
    return {
      ...
      type,
      message: logEntry.message || template?.message || 'Critical error detected',
