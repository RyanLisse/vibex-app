...
export class ExecutionComparisonEngine {
			...
			allPaths.forEach((path) => {
				if (
					!(
						...
						ignorePaths.some((p) => path.startsWith(p))
					)
				) {
					differences.push({
						path,
	...
	private calculateStatistics(
		differences: StateDifference[],
	): ExecutionComparison["statistics"] {
		const stats = {
			totalDifferences: 0,
			addedPaths: 0,
			removedPaths: 0,
		...
		stats.similarityScore =
			totalPaths > 0 ? Math.round((1 - changedPaths / totalPaths) * 100) : 100;
	...
	private async generateInsights(
		...
		const memoryDiff = differences.filter(
			(d) => d.path.startsWith("memory") && d.type === "added",
		);
		...
		const outputDiff = differences.filter(
			(d) => d.path.startsWith("outputs") && d.type === "added",
		);
		...
		metrics.forEach(({ key, threshold, unit }) => {
			...
			if (Math.abs(percentChange) > threshold) {
				...
				if (diff > 0) {
					degraded = true;
					changes.push(
						`${key} increased by ${Math.round(percentChange * 100)}% (+${diff} ${unit})`,
					);

					if (key === "memoryUsage") {
						recommendations.push(
							"Review memory allocations and potential leaks",
						);
	...
	private generatePatternDescription(snapshots: ExecutionSnapshot[]): string {
		...
		const operations = new Set(
			snapshots.map((s) => s.state.currentOperation?.type).filter(Boolean),
		);
	...
	private extractCharacteristics(
		...
		const characteristics: Record<string, any> = {
			...
			averageStep: Math.round(
				snapshots.reduce((sum, s) => sum + s.state.currentStep, 0) /
					snapshots.length,
			),
