...
export class DebugSessionManager {
  ...
  async createSession(
    ...
    try {
      ...
      const checkpoints = snapshots.filter((s) => s.checkpoint).map((s) => s.stepNumber)
  ...
  async removeBreakpoint(sessionId: string, stepNumber: number): Promise<void> {
    ...
    session.metadata.breakpoints = session.metadata.breakpoints.filter((bp) => bp !== stepNumber)
   ...
   */
  async removeWatchedVariable(sessionId: string, variable: string): Promise<void> {
    const session = this.sessions.get(sessionId)
    if (!session) {
      throw new Error(`Debug session ${sessionId} not found`)
    }

    session.metadata.watchedVariables = session.metadata.watchedVariables.filter(
      (v) => v !== variable
    )
  ...
  async stepTo(sessionId: string, stepNumber: number): Promise<ExecutionSnapshot | null> {
    ...
    try {
      ...
      const replaySession = timeTravel.replay.getSession(session.replaySessionId)
      if (!replaySession) {
  ...
  async stepForward(sessionId: string): Promise<ExecutionSnapshot | null> {
    ...
    const nextStep = Math.min(session.currentStepNumber + 1, session.totalSteps - 1)
   ...
   */
  async continueToBreakpoint(sessionId: string): Promise<ExecutionSnapshot | null> {
    const session = this.sessions.get(sessionId)
    if (!session) {
      throw new Error(`Debug session ${sessionId} not found`)
    }

    const nextBreakpoint = session.metadata.breakpoints.find((bp) => bp > session.currentStepNumber)
  ...
  async exportSession(sessionId: string): Promise<any> {
    ...
    const snapshots = await timeTravel.snapshots.getExecutionSnapshots(session.metadata.executionId)
  ...
  async closeSession(sessionId: string): Promise<void> {
    ...
    await observability.recordEvent('debug.session.closed', {
      ...
      duration: session.metadata.endedAt
        ? session.metadata.endedAt.getTime() - session.metadata.startedAt.getTime()
   ...
   */
  private async handleBreakpoint(sessionId: string, stepNumber: number): Promise<void> {
    const session = this.sessions.get(sessionId)
    if (!session) return
