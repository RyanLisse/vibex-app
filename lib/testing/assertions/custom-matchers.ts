...
const toBeValidUser: MatcherFunction<[received: unknown]> = function (
	received,
) {
	const { isNot } = this;
...
const toBeValidProject: MatcherFunction<[received: unknown]> = function (
	received,
) {
	const { isNot } = this;
...
const toHaveValidSchema: MatcherFunction<[received: unknown, schema: any]> =
...
const toBeWithinTimeRange: MatcherFunction<
	[received: Date, target: Date, toleranceMs: number]
> = function (received, target, toleranceMs) {
	...
	if (
		...
		throw new Error(
			"Expected target to be Date object, valid date string, or timestamp",
		);
	}
...
const toHaveValidApiResponse: MatcherFunction<[received: unknown]> = function (
	received,
) {
	const { isNot } = this;
...
const toMatchSnapshot: MatcherFunction<[received: unknown, name?: string]> =
...
const toBeAccessible: MatcherFunction<[received: unknown]> = function (
	received,
) {
	const { isNot } = this;
	...
	const hasTabIndex =
		element.attributes && Object.hasOwn(element.attributes, "tabindex");
...
> = function (received, thresholds) {
	...
	if (
		...
		metrics.memoryUsage > thresholds.memoryUsage.max
	) {
		failures.push(
...
// Async matchers
const toResolveWithin: MatcherFunction<
	[received: Promise<any>, timeoutMs: number]
...
> = async function (received, expected, options = {}) {
	...
	if (!isNot) {
		throw new Error(
			`Expected value to eventually equal ${expected} within ${timeout}ms`,
		);
	}
	...
	return {
		pass: true,
		message: () =>
			`Expected value not to eventually equal ${expected} within ${timeout}ms`,
	};
};
...
function validateSchema(data: any, schema: any): boolean {
	...
	for (const key in schema) {
		...
		if (expectedType && typeof expectedType.asymmetricMatch === "function") {
			...
			expectedType.constructor === RegExp
		) {
			...
			if (
				...
				!expectedType.sample.test(actualValue)
			) {
				return false;
			}
