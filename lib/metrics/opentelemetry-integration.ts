...
export class OpenTelemetryPrometheusIntegration {
	...
	static getInstance(
		...
		if (!OpenTelemetryPrometheusIntegration.instance) {
			OpenTelemetryPrometheusIntegration.instance =
				new OpenTelemetryPrometheusIntegration(config);
		}
	...
	recordSystemMetricsDual(
		...
		this.observability.metrics.recordOperation(
			`system_${metricType}_${operation}`,
			duration,
		);
	}
	...
	createMetricsMiddleware() {
		return async (req: any, res: any, next: any) => {
			...
			const correlationId =
				req.headers["x-correlation-id"] || `req_${Date.now()}`;
	...
	async healthCheck(): Promise<{
		...
		try {
			...
			const observabilityHealthy =
				this.observability.metrics.getOperationCount() >= 0;

			const allHealthy =
				prometheusHealthy && otelHealthy && observabilityHealthy;
