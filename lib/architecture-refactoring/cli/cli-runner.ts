...
function printSummary(report: any) {
  ...
  for (const category of report.categories) {
    console.log(`${category.category}: ${category.issueCount} issues (${category.criticalCount} critical)`);
  }
  ...
  for (const resource of report.migrationPlan.requiredResources) {
    console.log(`• ${resource.count} ${resource.skillLevel} ${resource.type}(s) for ${resource.duration} days`);
  }
}
