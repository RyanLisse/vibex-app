...
export class ReportGenerator {
    ...
    return {
      ...
      security: {
        ...
        criticalVulnerabilities: results.filter(r => 
          r.category === 'security' && r.severity === 'critical'
        ).length,
  ...
  private createMigrationPhases(prioritized: PrioritizedRecommendation[]): MigrationPhase[] {
    ...
    const phase1Items = prioritized.filter(p => 
    ...
    const phase2Items = prioritized.filter(p => 
      p.result.effort.complexity === 'moderate' &&
      p.result.severity !== 'critical'
    );
    ...
    const phase3Items = prioritized.filter(p => 
      p.result.effort.complexity === 'complex' ||
      p.result.effort.complexity === 'very-complex'
    );
...
`;
  ...
  private calculateAverageComplexity(results: AnalysisResult[]): number {
    const complexityResults = results.filter(r => 
      r.metadata && r.metadata.metrics && r.metadata.metrics.cyclomaticComplexity
    );

    if (complexityResults.length === 0) return 0;

    const totalComplexity = complexityResults.reduce((sum, r) => 
      sum + r.metadata.metrics.cyclomaticComplexity, 0
    );
