...
export class WorkflowErrorClassifier {
  private static classifications = new Map<WorkflowErrorCode, ErrorClassification>([
    [
...
export class RecoveryExecutor {
  ...
  async executeRecovery(error: any, context: RecoveryContext): Promise<RecoveryResult> {
    ...
    switch (classification.strategy) {
      ...
      case RecoveryStrategy.CIRCUIT_BREAKER:
        return await this.executeWithCircuitBreaker(error, context, classification)
  ...
  private async executeFallback(error: any, context: RecoveryContext): Promise<RecoveryResult> {
  ...
  private async executeCompensation(error: any, context: RecoveryContext): Promise<RecoveryResult> {
...
export class ErrorAggregator {
  ...
  private groupBySeverity(errors: WorkflowError[]): Record<string, number> {
    ...
    errors.forEach((error) => {
      const classification = WorkflowErrorClassifier.classify(error)
      severities[classification.severity] = (severities[classification.severity] || 0) + 1
    })
  ...
  private getLatest(errors: WorkflowError[]): WorkflowError {
    return errors.reduce((latest, error) => (error.timestamp > latest.timestamp ? error : latest))
  }
}
