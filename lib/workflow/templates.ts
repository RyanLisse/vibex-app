...
export class WorkflowTemplateRegistry {
	...
	getByTags(tags: string[]): WorkflowTemplate[] {
		return this.getAll().filter(
			(t) => t.tags?.some((tag) => tags.includes(tag)) ?? false,
		);
	}
...
templateRegistry.register({
	...
	description:
		"Process data through extraction, transformation, and loading stages",
	category: "data",
	...
	definition: {
		...
		steps: [
			...
			},
			{
				...
				condition: {
					expression:
						"aggregatedData != null && Object.keys(aggregatedData).length > 0",
...
export const TEMPLATE_CATEGORIES = [
	{
		...
		description: "Templates for ETL and data workflows",
	},
	{
		id: "business",
		name: "Business Process",
		description: "Templates for business workflows",
	},
	{
		id: "resilience",
		name: "Resilience",
		description: "Templates for fault-tolerant workflows",
	},
	{
		id: "operations",
		name: "Operations",
		description: "Templates for operational workflows",
	},
	{
		id: "integration",
		name: "Integration",
		description: "Templates for system integration",
	},
];
...
export function suggestTemplates(useCase: string): WorkflowTemplate[] {
	...
	return templates
		.map((template) => {
			...
			// Check description
			if (
				keywords.some((k) => template.description.toLowerCase().includes(k))
			) {
				score += 2;
			}
