...
export class WorkflowExecutionEngine {
   ...
   */
  async resumeExecution(executionId: string, fromStep?: string): Promise<ExecutionContext> {
        ...
        try {
          ...
          if (stepState.retryCount < (step.retryPolicy?.maxRetries || 3)) {
            ...
            const backoffMs = step.retryPolicy?.exponentialBackoff
              ? (step.retryPolicy?.backoffMs || 1000) * 2 ** (stepState.retryCount - 1)
  ...
  private areDependenciesMet(dependencies: string[], context: ExecutionContext): boolean {
    return dependencies.every((depId) => context.stepStates[depId]?.status === 'completed')
  }
