...
import { workflows, workflowExecutions, executionSnapshots } from '@/db/schema';
...
export class WorkflowExecutionEngine {
  ...
  private stepExecutors = new Map<string, (step: any, context: ExecutionContext) => Promise<any>>();
  ...
  async resumeExecution(
    ...
    // Resume execution
    this.executeWorkflow(executionId, definition, workflow.config).catch(error => {
    ...
    try {
      ...
      for (const step of steps) {
        ...
        try {
          ...
          if (definition.config.enableCheckpoints && 
          ...
          if (stepState.retryCount < (step.retryPolicy?.maxRetries || 3)) {
            ...
            const backoffMs = step.retryPolicy?.exponentialBackoff 
              ? (step.retryPolicy?.backoffMs || 1000) * Math.pow(2, stepState.retryCount - 1)
   ...
   */
  private areDependenciesMet(dependencies: string[], context: ExecutionContext): boolean {
    return dependencies.every(depId => 
      context.stepStates[depId]?.status === 'completed'
    );
  }
   ...
   */
  private async createCheckpoint(executionId: string, context: ExecutionContext): Promise<void> {
    const checkpoint = {
      variables: context.variables,
      stepStates: context.stepStates,
      currentStep: context.currentStep,
  ...
  private evaluateCondition(condition: string, variables: Record<string, any>): boolean {
    ...
    try {
      ...
      Object.keys(variables).forEach(key => {
        expression = expression.replace(new RegExp(`\\$${key}\\b`, 'g'), JSON.stringify(variables[key]));
      });
