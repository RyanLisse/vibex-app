...
export class MetricsAnalyzer {
   ...
   */
  static async calculateHealthScore(timeRange: { start: Date; end: Date }): Promise<{
    overall: number
    components: {
      database: number
      sync: number
    ...
    const totalEvents = await db
      ...
      .from(eventsTable)
      .where(
        and(gte(eventsTable.timestamp, timeRange.start), lte(eventsTable.timestamp, timeRange.end))
      )

    const errorRate = Number(errorEvents[0]?.count || 0) / Number(totalEvents[0]?.count || 1)
    ...
    const overall =
      (components.database + components.sync + components.wasm + components.queries) / 4
...
export const metrics = {
  ...
  syncLatency: (latency: number, syncType: string) =>
    PerformanceMetricsCollector.getInstance().recordMetric('sync_latency', latency, { syncType }),

  // WASM metrics
  wasmInitTime: (initTime: number, service: string) =>
    PerformanceMetricsCollector.getInstance().recordMetric('wasm_init_time', initTime, { service }),

  wasmExecutionTime: (executionTime: number, operation: string) =>
    PerformanceMetricsCollector.getInstance().recordMetric('wasm_execution_time', executionTime, {
      operation,
    }),

  // System metrics
  memoryUsage: (usage: number, component: string) =>
    PerformanceMetricsCollector.getInstance().recordMetric('memory_usage', usage, { component }),

  cacheHitRate: (hitRate: number, cacheType: string) =>
    PerformanceMetricsCollector.getInstance().recordMetric('cache_hit_rate', hitRate, {
      cacheType,
    }),
