...
export class AgentActivityTracker extends EventEmitter {
	...
	getSystemOverview(): {
		...
		const activeAgents = agents.filter((a) =>
			["active", "processing"].includes(a.status),
		);

		const totalCpu = agents.reduce(
			(sum, a) => sum + a.metrics.resourceUsage.cpu,
			0,
		);
		const totalMemory = agents.reduce(
			(sum, a) => sum + a.metrics.resourceUsage.memory,
			0,
		);
		const totalNetwork = agents.reduce(
			(sum, a) => sum + a.metrics.resourceUsage.network,
			0,
		);
	...
	getMigrationProgress(): {
		...
		const migrationAgents = Array.from(this.agents.values()).filter(
			(agent) =>
				agent.currentTask &&
				agent.currentTask.name.toLowerCase().includes("migration"),
		);
		...
		const overallProgress =
			agentProgress.length > 0 ? totalProgress / agentProgress.length : 0;

		const blockers = migrationAgents
			.filter(
				(a) => a.currentTask?.blockers && a.currentTask.blockers.length > 0,
			)
	...
	private updateAverageTaskTime(agent: AgentActivity): void {
		...
		agent.metrics.averageTaskTime =
			(agent.metrics.averageTaskTime * (totalTasks - 1) + taskDuration) /
			totalTasks;
	}
