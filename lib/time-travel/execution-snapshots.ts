...
export class ExecutionSnapshotManager {
	...
	async captureSnapshot(
		...
		const snapshot: ExecutionSnapshot = {
			...
			metadata: {
				...
				isCheckpoint,
				canRollback:
					isCheckpoint || type === "checkpoint" || type === "rollback_point",
			},
		};
		...
		if (previousSnapshot) {
			snapshot.metadata.diffFromPrevious = this.calculateStateDiff(
	...
	private async flushSnapshots(executionId?: string): Promise<void> {
		const executionsToFlush = executionId
			? [executionId]
			: Array.from(this.snapshotBuffer.keys());
		...
		for (const execId of executionsToFlush) {
			...
			try {
				...
				console.error(
					`Failed to flush snapshots for execution ${execId}:`,
					error,
				);
