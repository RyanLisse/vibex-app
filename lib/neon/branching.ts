...
export class NeonBranchingManager {
  ...
  async getBranchEndpoints(branchId: string): Promise<NeonEndpoint[]> {
    ...
    const result = await response.json()
    return result.endpoints.map((endpoint: any) => this.mapEndpointResponse(endpoint))
  }
  ...
  async runTestsInBranch<T>(
    ...
    try {
      ...
      if (cleanupOnError && testBranch) {
        try {
          await this.branchingManager.deleteBranch(testBranch.id)
          console.log(`🗑️ Test branch cleaned up after error: ${testBranch.name}`)
        } catch (cleanupError) {
...
export const createNeonBranchingManager = (config: { apiKey: string; projectId: string }) => {
