 ...
 */
export function handleAPIRouteError(error: unknown, options: ErrorHandlerOptions): NextResponse {
	const {
		route,
		span,
	...
	if (error && typeof error === "object" && "statusCode" in error) {
		...
		return NextResponse.json(
			createApiErrorResponse(apiError.message, apiError.statusCode, apiError.details),
	...
	return NextResponse.json(createApiErrorResponse(genericErrorMessage, 500, details), {
		status: 500,
	});
}
