...
export class QueryBuilder<T> {
  ...
  applyOptions(options: QueryOptions): this {
    ...
    if (options.search && options.search.query) {
      const searchFields = options.search.fields.map((field) => this.table[field]).filter(Boolean)
  ...
  async executePaginated(): Promise<QueryResult<T>> {
    ...
    const page =
      this.offsetValue && this.limitValue ? Math.floor(this.offsetValue / this.limitValue) + 1 : 1
