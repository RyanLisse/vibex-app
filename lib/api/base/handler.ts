...
export class BaseAPIHandler {
    ...
    try {
      ...
      if (process.env.NEXT_PHASE !== 'phase-production-build') {
        try {
          ...
          observability.metrics.httpRequestDuration(duration, context.method, context.path, 200)
        } catch (error) {
      ...
      if (error instanceof BaseAPIError) {
        return NextResponse.json(ResponseBuilder.error(error), { status: error.statusCode })
      }
      ...
      return NextResponse.json(ResponseBuilder.error(internalError), { status: 500 })
    }
  }
   ...
   */
  static async validateBody<T>(request: NextRequest, schema: z.ZodSchema<T>): Promise<T> {
    try {
   ...
   */
  static validateQuery<T>(searchParams: URLSearchParams, schema: z.ZodSchema<T>): T {
