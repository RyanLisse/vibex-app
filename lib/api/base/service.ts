...
export abstract class BaseAPIService {
  ...
  protected async executeDatabase<T>(operation: string, fn: () => Promise<T>): Promise<T> {
    try {
      ...
      throw new DatabaseError(`Database operation failed: ${operation}`, error as Error)
    }
  }
   ...
   */
  protected async recordEvent(action: string, message: string, data?: Record<string, any>) {
    if (process.env.NEXT_PHASE !== 'phase-production-build') {
      try {
        const { observability } = await import('@/lib/observability')
 ...
 */
export abstract class BaseCRUDService<T, CreateDTO, UpdateDTO> extends BaseAPIService {
   ...
   */
  abstract update(id: string, data: UpdateDTO, context: ServiceContext): Promise<T>
