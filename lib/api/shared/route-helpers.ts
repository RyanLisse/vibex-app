...
>(
  ...
  return withErrorHandling(async (
    ...
    const tracer = finalConfig.enableTracing 
      ? trace.getTracer(finalConfig.serviceName!)
      : null;
    ...
    try {
      ...
      const responseData = createApiSuccessResponse(
        result,
        getSuccessMessage(config.method)
      );
...
// PUT update resource
export function createPutHandler<TBody, TResponse>(
  config: {
    serviceName: string;
    bodySchema: z.ZodSchema<TBody>;
 ...
 */
export function createCRUDRoutes<TModel, TCreateData, TUpdateData>(
  config: {
    serviceName: string;
    ...
    service: {
      ...
      getList: (options: any) => Promise<{ items: TModel[]; total: number; page: number; limit: number }>;
