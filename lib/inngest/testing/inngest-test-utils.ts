...
export class InngestTestEventFactory {
   ...
   */
  static createTaskEvent(taskId: string, data: Record<string, any> = {}): EventPayload {
    return {
      name: 'task/created',
      data: {
        taskId,
   ...
   */
  static createAICompletionEvent(sessionId: string, data: Record<string, any> = {}): EventPayload {
    return {
      name: 'ai/completion',
      data: {
        sessionId,
   ...
   */
  static createErrorEvent(error: string, context: Record<string, any> = {}): EventPayload {
    return {
      name: 'system/error',
      data: {
        error,
   ...
   */
  static createBatchEvents(count: number, eventFactory: () => EventPayload): EventPayload[] {
...
export class InngestFunctionTester {
  ...
  async executeAndExpectFailure(event: EventPayload, expectedError?: string) {
    ...
    if (execution.status !== 'failed') {
      throw new Error(`Expected function to fail, but got status: ${execution.status}`)
    }
   ...
   */
  async validateTiming(event: EventPayload, expectedMinMs: number, expectedMaxMs: number) {
    ...
    if (elapsed > expectedMaxMs) {
      throw new Error(`Function took too long: ${elapsed}ms (expected maximum: ${expectedMaxMs}ms)`)
    }
...
export class InngestWorkflowTester {
   ...
   */
  async executeWorkflow(events: Array<{ functionName: string; event: EventPayload }>) {
    const results = []
   ...
   */
  async executeParallel(executions: Array<{ functionName: string; event: EventPayload }>) {
...
export class MockInngestClient {
  ...
  assertEventSent(name: string, data?: Partial<Record<string, any>>) {
    ...
    if (events.length === 0) {
      throw new Error(`Expected event "${name}" to be sent, but no events found`)
    }

    if (data) {
      const matchingEvent = events.find((event) => {
        return Object.entries(data).every(([key, value]) => event.data[key] === value)
      })
...
export const InngestTestHelpers = {
   ...
   */
  createDelayedEvent: (event: EventPayload, delayMs: number): Promise<EventPayload> => {
