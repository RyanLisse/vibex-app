	...
	async ({ event, step }) => {
		...
		const result = await step.run("process-alert", async () => {
			try {
				await alertService
					.getAlertManager()
					.processAlert(criticalError, config);
	...
	async ({ event, step }) => {
		...
		const result = await step.run("send-escalation-notifications", async () => {
			try {
				...
				await alertService
					.getAlertManager()
					.processAlert(alert, escalationConfig);
				...
				return {
					success: true,
					alertId,
					escalatedChannels: escalationChannels.length,
				};
			} catch (error) {
...
export const scheduleEscalation = inngest.createFunction(
	...
	async ({ event, step }) => {
		const { alertId, escalateAfterMinutes, escalationChannels } =
			event.data as {
				alertId: string;
...
export const retryFailedNotification = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		const notificationData = await step.run(
		...
		if (notificationData.status === AlertNotificationStatus.DELIVERED) {
			logger.info("Notification already delivered, skipping retry", {
				notificationId,
			});
		...
		const result = await step.run("retry-notification", async () => {
			try {
				...
				await alertService
					.getTransportService()
					.send(channel, alert, retryNotification);
	...
	async ({ event, step }) => {
		...
		const metrics = await step.run("calculate-metrics", async () => {
			try {
				...
				const last24HourAlerts = allAlerts.filter(
					(a) => a.timestamp >= last24Hours,
				);
				const last7DayAlerts = allAlerts.filter(
