...
export class ContainerUseIntegration {
  ...
  async createAgentTask(params: {
    ...
    try {
      ...
      const envResult = await this.modalManager.createAgentEnvironment(task.id, modalConfig);
  ...
  async completeAgentTask(taskId: string): Promise<{
    ...
    try {
      ...
      const environments = await this.modalManager.cleanupEnvironment(worktree.id);
