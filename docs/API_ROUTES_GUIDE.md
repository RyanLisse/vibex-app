...
const GetTasksQuerySchema = z.object({
  ...
  sortBy: z.enum(['created_at', 'updated_at', 'title', 'priority']).default('created_at'),
...
class TasksService {
  static async getTasks(params: z.infer<typeof GetTasksQuerySchema>) {
    ...
    try {
      ...
      if (params.search) conditions.push(like(tasks.title, `%${params.search}%`))
      ...
      throw new TasksAPIError('Failed to fetch tasks', 500, 'FETCH_TASKS_ERROR')
    } finally {
      span.end()
    }
  }
...
// Usage
export const DELETE = withPermissions(['tasks:delete'])(
...
// Compose schemas
const GetResourcesSchema = PaginationSchema
  .merge(SortingSchema)
  .merge(SearchSchema)
  .extend({
...
const ulidSchema = z.string().refine(
  (val) => /^[0-9A-HJKMNP-TV-Z]{26}$/.test(val),
...
export function withRateLimit(
  ...
  return function(handler: RouteHandler) {
    return async (request: NextRequest, ...args: any[]) => {
      ...
      const { allowed, remaining, resetTime } = await rateLimit(key, limit, window)
      
      if (!allowed) {
        return NextResponse.json(
          createApiErrorResponse('Rate limit exceeded', 429, 'RATE_LIMIT_EXCEEDED'),
          { 
            status: 429,
            headers: {
