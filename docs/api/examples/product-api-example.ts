...
export const POST = BaseAPIHandler.POST(
  async (context) => {
    ...
    const product = await productService.createProduct({ ...validated, imageUrl }, context)
...
class ProductService extends BaseAPIService {
  ...
  async searchProducts(params: any, fields: string[] | undefined, context: ServiceContext) {
    return this.executeWithTracing('searchProducts', context, async (span) => {
      ...
      if (params.search) {
        query.search([products.name, products.description, products.category], params.search)
      }
      ...
      span.setAttributes({
        ...
        'search.filters': Object.keys(params).filter((k) => params[k] !== undefined).length,
  ...
  async createProduct(data: any, context: ServiceContext): Promise<Product> {
    return this.executeWithTracing('createProduct', context, async (span) => {
      ...
      // Record event
      await this.recordEvent('product_created', `New product created: ${product.name}`, {
  ...
  async uploadProductImage(file: File, context: ServiceContext): Promise<string> {
    return this.executeWithTracing('uploadProductImage', context, async (span) => {
      // Validate file
      ...
      if (!ALLOWED_TYPES.includes(file.type)) {
        throw new ValidationError('Invalid image type (JPEG, PNG, WebP only)')
      }
  ...
  // Batch operations
  async batchUpdatePrices(updates: Array<{ id: string; price: number }>, context: ServiceContext) {
    return this.executeWithTracing('batchUpdatePrices', context, async (span) => {
      ...
      await this.recordEvent('batch_price_update', `Updated prices for ${succeeded} products`, {
        succeeded,
        failed,
...
export const POST = BaseAPIHandler.POST(
  async (context) => {
    ...
    const { updates } = await BaseAPIHandler.validateBody(context.request, BatchUpdateSchema)
