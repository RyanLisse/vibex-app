...
```typescript
async function executeRollback(migrationId: string): Promise<RollbackResult> {
  const steps = [
...
```typescript
async function restoreLocalStorage(migrationId: string): Promise<void> {
  const backup = await getBackupForMigration(migrationId);
...
async function handlePartialRollback(
  ...
  const hasCriticalFailure = failedSteps.some((step) =>
    criticalSteps.includes(step),
  );
