...
function isCompletedStreamMessage(message: unknown): message is IncomingMessage & {
                    ...
                    >
                      {isAssistant ? (
                        <div className="prose prose-sm dark:prose-invert max-w-none overflow-hidden">
                          <Markdown
                            repoUrl={
                              task?.repository ? `https://github.com/${task.repository}` : undefined
                            }
                            branch={task?.branch}
                          >
                            {message.data?.text as string}
                            ...
                            {typeof message.data.chunkIndex === 'number' &&
                              typeof message.data.totalChunks === 'number' && (
                                <span className="text-[10px] text-muted-foreground/60 font-mono">
                                  {Math.round(
                                    ((message.data.chunkIndex + 1) / message.data.totalChunks) * 100
                                  )}
                                  %
                                </span>
                              )}
                    ...
                    <div className="bg-card border border-border rounded-2xl px-5 py-3 shadow-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <TextShimmer className="text-sm">
                          {task?.statusMessage ? `${task.statusMessage}` : 'Working on task...'}
                        </TextShimmer>
                      </div>
                    </div>
                  </div>
