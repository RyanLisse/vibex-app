	...
	try {
		...
		switch (action) {
			case "start_voice_brainstorm": {
				...
				const enhancedPrompt = generateVoiceBrainstormPrompt(
					topic,
					creativityLevel,
					focusAreas,
				);
			...
			case "generate_insights": {
				...
				const nextStageRecommendations = getNextStageRecommendations(
					stage,
					insights,
				);
...
function generateVoiceBrainstormPrompt(
	...
	const creativityInstructions = {
		conservative:
			"Focus on practical, proven approaches. Prioritize feasibility.",
		balanced:
...
function extractIdeasFromTranscript(transcript: string): Array<{
	...
	const uniqueIdeas = ideas.filter(
		(idea, index, self) =>
			index ===
			self.findIndex(
				(i) => i.content.toLowerCase() === idea.content.toLowerCase(),
			),
	);
...
function analyzeIdeasWithAI(
	ideas: Array<{ content: string; source: string }>,
): {
	scores: Array<{
		content: string;
		feasibility: number;
...
function generateStageInsights(
	...
	switch ((session as any).type) {
		case "exploration":
			...
			if (ideas.length > 0) {
				insights.push(
					`Generated ${ideas.length} initial ideas from this input`,
				);
			}
			break;
...
function getNextStageRecommendations(
	stage: string,
	_insights: unknown,
): string[] {
	const stageMap: Record<string, string[]> = {
		...
		evaluation: [
			"Begin refinement - combine best elements",
			"Address identified weaknesses",
		],
		refinement: [
			"Create action plan with concrete steps",
			"Define timelines and resource needs",
		],
	};
