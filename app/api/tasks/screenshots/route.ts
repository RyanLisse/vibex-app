...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      createApiErrorResponse('Failed to create bug report', 500, 'CREATE_BUG_REPORT_ERROR'),
      { status: 500 }
    )
  } finally {
    span.end()
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      createApiErrorResponse('Failed to fetch screenshot tasks', 500, 'FETCH_SCREENSHOTS_ERROR'),
      { status: 500 }
    )
  } finally {
    span.end()
