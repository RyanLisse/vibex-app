...
class TasksService {
  ...
  static async createTask(taskData: z.infer<typeof CreateTaskSchema>) {
    ...
    try {
      ...
      throw new TasksAPIError('Failed to create task', 500, 'CREATE_TASK_ERROR')
    } finally {
      span.end()
    }
  }
  ...
  static async updateTask(id: string, updates: z.infer<typeof UpdateTaskSchema>) {
    ...
    try {
      ...
      throw new TasksAPIError('Failed to update task', 500, 'UPDATE_TASK_ERROR')
    } finally {
      span.end()
    }
  }
  ...
  static async deleteTask(id: string) {
    ...
    try {
      ...
      throw new TasksAPIError('Failed to delete task', 500, 'DELETE_TASK_ERROR')
    } finally {
      span.end()
    }
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(createApiSuccessResponse(task, 'Task created successfully'), {
      status: 201,
    })
  } catch (error) {
    if (error instanceof z.ZodError) {
