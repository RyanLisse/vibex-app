...
export async function GET(request: NextRequest) {
  return BaseAPIHandler.handle(request, async (context) => {
    ...
    const kanbanData = await tasksKanbanService.getKanbanBoard(queryParams, context)
...
export async function POST(request: NextRequest) {
  return BaseAPIHandler.handle(request, async (context) => {
    ...
    // Return success response
    const response = ResponseBuilder.success(result, 'Task moved successfully', context.requestId)
...
export async function PUT(request: NextRequest) {
  return BaseAPIHandler.handle(request, async (context) => {
    // Validate request body
    const body = await BaseAPIHandler.validateBody(request, KanbanBoardConfigSchema)
