...
export async function POST(request: NextRequest) {
  return BaseAPIHandler.handle(request, async (context) => {
    ...
    const response = ResponseBuilder.created(result, 'PR linked successfully', context.requestId)
...
export async function GET(request: NextRequest) {
  return BaseAPIHandler.handle(request, async (context) => {
    ...
    const result = await prIntegrationService.getPRIntegrationData(queryParams, context)
