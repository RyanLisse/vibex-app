...
const parseTaskFromTranscription = (text: string) => {
  ...
  const dueDateMatch = text.match(/(?:due|deadline|by)\s+([a-zA-Z]+\s+\d{1,2}(?:,?\s+\d{4})?)/i)
  ...
  try {
    ...
    if (!audioFile) {
      return NextResponse.json(
        createApiErrorResponse('Audio file is required', 400, 'MISSING_AUDIO_FILE'),
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      createApiErrorResponse('Failed to create voice task', 500, 'CREATE_VOICE_TASK_ERROR'),
      { status: 500 }
    )
  } finally {
    span.end()
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      createApiErrorResponse('Failed to transcribe audio', 500, 'TRANSCRIPTION_ERROR'),
      { status: 500 }
    )
  } finally {
    span.end()
