...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(createApiSuccessResponse(task, 'Task updated successfully'))
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(createApiSuccessResponse(task, 'Task deleted successfully'))
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createApiErrorResponse('Invalid task ID', 400, 'INVALID_TASK_ID', error.issues),
