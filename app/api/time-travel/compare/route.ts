...
export async function POST(request: NextRequest) {
	try {
		...
		for (let i = 0; i < snapshots.length - 1; i++) {
			...
			const comparison = await timeTravelService.compareSnapshots(
				current,
				next,
				{
		...
		return NextResponse.json({
			...
			summary: {
				...
				timespan:
					snapshots.length > 1
						? snapshots.at(-1).timestamp.getTime() -
