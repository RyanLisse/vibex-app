...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await workflowEngine.pauseExecution(executionId, pauseRequest.reason)
    ...
    return NextResponse.json({ error: 'Failed to pause execution' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to stop execution' }, { status: 500 })
  }
}
