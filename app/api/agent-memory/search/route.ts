...
export async function POST(request: NextRequest) {
	try {
		...
		const searchResults = await (vectorSearchService as any).searchMemories(
			searchRequest.query,
			{
				limit: searchRequest.limit,
		...
		return NextResponse.json({
			...
			metadata: {
				...
				avgSimilarity:
					searchResults.memories.reduce(
						(sum, m) => sum + (m.similarity || 0),
						0,
					) / searchResults.memories.length,
				maxSimilarity: Math.max(
					...searchResults.memories.map((m) => m.similarity || 0),
				),
			},
		});
	} catch (error) {
